...
// keyed by sub-benchmark name. Mirrors runsc/container/lifecycle_bench_test.go.
...
func BenchmarkShimPause(b *testing.B) {
	for platformName, conf := range benchmarkConfigs(b) {
		b.Run(platformName, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				func() {
					_, shim, id, cleanup := startSleepingSandbox(b, conf)
					defer cleanup()
					ctx := context.Background()
...
// BenchmarkShimTimeToReady measures cold-start time through the shim until the
// application outputs "ready" over a named pipe.
...
func BenchmarkShimSubcontainerEndToEnd(b *testing.B) {
	for platformName, conf := range benchmarkConfigs(b) {
		b.Run(platformName, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				func() {
					containerd, shim, sandboxID, cleanup := startSleepingSandbox(b, conf)
					defer cleanup()
