...
func TestSpec(t *testing.T) {
	for _, enableNetworking := range []bool{false, true} {
		t.Run(fmt.Sprintf("networking=%v", enableNetworking), func(t *testing.T) {
			spec, err := sandbox.Spec(sandbox.WithNetworking(enableNetworking))
			if err != nil {
				t.Fatalf("Spec(enableNet=%v) failed: %v", enableNetworking, err)
			}
...
func TestSpecEnv(t *testing.T) {
	spec, err := sandbox.Spec(sandbox.WithEnv("TEST_VAR=A", "TEST_VAR=B"))
	if err != nil {
		t.Fatalf("Spec failed: %v", err)
	}
...
func TestSpecWorkingDir(t *testing.T) {
	...
	spec, err := sandbox.Spec(sandbox.WithWorkingDir(customCwd))
	if err != nil {
		t.Fatalf("Spec failed: %v", err)
	}
