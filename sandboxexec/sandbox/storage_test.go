...
func TestFilesystemStorage(t *testing.T) {
	...
	t.Run("ListAssets", func(t *testing.T) {
		...
		// createTestSnapshot creates MetadataAsset and CheckpointAsset.
		expectedAssets := map[sandbox.Asset]bool{
			sandbox.MetadataAsset:   true,
			sandbox.CheckpointAsset: true,
		}

		if len(assets) != len(expectedAssets) {
			t.Errorf("got %d assets, want %d", len(assets), len(expectedAssets))
		}
