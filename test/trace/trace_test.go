...
func checkSyscallPoll(msg test.Message) error {
	...
	if len(p.Fds) != len(p.Events) {
		return fmt.Errorf("fds length (%d) does not match events length (%d)", len(p.Fds), len(p.Events))
	}
	for i, fd := range p.Fds {
		if fd != 0 && fd != 3 {
			return fmt.Errorf("unexpected polled fd: want 0 or 3, got %d", fd)
		}
		if fd == 0 && p.Events[i] != unix.POLLIN {
			return fmt.Errorf("unexpected event for FD 0: want POLLIN (%d), got %d", unix.POLLIN, p.Events[i])
		}
	}
	return nil
}
