...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn<RequiresCapAdminTestParams>({
        {"GetInfo",
         ...
         {SOL_IPV6, IP6T_SO_GET_INFO, std::make_shared<struct ipt_getinfo>(),
          sizeof(struct ipt_getinfo)}},
        {"GetEntries",
         {AF_INET6, SOCK_RAW, IPPROTO_RAW},
         {SOL_IPV6, IP6T_SO_GET_ENTRIES,
          std::make_shared<struct ipt_get_entries>(),
          sizeof(struct ipt_get_entries)}},
        {"SetReplace",
         {AF_INET6, SOCK_RAW, IPPROTO_RAW},
         {SOL_IPV6, IP6T_SO_SET_REPLACE, std::make_shared<struct ipt_entry>(),
          sizeof(struct ipt_entry)}},
        {"GetRevisionTarget",
         {AF_INET6, SOCK_RAW, IPPROTO_RAW},
         {SOL_IPV6, IP6T_SO_GET_REVISION_TARGET,
          std::make_shared<struct xt_get_revision>(),
          sizeof(struct xt_get_revision) + 1}},
        {"GetRevisionMatch",
         {AF_INET6, SOCK_RAW, IPPROTO_RAW},
         {SOL_IPV6, IP6T_SO_GET_REVISION_MATCH,
          std::make_shared<struct xt_get_revision>(),
