...
TEST(KeysTest, SessionKeyringAfterUnshareUserNamespace) {
  EXPECT_THAT(InForkedProcess([] {
                const int64_t id = syscall(
                    SYS_keyctl, KEYCTL_JOIN_SESSION_KEYRING, 0, 0, 0, 0);
                TEST_PCHECK(id > 0);
                ...
                TEST_CHECK_ERRNO(syscall(SYS_keyctl, KEYCTL_SETPERM, id,
                                 ...
                                 EACCES);
                TEST_CHECK_ERRNO(
                    syscall(SYS_keyctl, KEYCTL_SETPERM,
                            KEY_SPEC_SESSION_KEYRING, KEY_USR_SEARCH, 0, 0),
                    EACCES);
                _exit(0);
              }),
    ...
    if (test.accessor != KeyAccessor::kOwner) {
      ...
      ASSERT_THAT(
          syscall(SYS_setgroups,
                  test.accessor == KeyAccessor::kSupplementaryGroup, &key_gid),
          SyscallSucceeds());
...
INSTANTIATE_TEST_SUITE_P(
    , KeyPermissionTest,
    ::testing::Values(
        KeyPermissionTestCase{"Owner", KeyAccessor::kOwner, KEY_USR_SEARCH,
                              true},
        ...
        KeyPermissionTestCase{"OwnerPossessor", KeyAccessor::kOwner,
                              KEY_POS_SEARCH, true},
        KeyPermissionTestCase{"Group", KeyAccessor::kGroup, KEY_GRP_SEARCH,
                              true},
        ...
        KeyPermissionTestCase{"EmptySupplementaryGroupUsesOther",
                              ...
                              true},
        KeyPermissionTestCase{"Other", KeyAccessor::kOther, KEY_OTH_SEARCH,
                              true},
