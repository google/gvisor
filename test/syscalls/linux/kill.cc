...
// arch_do_signal_or_restart() must not cause the syscall to execute twice. Test
// that all restart-code arguments result in exactly one execution of SYS_kill,
// matching real Linux kernel behavior.
TEST(KillTest, RestartArgumentNotExecutedTwice) {
  for (int64_t arg : {-512, -513, -514, -516}) {
    // Test both raw 64-bit syscall(SYS_kill, ...) and C library kill() which
    // passes a 32-bit pid_t (where upper 32 bits may be zero-extended on
    // aarch64).
    for (bool use_libc_kill : {false, true}) {
      pid_t child = fork();
      if (child == 0) {
        TEST_PCHECK(ptrace(PTRACE_TRACEME, 0, nullptr, nullptr) == 0);
        kill(getpid(), SIGSTOP);
        if (use_libc_kill) {
          kill(static_cast<pid_t>(arg), 0);
        } else {
      ...
      while (true) {
        ...
        ASSERT_THAT(waitpid(child, &status, 0),
                    SyscallSucceedsWithValue(child));
        if (WIFEXITED(status)) {
      ...
      EXPECT_EQ(kill_count, 2)
          << "for arg " << arg << " (libc_kill=" << use_libc_kill << ")";
    }
  }
}
