...
// See test/syscalls/linux/ptrace.cc's kPtraceEventStop.
...
// W -> A -> B hierarchy with a task in A and a task in B, A's own task
// parking (while B's hasn't) wrongly propagates "settled" past A up to W;
// once B's task eventually settles too, W's unsettled-child counter goes
// permanently negative, and W reports "frozen 1" immediately on every
// later freeze, before anything has actually parked.
//
// a_pid is a plain progress-marking loop, genuinely running (unparked)
// until freeze forces it to park -- exercising the ordinary, non-ptrace
// path through applyFreezeSigLocked. b_pid is ptrace-stopped instead: a
// group-stopped task can no longer stand in for "hasn't parked yet" here
// (it's now counted as already frozen the moment freeze is ordered -- see
// the case (a) credit-skip in applyFreezeSigLocked), but a ptrace-stopped
// task isn't special-cased (case (a)'s check is specifically
// t.stop.(*groupStop), which a *ptraceStop fails), so it still holds its
// credit indefinitely until continued. This keeps both halves of the
// regression deterministic: no polling/timing needed to catch B unsettled.
TEST_F(Cgroup2Test, FreezeSettledStateAccountsForGrandchildren) {
  ...
  // b_pid: just spins until ptrace-stopped; no marker pipe needed since
  // ptrace itself proves the stop.
  pid_t b_pid = fork();
  if (b_pid == 0) {
    while (true) {
      pause();
  ...
  auto cleanup = Cleanup([&] {
    ...
    // No PTRACE_DETACH needed first -- SIGKILL alone reaps a ptrace-stopped
    // tracee, same as every other cleanup in ptrace.cc (e.g. lines
    // 297-300).
    kill(b_pid, SIGKILL);
    waitpid(b_pid, nullptr, 0);
  });

  // Stop b_pid and prove it -- via a real blocking waitpid on the
  // PTRACE_EVENT_STOP trap, not a sleep -- before entering it into B.
  // Doing this before b.Enter(b_pid) matters: entering it first would leave
  // a window where it could self-park before the stop takes effect.
  ASSERT_THAT(ptrace(PTRACE_SEIZE, b_pid, 0, 0), SyscallSucceeds());
  int status;
  EXPECT_THAT(waitpid(b_pid, &status, WNOHANG), SyscallSucceedsWithValue(0));
  ASSERT_THAT(ptrace(PTRACE_INTERRUPT, b_pid, 0, 0), SyscallSucceeds());
  ASSERT_THAT(waitpid(b_pid, &status, 0), SyscallSucceedsWithValue(b_pid));
  ASSERT_EQ(status >> 8, SIGTRAP | (kPtraceEventStop << 8))
      << "status = " << status;
  ...
  // Freeze at the root. a_pid can and will actually park; b_pid, still
  // ptrace-stopped, provably cannot until continued -- so B, and therefore
  // A and the root, must all stay unsettled the whole time.
  ...
  // a_pid has parked; b_pid is still ptrace-stopped and provably cannot
  // have parked on its own. If A's transition wrongly propagated past A
  // without checking A's own unsettled child (B), the root would already
  // claim "frozen 1" here.
  ...
  // Let b_pid settle too, while still frozen: PTRACE_CONT lets it resume
  // and actually enter frozenStop. This must still propagate normally --
  // the new guard only short-circuits while a child is unsettled, not once
  // it settles -- so both A (nrUnsettledChildren 1->0) and the root
  // (1->0, from A's own transition) must actually reach "frozen 1" here.
  ASSERT_THAT(ptrace(PTRACE_CONT, b_pid, 0, 0), SyscallSucceeds());
  WaitForFrozen(a, 1);
  WaitForFrozen(c(), 1);

  // Thaw everything, then ptrace-stop b_pid again and re-freeze. If the
  // first settle (a_pid parking behind an unsettled B) already corrupted
  // the root's unsettled-child counter, and b_pid's own settle then drove
  // it negative, the counter can no longer register any child's unsettled
  // state at all -- the root would immediately claim "frozen 1" here even
  // though b_pid, freshly re-stopped, has not parked again.
  ASSERT_NO_ERRNO(c().WriteControlFile("cgroup.freeze", "0"));
  WaitForFrozen(c(), 0);

  // b_pid is still seized (PTRACE_CONT doesn't detach); re-interrupt and
  // prove the stop the same way as before. PTRACE_INTERRUPT works
  // regardless of b_pid's current state -- "[does] not require that it is
  // ptrace-stopped" (pkg/sentry/kernel/ptrace.go), matching Linux's
  // kernel/ptrace.c: "[a]t least one trap is guaranteed to happen after
  // this request."
  ASSERT_THAT(ptrace(PTRACE_INTERRUPT, b_pid, 0, 0), SyscallSucceeds());
  ASSERT_THAT(waitpid(b_pid, &status, 0), SyscallSucceedsWithValue(b_pid));
  ASSERT_EQ(status >> 8, SIGTRAP | (kPtraceEventStop << 8))
