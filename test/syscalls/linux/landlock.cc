...
// headers; these layouts and values are part of the stable kernel ABI. Each
// symbol is annotated with the ABI version that introduced it; tests guard
// their use with SKIP_IF(LandlockAbiVersion() < N) so nothing here runs on a
...
TEST(LandlockTest, ConnectTcpAllowedPortAllowed) {
  ...
  int status = ASSERT_NO_ERRNO_AND_VALUE(InForkedProcess([] {
    ...
    // Nothing is listening; ECONNREFUSED means Landlock let the connect
    // through.
    struct sockaddr_in addr = LoopbackAddr(kAllowedPort);
    int rc =
        connect(s, reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr));
