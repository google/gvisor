...
// Exit codes of forked test bodies. kSetup means the test's own setup failed,
// including a landlock_create_ruleset(2), landlock_add_rule(2) or
// landlock_restrict_self(2) that the helpers below expected to succeed, and
// fails the test. kUnsupported means the environment lacks something the test
// needs, such as a filesystem type or a syscall, and is the only child exit a
// test should turn into a skip.
enum ChildResult {
  kAllowed = 0,
  kDenied = 100,
  kSetup = 101,
...
// TryAttach attaches to tracee, detaches again if that succeeded, then
// reaps it.
