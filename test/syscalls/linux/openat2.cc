...
TEST_F(Openat2Test, NoXDevAbsoluteSymlinkSameMount) {
  ...
  // which here is on the same mount as the dirfd, so no mountpoint boundary is
  ...
  EXPECT_NO_ERRNO(OpenAt2(dirfd_.get(), name, O_RDONLY));
  EXPECT_NO_ERRNO(OpenAt2(dirfd_.get(), name, O_RDONLY, 0, RESOLVE_NO_XDEV));
  EXPECT_NO_ERRNO(OpenAt2(dirfd_.get(),
                          JoinPath(Basename(sub.path()), "..", name), O_RDONLY,
                          0, RESOLVE_NO_XDEV));
}
