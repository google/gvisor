...
// Launch a bunch of ptracers vying for the ptraceExecveMutex to stress the lock
// implementation.
TEST(ExecTest, ExecvePtraceLockStress) {
  SKIP_IF(ASSERT_NO_ERRNO_AND_VALUE(YamaPtraceScope()) < 1);
  const DisableSave ds;  // Too many threads
  const int kThreadCount = 20;
  const int kAttachAttempts = 100;
  ...
  if (tracee_pid == 0) {
    TEST_PCHECK(close(sockets[1]) == 0);
    ASSERT_THAT(prctl(PR_SET_PTRACER, PR_SET_PTRACER_ANY), SyscallSucceeds());
    TEST_PCHECK(write(sockets[0], "x", 1) == 1);  // tracers can trace

    char done;
    TEST_PCHECK(read(sockets[0], &done, 1) == 1);  // block on parent's go ahead

    const ExecveArray argv = {"/bin/true"};
  ...
  char done;
  TEST_PCHECK(read(sockets[1], &done, 1) == 1);  // wait till tracee is ready
  ...
  // The tracee was able to get the excevePtraceMutex lock, proving that the
