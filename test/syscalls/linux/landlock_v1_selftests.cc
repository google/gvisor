...
//
// get_ruleset_from_fd() takes the ruleset fd with fdget(), which leaves O_PATH
// descriptors out, so one is EBADF like a closed fd rather than the EBADFD of
// an open fd that is not a ruleset. That holds for landlock_add_rule(2) and
// landlock_restrict_self(2) alike.
TEST(LandlockV1Test, OPathRulesetFdIsEbadf) {
  SKIP_IF(LandlockAbiVersion() < 1);
  const TempPath dir = ASSERT_NO_ERRNO_AND_VALUE(TempPath::CreateDir());
  int opath_fd = open(dir.path().c_str(), O_PATH | O_DIRECTORY | O_CLOEXEC);
  ASSERT_THAT(opath_fd, SyscallSucceeds());
  auto close_opath = Cleanup([opath_fd] { close(opath_fd); });
  int parent_fd = open(dir.path().c_str(), O_PATH | O_DIRECTORY | O_CLOEXEC);
  ASSERT_THAT(parent_fd, SyscallSucceeds());
  auto close_parent = Cleanup([parent_fd] { close(parent_fd); });
  landlock_path_beneath_attr path_beneath = {};
  path_beneath.allowed_access = LANDLOCK_ACCESS_FS_READ_FILE;
  path_beneath.parent_fd = parent_fd;
  EXPECT_THAT(
      landlock_add_rule(opath_fd, LANDLOCK_RULE_PATH_BENEATH, &path_beneath, 0),
      SyscallFailsWithErrno(EBADF));
  // The same directory opened without O_PATH is the open-but-not-a-ruleset
  // case.
  int plain_fd = open(dir.path().c_str(), O_RDONLY | O_DIRECTORY | O_CLOEXEC);
  ASSERT_THAT(plain_fd, SyscallSucceeds());
  auto close_plain = Cleanup([plain_fd] { close(plain_fd); });
  EXPECT_THAT(
      landlock_add_rule(plain_fd, LANDLOCK_RULE_PATH_BENEATH, &path_beneath, 0),
      SyscallFailsWithErrno(EBADFD));

  // landlock_restrict_self(2) checks no_new_privs before the fd, so the fd
  // check is reached in a child that has set it.
  constexpr int kBadf = 130;
  constexpr int kBadfd = 131;
  ...
  EXPECT_TRUE(WIFEXITED(status) && WEXITSTATUS(status) == kBadf)
      << "exit status " << status;
}
...
TEST(LandlockV1Test, RenameExchangeRequiresExchangeRights) {
  ...
  // The denial comes from security_path_rename(), which runs before the
  // filesystem's rename operation gets to say whether it implements
  // RENAME_EXCHANGE at all, so it holds even where the exchange itself would
...
TEST(LandlockV1Test, RenameExchangeAcrossDirectoriesRefusedWithExdev) {
  ...
  // Both answers come from security_path_rename(), which runs before the
  // filesystem's rename operation gets to say whether it implements
  // RENAME_EXCHANGE at all, so they hold on a filesystem that would refuse
  // the exchange with EINVAL too.
  int granted = ASSERT_NO_ERRNO_AND_VALUE(InForkedProcess([&] {
...
TEST(LandlockV1Test, OverlayCopyUpAllowedByRuleOnMergedDir) {
  ...
  SKIP_IF(WIFEXITED(status) && WEXITSTATUS(status) == kUnsupported);
  EXPECT_TRUE(WIFEXITED(status) && WEXITSTATUS(status) == kAllowed)
      << "exit status " << status;
}
...
// Linux selftest: fs_test.c layout2_overlay.same_content_different_file
//
// A layer file and the merged file built from it are different files: on Linux
// they are distinct inodes, so a rule on the merged directory grants nothing
// on the lower or upper directory it is assembled from, and a rule on a layer
// directory grants nothing on the merged one, even though both paths lead to
// the same content. Each leg mounts its own overlay and checks one direction;
// the first three are controls showing the rules do grant what they name.
TEST(LandlockV1Test, OverlayLayerFilesAndMergedFilesAreDifferent) {
  SKIP_IF(LandlockAbiVersion() < 1);
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveCapability(CAP_SYS_ADMIN)));
  ...
  const std::string data =
      "lowerdir=" + lower + ",upperdir=" + upper + ",workdir=" + work;
  // fl is created in the lower layer; fu is created through the merged
  // directory and so lives in the upper layer.
  const std::string lower_fl = JoinPath(lower, "fl");
  const std::string merged_fl = JoinPath(merged, "fl");
  const std::string upper_fu = JoinPath(upper, "fu");
  const std::string merged_fu = JoinPath(merged, "fu");

  // Mounts a fresh overlay, grants READ_FILE beneath rule_dir and nowhere
  // else, and reports how opening target for reading fares.
  auto check = [&](const std::string& rule_dir, const std::string& target) {
    return InForkedProcess([&] {
      ...
      TEST_PCHECK(mkdir(work.c_str(), 0777) == 0);
      TEST_PCHECK(mkdir(merged.c_str(), 0777) == 0);
      int fd = open(lower_fl.c_str(), O_WRONLY | O_CREAT | O_EXCL, 0644);
      TEST_PCHECK(fd >= 0);
      TEST_PCHECK(close(fd) == 0);
      if (mount("overlay", merged.c_str(), "overlay", 0, data.c_str()) != 0) {
        _exit(kUnsupported);
      }
      fd = open(merged_fu.c_str(), O_WRONLY | O_CREAT | O_EXCL, 0644);
      TEST_PCHECK(fd >= 0);
      TEST_PCHECK(close(fd) == 0);
      ApplyFsPolicy(LANDLOCK_ACCESS_FS_READ_FILE, rule_dir,
                    LANDLOCK_ACCESS_FS_READ_FILE);
      _exit(TryReadOpen(target));
    });
  };
  struct Leg {
    const std::string& rule_dir;
    const std::string& target;
    ChildResult want;
    const char* what;
  };
  const Leg legs[] = {
      {merged, merged_fl, kAllowed,
       "merged rule reaches the merged lower-backed file"},
      {merged, merged_fu, kAllowed,
       "merged rule reaches the merged upper-backed file"},
      {lower, lower_fl, kAllowed, "lower rule reaches the lower file"},
      {merged, lower_fl, kDenied, "merged rule must not reach the lower file"},
      {merged, upper_fu, kDenied, "merged rule must not reach the upper file"},
      {lower, merged_fl, kDenied, "lower rule must not reach the merged file"},
      {upper, merged_fu, kDenied, "upper rule must not reach the merged file"},
  };
  for (const Leg& leg : legs) {
    int status = ASSERT_NO_ERRNO_AND_VALUE(check(leg.rule_dir, leg.target));
    SKIP_IF(WIFEXITED(status) && WEXITSTATUS(status) == kUnsupported);
    EXPECT_TRUE(WIFEXITED(status) && WEXITSTATUS(status) == leg.want)
        << leg.what << ": exit status " << status;
  }
}
...
//     Partially ported as OverlayLayerFilesAndMergedFilesAreDifferent,
//     OverlayCopyUpAllowedByRuleOnMergedDir and
//     OverlayRuleOnLowerFileSurvivesCopyUp. Upstream walks a five-layer matrix
//     across lower/upper/merge for every file in the overlay; the port covers
//     layer/merged distinctness in both directions, copy-up, and
//     rule-follows-inode, which are the cases where gVisor's overlay differs
//     structurally from Linux's. The upstream test itself runs against gVisor
//     with one change, absolute lowerdir/upperdir/workdir mount options, and
//     passes.
//
...
//     layout1_bind.reparent_cross_mount,
//     path_disconnected_rename, path_disconnected_link, and the whole
//     layout4_disconnected_leafs and layout5_disconnected_branch fixtures.
//     Under v1 there is no REFER right at all, so every reparenting rename or
//     link is unconditionally EXDEV. That v1-visible behaviour is covered by
