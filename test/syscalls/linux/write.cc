...
TEST_F(WriteTest, WriteNullBuffer) {
  ...
  EXPECT_THAT(write(fd.get(), nullptr, 1), SyscallFailsWithErrno(EFAULT));
}
...
TEST_F(WriteTest, PwriteNullBuffer) {
  ...
  EXPECT_THAT(pwrite(fd.get(), nullptr, 1, 0), SyscallFailsWithErrno(EFAULT));
}
...
TEST_F(WriteTest, WriteClosedFd) {
  TempPath tmpfile = ASSERT_NO_ERRNO_AND_VALUE(TempPath::CreateFile());
  int fd = open(tmpfile.path().c_str(), O_WRONLY);
  ASSERT_GE(fd, 0);
  ASSERT_THAT(close(fd), SyscallSucceeds());

  char buf[16];
  EXPECT_THAT(write(fd, buf, sizeof(buf)), SyscallFailsWithErrno(EBADF));
}
...
TEST_F(WriteTest, PwriteClosedFd) {
  ...
  char buf[16];
  EXPECT_THAT(pwrite(fd, buf, sizeof(buf), 0), SyscallFailsWithErrno(EBADF));
}
...
// Test that write to a directory fails with EBADF.
TEST_F(WriteTest, WriteDirectory) {
...
// Test that offset is correctly incremented with multiple writes.
TEST_F(WriteTest, MultipleWritesIncrementOffset) {
  TempPath tmpfile = ASSERT_NO_ERRNO_AND_VALUE(TempPath::CreateFile());
  FileDescriptor fd =
      ASSERT_NO_ERRNO_AND_VALUE(Open(tmpfile.path().c_str(), O_WRONLY));
  ...
  EXPECT_THAT(write(fd.get(), data1.data(), data1.size()),
              SyscallSucceedsWithValue(data1.size()));
  EXPECT_THAT(lseek(fd.get(), 0, SEEK_CUR),
              SyscallSucceedsWithValue(data1.size()));

  EXPECT_THAT(write(fd.get(), data2.data(), data2.size()),
              SyscallSucceedsWithValue(data2.size()));
  EXPECT_THAT(lseek(fd.get(), 0, SEEK_CUR),
              SyscallSucceedsWithValue(data1.size() + data2.size()));

  EXPECT_THAT(write(fd.get(), data3.data(), data3.size()),
              SyscallSucceedsWithValue(data3.size()));
  EXPECT_THAT(
      lseek(fd.get(), 0, SEEK_CUR),
      SyscallSucceedsWithValue(data1.size() + data2.size() + data3.size()));
}

// Test that pwrite does not change offset.
TEST_F(WriteTest, PwriteDoesNotChangeOffset) {
  TempPath tmpfile = ASSERT_NO_ERRNO_AND_VALUE(TempPath::CreateFile());
  FileDescriptor fd =
      ASSERT_NO_ERRNO_AND_VALUE(Open(tmpfile.path().c_str(), O_WRONLY));

  const std::string data = "pwrite data";

  // Write some initial data.
  ASSERT_THAT(write(fd.get(), data.data(), data.size()),
              SyscallSucceedsWithValue(data.size()));
  const off_t offset_after_write = data.size();
  EXPECT_THAT(lseek(fd.get(), 0, SEEK_CUR),
              SyscallSucceedsWithValue(offset_after_write));

  // pwrite should not change the offset.
  EXPECT_THAT(pwrite(fd.get(), data.data(), data.size(), 100),
              SyscallSucceedsWithValue(data.size()));
  EXPECT_THAT(lseek(fd.get(), 0, SEEK_CUR),
              SyscallSucceedsWithValue(offset_after_write));
...
TEST_F(WriteTest, WriteExtendsPastEOF) {
  ...
  EXPECT_EQ(st.st_size, static_cast<off_t>(initial_data.size() + additional.size()));
}
