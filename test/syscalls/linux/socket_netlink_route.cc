...
TEST_P(NetlinkSetLinkTest, ChangeMTU) {
  ...
  // Change the MTU.
  MtuRequest req =
      GetMtuRequest(loopback_link, GetParam(), loopback_link.mtu + 10);
  EXPECT_NO_ERRNO(NetlinkRequestAckOrError(fd, kSeq, &req, sizeof(req)));
...
TEST(NetlinkRouteTest, LinkMulticastGroupBasic) {
  ...
  for (const auto& tc : test_cases) {
    ...
    ASSERT_EQ(poll_ret, 1) << "Did not get link event " << tc.event_name
                           << " on the " << tc.name << " netlink socket.";
    ...
    EXPECT_TRUE(got_msg) << "Did not get link event " << tc.event_name
                         << " on the " << tc.name << " netlink socket.";
  }
}
