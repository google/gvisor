...
TEST_F(PtyTest, TCSETSWPreservesReplicaInput) {
  ...
  ASSERT_THAT(ioctl(replica_.get(), TCSETSW, &t), SyscallSucceeds());

  char buf[sizeof(kInput)] = {};
  EXPECT_THAT(ReadFd(replica_.get(), buf, sizeof(kInput) - 1),
              SyscallSucceedsWithValue(sizeof(kInput) - 1));
  EXPECT_EQ(memcmp(buf, kInput, sizeof(kInput) - 1), 0);
}
