...
TEST(ProcSysKernelRandomUuid, Exists) {
  EXPECT_THAT(open("/proc/sys/kernel/random/uuid", O_RDONLY), SyscallSucceeds());
}

TEST(ProcSysKernelRandomUuid, DifferentEveryTime) {
  auto uuid1 = ASSERT_NO_ERRNO_AND_VALUE(GetContents("/proc/sys/kernel/random/uuid"));
  auto uuid2 = ASSERT_NO_ERRNO_AND_VALUE(GetContents("/proc/sys/kernel/random/uuid"));
