...
void RunSubthreadTracedHelper() {
  int rfd = absl::GetFlag(FLAGS_pidfd_pipe_fd);
  int tid_wfd = absl::GetFlag(FLAGS_pidfd_tid_pipe_fd);

  ScopedThread t([rfd, tid_wfd] {
    pid_t tid = gettid();
    TEST_PCHECK(write(tid_wfd, &tid, sizeof(tid)) == sizeof(tid));
    char buf;
    (void)read(rfd, &buf, 1);  // Wait to exit.
  });
}
...
TEST(PidfdTest, SendSignalViaPidfdOpenPidfdToDeadChildFails) {
  pid_t child = fork();
  if (child == 0) {
    _exit(0);
  }
...
TEST(PidfdTest, SendSignalToZombieSucceeds) {
  pid_t child = -1;
  auto pidfd =
      ASSERT_NO_ERRNO_AND_VALUE(Clone3Pidfd(child, []() { _exit(0); }));
  ScopedChildReaper cleanup(child);
  ...
  // Target process is a zombie. pidfd_send_signal should fail with ESRCH.
  EXPECT_THAT(PidfdSendSignal(pidfd.get(), 0, nullptr, 0), SyscallSucceeds());
}
...
TEST(PidfdTest, SendSignalRejection) {
  pid_t child = -1;
  auto pidfd = ASSERT_NO_ERRNO_AND_VALUE(Clone3Pidfd(child, []() {
    pause();
    _exit(0);
  ...
  EXPECT_THAT(PidfdSendSignal(pidfd.get(), SIGUSR2, &info, 0),
  ...
  EXPECT_THAT(PidfdSendSignal(pidfd.get(), SIGUSR1, &info, 0),
  ...
  // But can spoof self.
  auto self_pidfd = ASSERT_NO_ERRNO_AND_VALUE(PidfdOpen(getpid(), 0));
  info = {};
  info.si_signo = 0;
  info.si_code = SI_TKILL;
  EXPECT_THAT(PidfdSendSignal(self_pidfd.get(), 0, &info, 0),
              SyscallSucceeds());

  // Valid signal delivery.
  EXPECT_THAT(PidfdSendSignal(pidfd.get(), SIGUSR1, nullptr, 0),
              SyscallSucceeds());
  info = {};
...
TEST(PidfdTest, Poll) {
  ...
  // Create a second child that only exits when told to do so.
  int pfd[2];
  ASSERT_THAT(pipe(pfd), SyscallSucceeds());
  FileDescriptor read_pipe(pfd[0]);
  FileDescriptor write_pipe(pfd[1]);
  ...
  pid_t child2 = -1;
  int rfd = read_pipe.get();
  auto pidfd2 = ASSERT_NO_ERRNO_AND_VALUE(Clone3Pidfd(child2, [rfd]() {
    char buf;
    read(rfd, &buf, 1);
    _exit(0);
  }));
  ScopedChildReaper cleanup2(child2);
  read_pipe.reset();
  ...
  ASSERT_THAT(poll(&fds2, 1, 100), SyscallSucceedsWithValue(0));
  // Instruct child2 to exit.
  ASSERT_THAT(write(write_pipe.get(), "a", 1), SyscallSucceedsWithValue(1));
  write_pipe.reset();
  // Now pollable.
  ASSERT_THAT(poll(&fds2, 1, 5000), SyscallSucceedsWithValue(1));
  EXPECT_TRUE(fds2.revents & POLLIN);
}
...
TEST(PidfdTest, SetnsZombieChild) {
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveCapability(CAP_SYS_ADMIN)));
  pid_t child = -1;
  auto pidfd =
      ASSERT_NO_ERRNO_AND_VALUE(Clone3Pidfd(child, []() { _exit(0); }));
...
TEST(PidfdTest, PidfdThreadTracedSubthreadNotification) {
  if (!IsRunningOnGvisor()) {
    // Test only on kernels bearing this patch:
    // https://github.com/torvalds/linux/commit/0fb482728ba1
    KernelVersion version = ASSERT_NO_ERRNO_AND_VALUE(GetKernelVersion());
  ...
  auto cleanup = ASSERT_NO_ERRNO_AND_VALUE(
      ForkAndExec("/proc/self/exe",
                  {"/proc/self/exe", "--pidfd_helper=RunSubthreadTracedHelper",
                   absl::StrCat("--pidfd_pipe_fd=", read_pipe.get()),
                   absl::StrCat("--pidfd_tid_pipe_fd=", tid_write_pipe.get())},
                  {}, &child, &exec_errno));
  ASSERT_EQ(exec_errno, 0);
  ...
  // Wait for the TID of the subthread and open a pidfd.
  pid_t subthread_tid;
  ASSERT_THAT(read(tid_read_pipe.get(), &subthread_tid, sizeof(subthread_tid)),
              SyscallSucceedsWithValue(sizeof(subthread_tid)));
  auto subthread_pidfd =
  ...
  // Start tracing the subthread.
  ASSERT_THAT(ptrace(PTRACE_ATTACH, subthread_tid, nullptr, nullptr),
              SyscallSucceeds());
  int status;
  ASSERT_THAT(waitpid(subthread_tid, &status, 0),
              SyscallSucceedsWithValue(subthread_tid));
  ASSERT_TRUE(WIFSTOPPED(status) && WSTOPSIG(status) == SIGSTOP);
  ASSERT_THAT(ptrace(PTRACE_CONT, subthread_tid, nullptr, nullptr),
              SyscallSucceeds());

  // Tell the thread to exit.
  ASSERT_THAT(write(write_pipe.get(), "a", 1), SyscallSucceedsWithValue(1));

  // The pidfd should notify even though the tracer has not yet acked the exit.
  struct pollfd pfd = {
      .fd = subthread_pidfd.get(),
      .events = POLLIN,
  };
  EXPECT_THAT(poll(&pfd, 1, 1000), SyscallSucceedsWithValue(1));
  EXPECT_TRUE(pfd.revents & POLLIN);
...
// PidfdThreadTracedLeaderNotification verifies that a PIDFD_THREAD for a
// traced thread group leader is delayed until the entire thread group exits,
// but once triggered, the notification occurs before the tracer reaps the
// leader.
TEST(PidfdTest, PidfdThreadTracedLeaderNotification) {
  if (!IsRunningOnGvisor()) {
    // Test behavior on kernels with PIDFD_THREAD.
    KernelVersion version = ASSERT_NO_ERRNO_AND_VALUE(GetKernelVersion());
    if (version.major < 6 || (version.major == 6 && version.minor < 15)) {
      GTEST_SKIP() << "PIDFD_THREAD behavior requires kernel 6.15+";
    }
  ...
  // Start tracing the leader.
  ASSERT_THAT(ptrace(PTRACE_ATTACH, child, nullptr, nullptr),
              SyscallSucceeds());
  int status;
  ASSERT_THAT(waitpid(child, &status, 0), SyscallSucceedsWithValue(child));
  ASSERT_TRUE(WIFSTOPPED(status) && WSTOPSIG(status) == SIGSTOP);
  ASSERT_THAT(ptrace(PTRACE_CONT, child, nullptr, nullptr), SyscallSucceeds());

  // The leader should exit quickly as it calls syscall(SYS_exit).
  // Poll should timeout as long as the subthread is alive, confirming the
  // delay is preserved even for traced leaders.
  struct pollfd pfd = {
      .fd = leader_pidfd.get(),
      .events = POLLIN,
  };
  ...
  // Now let the subthread exit.
  ASSERT_THAT(write(write_pipe.get(), "a", 1), SyscallSucceedsWithValue(1));
  write_pipe.reset();

  // Now the leader's pidfd should notify, even though the tracer has not yet
  // acked the exit.
  EXPECT_THAT(poll(&pfd, 1, 5000), SyscallSucceedsWithValue(1));
  EXPECT_TRUE(pfd.revents & POLLIN);
...
int main(int argc, char** argv) {
  ...
  if (!helper.empty()) {
    ...
    } else if (helper == "RunSubthreadTracedHelper") {
      gvisor::testing::RunSubthreadTracedHelper();
