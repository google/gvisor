...
TEST(RawSocketTest, V6Only) {
...

  {
    ...
    ASSERT_THAT(getsockopt(fd.get(), SOL_IPV6, IPV6_V6ONLY, &v, &len),
                SyscallSucceeds());
    ASSERT_THAT(len, sizeof(len));

    EXPECT_EQ(v, 1);
    v = 0;
    if (IsRunningOnGvisor() && !IsRunningWithHostinet()) {
      // TODO(https://gvisor.dev/issue/6966): This should not succeed.
      ASSERT_THAT(setsockopt(fd.get(), SOL_IPV6, IPV6_V6ONLY, &v, sizeof(v)),
                  SyscallSucceeds());
    } else {
      ASSERT_THAT(setsockopt(fd.get(), SOL_IPV6, IPV6_V6ONLY, &v, sizeof(v)),
  ...
  {
    TestAddress addr = V4MappedLoopback();
    ASSERT_THAT(
        bind(fd.get(), reinterpret_cast<sockaddr*>(&addr.addr), addr.addr_len),
