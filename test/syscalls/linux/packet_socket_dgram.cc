...
void ReceiveMessage(int sock, int ifindex, int expected_pkttype) {
  ...
  EXPECT_EQ(src.sll_pkttype, expected_pkttype);
...
TEST_P(CookedPacketTest, Receive) {
  ...
  if (GetParam() == ETH_P_ALL) {
    // Only ETH_P_ALL sockets receive outbound packets on linux.
    ReceiveMessage(socket_, loopback_index, PACKET_OUTGOING);
  }
  ReceiveMessage(socket_, loopback_index, PACKET_HOST);
}
...
TEST_P(CookedPacketTest, BindReceive) {
  ...
  if (GetParam() == ETH_P_ALL) {
    // Only ETH_P_ALL sockets receive outbound packets on linux.
    ReceiveMessage(socket_, bind_addr.sll_ifindex, PACKET_OUTGOING);
  }
  ReceiveMessage(socket_, bind_addr.sll_ifindex, PACKET_HOST);
}
...
TEST_P(CookedPacketTest, ReceiveOutbound) {
  // Seems like gvisor platform will add default routes? Do we get non-loopback
  // interfaces?
  //
  // This fails when running on linux native when we perform the sendto
  // operation.
