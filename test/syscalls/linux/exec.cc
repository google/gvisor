...
constexpr char kEtxtbsyWorkload[] = "test/syscalls/linux/exec_etxtbsy_workload";
...
TEST(ExecTest, ExecveArgvE2BIG) {
  int execve_errno = 0;
  ASSERT_NO_ERRNO_AND_VALUE(
      ForkAndExec("/bin/true", OversizedExecveArgv("/bin/true"), {},
                  /*child=*/nullptr, &execve_errno));
...
TEST(ExecTest, ExecveEnvvE2BIG) {
  ...
  int execve_errno = 0;
  ASSERT_NO_ERRNO_AND_VALUE(
      ForkAndExec("/bin/true", {"/bin/true"}, ExecveArray(envv),
                  /*child=*/nullptr, &execve_errno));
  EXPECT_EQ(execve_errno, E2BIG);
}
...
TEST(ExecTest, ExecveEnoentBeforeE2BIG) {
  ...
  int execve_errno = 0;
  ASSERT_NO_ERRNO_AND_VALUE(
      ForkAndExec(path, OversizedExecveArgv(path), {}, /*child=*/nullptr,
                  &execve_errno));
  EXPECT_EQ(execve_errno, ENOENT);
}
...
TEST(ExecTest, ExecveEnametoolongBeforeE2BIG) {
  ...
  int execve_errno = 0;
  ASSERT_NO_ERRNO_AND_VALUE(
      ForkAndExec(long_path, OversizedExecveArgv(long_path), {},
                  /*child=*/nullptr, &execve_errno));
