...
TEST(NetlinkNetfilterTest, ErrGetTableWithDumpFlag) {
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveCapability(CAP_NET_RAW)));
  ...
  std::vector<char> get_request_buffer =
      NetlinkRequestBuilder()
          .SetMessageType(NFT_MSG_GETTABLE)
          .SetFlags(NLM_F_REQUEST | NLM_F_DUMP)
          .SetFamily(NFPROTO_IPV4)
          .SetSequenceNumber(kSeq + 1)
          .AddAttribute(NFTA_TABLE_NAME, test_table_name,
                        strlen(test_table_name) + 1)
  ...
  ASSERT_THAT(NetlinkRequestAckOrError(fd, kSeq + 1, get_request_buffer.data(),
...
TEST(NetlinkNetfilterTest, ErrNewChainWithNonexistentTable) {
  ...
  std::vector<char> add_chain_request_buffer =
      NetlinkRequestBuilder()
          ...
          .SetFamily(NFPROTO_INET)
          .SetSequenceNumber(kSeq + 1)
          .AddAttribute(NFTA_CHAIN_TABLE, test_table_name,
                        strlen(test_table_name) + 1)
          .Build();
...
TEST(NetlinkNetfilterTest, ErrGetChainWithDumpFlagSet) {
  ...
  std::vector<char> get_chain_request_buffer =
      NetlinkRequestBuilder()
          .SetMessageType(NFT_MSG_GETCHAIN)
          .SetFlags(NLM_F_REQUEST | NLM_F_ACK | NLM_F_DUMP)
          .SetFamily(NFPROTO_INET)
          .SetSequenceNumber(kSeq + 2)
          .AddAttribute(NFTA_CHAIN_TABLE, test_table_name,
                        strlen(test_table_name) + 1)
...
TEST(NetlinkNetfilterTest, GetChain) {
  ...
  std::vector<char> add_chain_request_buffer =
      NetlinkRequestBuilder()
          ...
          .AddAttribute(NFTA_CHAIN_NAME, test_chain_name,
                        strlen(test_chain_name) + 1)
          .AddAttribute(NFTA_CHAIN_USERDATA, test_user_data,
                        expected_udata_size)
          .Build();
