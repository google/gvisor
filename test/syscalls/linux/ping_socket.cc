...
TEST_P(Fixture, Bind) {
  ...
  auto socket_fd = ASSERT_NO_ERRNO_AND_VALUE(socket_factory.Create());

  int want = test_case.want_gvisor.has_value() && IsRunningOnGvisor()
                 ? test_case.want_gvisor.value()
                 : test_case.want;
  if (want == 0) {
    ASSERT_THAT(bind(socket_fd->get(), AsSockAddr(&test_case.bind_to.addr),
