...
static RecvTosOption GetRecvTosOption(int domain) {
...
TEST_P(UDPSocketTest, RecvTosDefault) {
  auto socket = ASSERT_NO_ERRNO_AND_VALUE(NewSocket());
  RecvTosOption t = GetRecvTosOption(GetParam().domain);
  int get = -1;
  socklen_t get_len = sizeof(get);
  ASSERT_THAT(getsockopt(socket->get(), t.level, t.option, &get, &get_len),
              SyscallSucceedsWithValue(0));
  EXPECT_EQ(get_len, sizeof(get));
  EXPECT_EQ(get, kSockOptOff);
}
...
// expected.
TEST_P(UDPSocketTest, SetRecvTos) {
  auto socket = ASSERT_NO_ERRNO_AND_VALUE(NewSocket());
  RecvTosOption t = GetRecvTosOption(GetParam().domain);

  ASSERT_THAT(setsockopt(socket->get(), t.level, t.option, &kSockOptOff,
                         sizeof(kSockOptOff)),
              SyscallSucceeds());
  ...
  ASSERT_THAT(setsockopt(socket->get(), t.level, t.option, &kSockOptOn,
                         sizeof(kSockOptOn)),
              SyscallSucceeds());
...
TEST_P(UDPSocketTest, TOSRecvMismatch) {
  auto socket = ASSERT_NO_ERRNO_AND_VALUE(NewSocket());
...
TEST(UDPSocketTest, TClassRecvMismatch) {
  auto socket =
      ASSERT_NO_ERRNO_AND_VALUE(SimpleSocket(AF_INET, SOCK_DGRAM, 0).Create());
  int get = -1;
  socklen_t get_len = sizeof(get);

  ASSERT_THAT(
      getsockopt(socket->get(), IPPROTO_IPV6, IPV6_RECVTCLASS, &get, &get_len),
