...
constexpr const char kMacA[ETH_ALEN] = {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA};
constexpr const char kMacB[ETH_ALEN] = {0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB};
...
ping_pkt CreatePingPacket(const char srcmac[ETH_ALEN], const char* srcip,
                          const char dstmac[ETH_ALEN], const char* dstip) {
  ping_pkt pkt = {};
...
struct arp_pkt {
  ...
  unsigned char arp_spa[kIPLen];
  unsigned char arp_tha[ETH_ALEN];
  unsigned char arp_tpa[kIPLen];
} __attribute__((packed));

std::string CreateArpPacket(const char srcmac[ETH_ALEN], const char* srcip,
                            const char dstmac[ETH_ALEN], const char* dstip) {
  std::string buffer;
  buffer.resize(sizeof(arp_pkt));

  arp_pkt* pkt = reinterpret_cast<arp_pkt*>(buffer.data());
  {
...
TEST_F(TuntapTest, PingKernel) {
  ...
  while (1) {
    ...
    if (n < sizeof(pihdr)) {
      LOG(INFO) << "Ignored packet, protocol: " << r.pi.pi_protocol
                << " len: " << n;
      continue;
    }
