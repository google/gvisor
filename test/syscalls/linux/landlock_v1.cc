...
// Linux resolves move_mount's target path before its source path, so when
// both are bad the target's errno wins even for a landlocked caller.
TEST(LandlockV1Test, MoveMountResolvesTargetBeforeSource) {
  SKIP_IF(LandlockAbiVersion() < 1);
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveCapability(CAP_SYS_ADMIN)));
  MoveMount(-1, "", -1, "", 0);
