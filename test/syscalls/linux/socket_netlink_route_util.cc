...
PosixError NeighModify(int index, int family, const void* addr, int addrlen,
  ...
  if (lladdr) {
    ...
    req.hdr.nlmsg_len =
        NLMSG_ALIGN(NLMSG_ALIGN(req.hdr.nlmsg_len) + RTA_LENGTH(lladdrlen));
  }
...
PosixError LinkAdd(const std::string name, const std::string kind) {
  ...
  struct rtattr* rta_name = reinterpret_cast<struct rtattr*>(req.buf);
  ...
  struct rtattr* rta_info = reinterpret_cast<struct rtattr*>(req.buf + rta_len);

  rta_info->rta_type = IFLA_LINKINFO;

  {
    struct rtattr* rta_info_kind =
        reinterpret_cast<struct rtattr*>(RTA_DATA(rta_info));
    rta_info_kind->rta_type = IFLA_INFO_KIND;
    rta_info_kind->rta_len = RTA_LENGTH(kind.size());
    char* rta_info_kind_d = reinterpret_cast<char*>(RTA_DATA(rta_info_kind));
