...
TEST(MsgqueueTest, MsgOpGeneral) {
  ...
  std::map<long, msgbuf> typeToBuf = {{1, msgbuf{1, "Message 1."}},
                                      {2, msgbuf{2, "Message 2."}},
...
TEST(MsgqueueTest, MsgCtlIpcStat) {
  ...
  const time_t pre_send = time(nullptr);
  ...
  EXPECT_GE(ds.msg_stime, pre_send);
  EXPECT_EQ(ds.msg_rtime, 0);
  EXPECT_GE(ds.msg_ctime, start);
  ...
  absl::SleepFor(absl::Seconds(1));
  const time_t pre_receive = time(nullptr);
  ...
  EXPECT_GE(ds.msg_stime, pre_send);
  EXPECT_GE(ds.msg_rtime, pre_receive);
  EXPECT_GE(ds.msg_ctime, start);
