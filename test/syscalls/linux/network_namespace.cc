...
TEST(NetworkNamespaceTest, LoopbackAddressesAddedOnUp) {
  ...
  ScopedThread t([&] {
    ...
    // Bringing it up assigns the default loopback addresses, just like Linux,
    // so binding to them now succeeds.
    {
      FileDescriptor s4 =
          ASSERT_NO_ERRNO_AND_VALUE(Socket(AF_INET, SOCK_STREAM, 0));
      EXPECT_THAT(bind(s4.get(), AsSockAddr(&addr4), sizeof(addr4)),
...
// Like Linux, bringing an interface down flushes its IPv6 addresses and
// routes. Bringing it back up restores neither; only loopback's ::1 is
// recreated.
TEST(NetworkNamespaceTest, IPv6FlushedOnDown) {
  // TODO(b/267210840): Fix this tests for hostinet.
  SKIP_IF(IsRunningWithHostinet());
  ...
  ScopedThread t([&] {
    ...
    // A global IPv6 address (2001:db8::1) assigned to loopback.
    ...
    // An extra route (2001:db8:1::/64) via loopback. Like Linux, bringing the
    // interface down flushes every IPv6 route through it.
    struct in6_addr extra = global;
    extra.s6_addr[5] = 0x01;
    extra.s6_addr[15] = 0x00;
    ASSERT_NO_ERRNO(AddUnicastRoute(lo.index, AF_INET6, /*prefixlen=*/64,
                                    &extra, sizeof(extra)));

    // Bringing the interface down also removes ::1.
    ASSERT_NO_ERRNO(LinkChangeFlags(lo.index, 0, IFF_UP));
    {
      FileDescriptor s6 =
          ASSERT_NO_ERRNO_AND_VALUE(Socket(AF_INET6, SOCK_STREAM, 0));
    ...
    // Bringing loopback back up recreates ::1.
    {
      FileDescriptor s6 =
          ASSERT_NO_ERRNO_AND_VALUE(Socket(AF_INET6, SOCK_STREAM, 0));
      EXPECT_THAT(bind(s6.get(), AsSockAddr(&loaddr6), sizeof(loaddr6)),
    ...
    // The extra route stays flushed after the interface comes back up.
    EXPECT_THAT(DelUnicastRoute(lo.index, AF_INET6, /*prefixlen=*/64, &extra,
                                sizeof(extra)),
                PosixErrorIs(ESRCH, ::testing::_));
  });
}
