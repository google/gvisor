...
void CheckNetfilterTableAttributes(
    ...
    uint64_t expected_handle, uint32_t expected_flags, uint32_t expected_owner,
    uint8_t* expected_udata, size_t expected_udata_size) {
  ...
  const struct nfattr* handle_attr = FindNfAttr(hdr, genmsg, NFTA_TABLE_HANDLE);
  ...
  // Check for absence of the NFTA_TABLE_OWNER attribute.
  ...
  // Check for absence of the NFTA_TABLE_USERDATA attribute.
  const struct nfattr* user_data_attr =
      FindNfAttr(hdr, genmsg, NFTA_TABLE_USERDATA);
  if (user_data_attr != nullptr) {
