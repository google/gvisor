...
void AddDefaultTable(const AddDefaultTableOptions& options) {
  absl::string_view table_name = options.table_name;
  if (table_name.empty()) {
...
void AddDefaultBaseChain(const AddDefaultBaseChainOptions& options) {
  ...
  absl::string_view chain_name = options.chain_name;
  if (chain_name.empty()) {
...
NlReq& NlReq::StrAttr(uint16_t attr_type, absl::string_view payload) {
  return RawAttr(attr_type, payload.data(), payload.size() + 1);
}
