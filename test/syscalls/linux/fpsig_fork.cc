...
#define GET_FPREG( var, regname ) asm volatile("str "__stringify(regname)", %0" : "=m" (var) )
#define SET_FPREG( var, regname ) asm volatile("ldr "__stringify(regname)", %0" : "=m" (var) )
...
TEST(FPSigTest, Fork) {
  ...
  asm volatile(
      ...
      "mov x1, %2\n"
      "mov x2, %3\n"
      "svc #0\n"::"r"(__NR_tgkill), "r"(parent), "r"(parent_tid), "r"(SIGUSR1));
#endif
