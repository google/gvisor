...
// TODO: This is largely copied from other places (//runsc/sandbox/network.go).
...
// TODO: return a struct. Endpoint type should probably be public.
func newXDPEndpoint(ifaceName string, mac net.HardwareAddr) (stack.LinkEndpoint, error) {
	...
	rawLink, err := link.AttachRawLink(link.RawLinkOptions{
		...
		// TODO: Verify that we're in the expected mode (driver, since we're talking about VETH).
	})
	if err != nil {
	...
	// TODO: I don't think we need these, as the program is loaded. Delete
	// them here, and maybe in runsc too.
	// We need to keep the Program, SockMap, and link FDs open until they
	// can be passed to the sandbox process.
	if _, err := unix.Dup(objects.Program.FD()); err != nil {
		return nil, fmt.Errorf("failed to dup BPF program: %v", err)
	}
	if _, err := unix.Dup(objects.SockMap.FD()); err != nil {
		return nil, fmt.Errorf("failed to dup BPF map: %v", err)
	}
	if _, err := unix.Dup(rawLink.FD()); err != nil {
