...
PosixErrorOr<std::string> MountVerity(std::string tmpfs_dir,
  ...
  if (!symlink.empty()) {
    ASSIGN_OR_RETURN_ERRNO(auto symlink_fd,
                           Open(JoinPath(verity_dir.path(), symlink),
                                O_RDONLY | O_NOFOLLOW, 0777));
    RETURN_ERROR_IF_SYSCALL_FAIL(ioctl(symlink_fd.get(), FS_IOC_ENABLE_VERITY));
  }
