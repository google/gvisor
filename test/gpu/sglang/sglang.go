...
func New(ctx context.Context, server Server, logger testutil.Logger) (*SGLang, error) {
	...
	// Not sure if this is necessary. We might remove it after more testing.
	...
	if !strings.Contains(logs, "Capture cuda graph begin") {
		return nil, fmt.Errorf("cannot determine whether sglang is using GPU from logs:\n%s", logs)
	}
...
func (r *Response) String() string {
	...
	for i, token := range r.data {
		...
		if token.MetaInfo.E2ELatency > 0 {
			if i != len(r.data)-1 {
