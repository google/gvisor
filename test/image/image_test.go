...
func testDocker(t *testing.T, overlay, hostNetwork, startPrivilegedContainer bool) {
	...
	var cmd []string
	switch {
	case !hostNetwork && !startPrivilegedContainer:
		cmd = []string{"docker", "run", "--rm", "alpine", "sh", "-c", "apk add curl && curl -h"}
	case !hostNetwork && startPrivilegedContainer:
		cmd = []string{"docker", "run", "--privileged", "--rm", "alpine", "sh", "-c", "apk add curl && curl -h"}
	case hostNetwork && !startPrivilegedContainer:
		cmd = []string{"docker", "run", "--network", "host", "--rm", "alpine", "sh", "-c", "apk add curl && curl -h"}
	case hostNetwork && !startPrivilegedContainer:
		cmd = []string{"docker", "run", "--privileged", "--network", "host", "--rm", "alpine", "sh", "-c", "apk add curl && curl -h"}
	}
