		...
		for _, hostNetwork := range hostNetworkOpts {
			for _, privileged := range privilegedOpts {
				...
				t.Run(name, func(t *testing.T) {
					ok, err := waitForDockerDaemon(ctx, d)
					if err != nil {
						t.Fatalf("docker exec failed: %v", err)
					}
					if !ok {
						t.Fatalf("docker daemon not ready!")
					}
...
func waitForDockerDaemon(ctx context.Context, d *dockerutil.Container) (bool, error) {
	for range 10 {
		var err error
		output, err := dockerInGvisorExecOutput(ctx, d, []string{"docker", "info"})
		if err != nil {
			return false, fmt.Errorf("docker exec failed: %v", err)
		}
