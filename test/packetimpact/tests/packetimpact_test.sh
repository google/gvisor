...
declare -r LONGOPTS="dut_platform:,stub_binary:,test_binary:,runtime:,tshark"
...
while true; do
  case "$1" in
    --dut_platform)
      ...
      ;;
    --stub_binary)
      declare -r STUB_BINARY="$2"
      shift 2
      ;;
    --test_binary)
      declare -r TEST_BINARY="$2"
      shift 2
      ;;
    --runtime)
...
if [[ ! -f "${STUB_BINARY-}" ]]; then
  echo "FAIL: Bad or missing --stub_binary: ${STUB-}"
  exit 2
fi
if [[ ! -f "${TEST_BINARY-}" ]]; then
  echo "FAIL: Bad or missing --test_binary: ${TEST_BINARY-}"
  exit 2
fi
...
# TODO(eyalsoha): Rebuild gcr.io/gvisor-presubmit/packetdrill and use it.
...
# Start the stub in the DUT.
declare -r DOCKER_STUB_BINARY="/$(basename ${STUB_BINARY})"
docker cp -L ${STUB_BINARY} "${DUT}:${DOCKER_STUB_BINARY}"

docker exec -t "${DUT}" \
  /bin/bash -c "${DOCKER_STUB_BINARY} \
...
declare -r DOCKER_TEST_BINARY="/$(basename ${TEST_BINARY})"
docker cp -L "${TEST_BINARY}" "${TESTBENCH}:${DOCKER_TEST_BINARY}"

if [[ -z "${TSHARK-}" ]]; then
  # Run tcpdump in the test bench unbuffered, without dns resolution, just on the
  # interface with the test packets.
  docker exec -t "${TESTBENCH}" tcpdump -S -vvv -U -n -i "${TEST_DEVICE}" net "${TEST_NET_PREFIX}/24" &
else
  # Run tshark in the test bench unbuffered, without dns resolution, just on the
  # interface with the test packets.
  docker exec -t "${TESTBENCH}" tshark -V -l -n -i "${TEST_DEVICE}" host "${TEST_NET_PREFIX}${TESTBENCH_NET_SUFFIX}" &
fi
