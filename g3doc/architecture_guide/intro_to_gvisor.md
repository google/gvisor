...
very set of system calls the workload needs and no others. Creating a generic
...
While gVisor can use virtualization (specifically: KVM) as one of its
implementations, it can also work without virtualization while maintaining a
...
Like a user application, the gVisor Sentry *may* make system calls to the host
Linux kernel. It does so when it determines that servicing the sandboxed
workload's request requires information from the host machine and that the
sandboxed workload was initially configured to allow such access.

This means **the gVisor Sentry needs to re-implement Linux in Go**. The gVisor
Sentry contains a Go-based, from-scratch reimplementation of the POSIX system
call interface, CPU scheduling, memory management, filesystems, a network stack,
process management, signal handling, namespaces, etc. **gVisor never passes
through any request to the host**. Therefore, if a kernel feature isn't
reimplemented in gVisor, then the sandboxed workload cannot use it.

Let's walk through an example. Say a sandboxed process wishes to call
`getpid(2)`. gVisor intercepts this system call. gVisor keeps track of its own
PID table representing the processes in the sandbox. These are not real host
processes! Running top on the host will not show them. gVisor uses its own PID
table to find the PID of the sandboxed process, and returns that. From its
...
like `open(2)`, `exec(2)`, `connect(2)`, and their respective variants. It has
an isolated view of the host filesystem using `pivot_root(2)`, and runs as an
unprivileged user with no capabilities. **This does *not* mean that the
sandboxed workload can't use these system calls; it actually can!** But their
logic and implementation is entirely handled within the gVisor Sentry's kernel
logic, rather than delegating any of it to the host kernel.
...
companion process running in a slightly-more-privileged context. For example, if
the sandboxed workload needs to open a host file, the Sentry cannot do that (its
system call filter prevents this). Instead, it will ask the gVisor Gofer to open
the file. The Gofer will re-verify that the sandbox is allowed to access this
host file, open the host file, and pass the open file handle to the Sentry over
the communication link between the Sentry and the Gofer.
...
virtual machines, these components run in userspace and have the flexibility to
allocate and release host resources (CPU, memory) at runtime, providing better
efficiency and utilization without compromising on the security benefits of the
VM-like dual-kernel security architecture.
...
*   Exploits *within* the sandboxed workload itself, e.g. a gVisor sandbox
    ...
    sandbox is configured to have access to. Note however that gVisor has a
...
```shell
$ sudo runsc do echo Hello world Hello world
```
...
run as an unprivileged user with no capabilities from the perspective of the
host kernel. However, the sandbox setup process requires privileges,
