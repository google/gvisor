...
the performance penalty incurred by context switching through the I/O 
proxy mechansim that's otherwise used to expose host filesystems.
...
`FUSE_OPEN`, `FUSE_READ`, `FUSE_RELEASE`, and `FUSE_ACCESS`. Additional
opcodes like `FUSE_WRITE`, `FUSE_FLUSH`, `FUSE_STATFS`, and `FUSE_CREATE` can
be added as needed.
