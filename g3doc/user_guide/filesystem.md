...
By default, each gofer runs in a new, empty network namespace. To avoid
creating a network namespace for every gofer, set
`--gofer-network-namespace=host`; the gofer then uses runsc caller's current
network namespace. To use another existing network namespace, pass its absolute
path, for example `--gofer-network-namespace=/var/run/netns/gofer`.
