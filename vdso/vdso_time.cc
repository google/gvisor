...
//
// When aliasing, it serves CLOCK_MONOTONIC in the VDSO. Otherwise, if the raw
// params are not yet calibrated, it falls back to a syscall for the brief
// startup/restore window; otherwise it is computed in the VDSO from the raw
// params.
int ClockMonotonicRaw(struct timespec* ts) {
  ...
  do {
    seq = read_seqcount_begin(&params->seq_count);
    alias = params->monotonic_raw_alias;
