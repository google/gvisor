...
SHOT_REL = os.environ.get(
    "SHOT_REL", "/home/node/.openclaw/workspace/screenshots"
)
...
# An action and the arguments it was called with, so an identical repeat is
# recognisable.
_Signature = tuple[object, ...]

# What every tool hands back: what happened, then the screen it produced.
_Result = list[str | mcpserver.Image]

# The last few (signature, did-the-screen-change) pairs.
_recent: collections.deque[tuple[_Signature, bool]] = collections.deque(
    maxlen=STUCK_LIMIT
)
...
def traced(fn):
  ...
  Returns:
    The same function, wrapped so that every call logs its arguments first.
  """
...
def _xdo(*args: str) -> str:
  """Run an xdotool command against the desktop's display.

  Args:
    *args: The xdotool subcommand and its arguments.

  Returns:
    Whatever xdotool printed, stripped.

  Raises:
    RuntimeError: If xdotool exits non-zero.
  """
  env = dict(os.environ, DISPLAY=DISPLAY)
  # check=False: the return code is inspected below so the error message can
  # name the xdotool command that failed.
  proc = subprocess.run(
      ["xdotool", *args],
      env=env,
      capture_output=True,
  ...
  if proc.returncode != 0:
    raise RuntimeError(
        f"xdotool {' '.join(args)} failed: {proc.stderr.strip()}"
    )
...
def _px(x: int, y: int) -> tuple[int, int]:
  ...
  if not (0 <= x <= NORM and 0 <= y <= NORM):
    raise ValueError(
        ...
        "screenshot and read the position off the grid."
    )
  return (
...
def _raw() -> bytes:
  ...
  subprocess.run(
      ["scrot", "-o", "-p", "-F", path], env=env, check=True, timeout=30
  )
...
def _fingerprint(png: bytes) -> str:
  ...
  Args:
    png: The screen grab to fingerprint.

  Returns:
    A hex digest that is stable across incidental repaints.
  """
...
def _ocr(img: PILImage.Image) -> list[tuple[int, int, int, int, str]]:
  ...
  # Keyed by (block, paragraph, line); each value is the (word, left, top,
  # width, height) of every word on that line.
...
def _lines(png: bytes) -> list[tuple[int, int, str]]:
  ...
  Args:
    png: The screen grab to run OCR on.

  Returns:
    Every line of text on screen as (nx, ny, text), where (nx, ny) is the
    centre of the line in normalised coordinates, topmost line first.
  """
...
def _inventory(png: bytes, limit: int = 60) -> str:
  ...
  Args:
    png: The screen grab to read the text off.
    limit: The most lines to list before summarising the remainder.

  Returns:
    The on-screen text, one line at a time, each with the normalised
    coordinates to click it.
  """
  items = _lines(png)
  if not items:
...
def _encodings(
  ...
  Args:
    img: The screen grab to encode.

  Yields:
    A PIL format name and the keyword arguments to save with, including the
    image to save under the "img" key.
  """
...
def _for_delivery(png: bytes) -> tuple[bytes, str]:
  ...
  Args:
    png: The raw screen grab to re-encode.

  Returns:
    The encoded image bytes and the format they are in, lowercased.
  """
...
def _save(png: bytes, name: str = "") -> str | None:
  ...
  Args:
    png: The raw screen grab to write.
    name: An optional base name to use for the file.

  Returns:
    The path to the saved file as the agent sees it, or None if the directory
    is not writable -- the bridge still works without the bind mount, it just
    cannot hand anything to a human.
  """
  ...
  fname = (
      f"{safe}-{stamp}-{os.urandom(3).hex()}.{'jpg' if fmt == 'jpeg' else fmt}"
  )
  try:
...
def _as_image(png: bytes) -> mcpserver.Image:
  """The screen, gridded if configured, encoded small enough to render."""
  data, fmt = _for_delivery(_grid(png) if GRID else png)
  return mcpserver.Image(data=data, format=fmt)


def _acted(summary: str, before: str, signature: _Signature) -> _Result:
  ...
  Args:
    summary: A human-readable description of the action that was just taken.
    before: The fingerprint of the screen from before the action.
    signature: The action and its arguments, used to notice the same call being
      repeated with no effect.

  Returns:
    The summary, whether the screen changed and the on-screen text, followed
    by the resulting screenshot.

  Raises:
    ValueError: If the same action has been repeated STUCK_LIMIT times in a
      row without changing the screen.
  """
  time.sleep(SETTLE)
  png = _raw()
  ...
  if stuck:
    ...
    raise ValueError(
        ...
        "identical calls in a row, so this is doing nothing. Stop "
        "repeating it. Look at the last screenshot and either aim "
        "somewhere else, use find_text to locate what you want, or "
        "reconsider whether this is the right control."
    )
  ...
  if changed:
    ...
    note = (
        "SCREEN UNCHANGED -- that had no visible effect, so it probably "
...
def save_screenshot(name: str = "") -> str:
  ...
  Raises:
    RuntimeError: If the shared screenshot directory is not writable.
  """
  png = _raw()
  rel = _save(png, name)
  if not rel:
    raise RuntimeError(
        ...
        "directory bind-mounted from the agent's workspace."
    )
  return (
      f"Saved the screen to {rel}\n\n"
      "YOU ARE NOT DONE. The file exists but has not been sent.\n\n"
...
def find_text(pattern: str) -> str:
  ...
  if not hits:
    return (
        ...
        "scrolled out of view, or behind another window, or the app that "
        "shows it is not open yet."
    )
  hits.sort()
...
@mcp.tool()
@traced
def click(x: int, y: int, button: str = "left") -> _Result:
  ...
  return _acted(
      f"clicked {button} at ({x}, {y})", before, ("click", x, y, button)
  )


@mcp.tool()
@traced
def double_click(x: int, y: int) -> _Result:
...
@mcp.tool()
@traced
def move_mouse(x: int, y: int) -> _Result:
...
@mcp.tool()
@traced
def drag(from_x: int, from_y: int, to_x: int, to_y: int) -> _Result:
...
@mcp.tool()
@traced
def scroll(x: int, y: int, direction: str = "down", clicks: int = 3) -> _Result:
...
@mcp.tool()
@traced
def type_text(text: str) -> _Result:
...
@mcp.tool()
@traced
def press_key(keys: str) -> _Result:
...
@mcp.tool()
@traced
def wait(seconds: float = 2.0) -> _Result:
