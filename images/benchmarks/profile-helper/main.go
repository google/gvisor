...
func makeDir() {
	...
	if err := os.MkdirAll(*dirPath, 0755); err != nil {
		exitf("Cannot create directory %q: %v", *dirPath, err)
	}
}

// chmodDir chmods a directory.
func chmodDir() {
	if *dirPath == "" {
		usagef("Must provide --dir")
	}
	if *chmod == -1 {
		usagef("Must provide --chmod")
	}
	if *chmod > 0777 {
		usagef("--chmod must be in decimal, got %d (octal 0%o) which is larger than %d (octal 0%o)", *chmod, *chmod, 0777, 0777)
	}
	if err := os.Chmod(*dirPath, fs.FileMode(*chmod)); err != nil {
		exitf("Cannot chmod directory %q to 0%o: %v", *dirPath, *chmod, err)
	}
}
...
func deleteDir() {
	...
	if err := os.RemoveAll(*dirPath); err != nil {
		exitf("Cannot delete directory %q: %v", *dirPath, err)
	}
}
