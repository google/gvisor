...
//
//  cd /sys/devices/system/cpu/cpu<wakee>/cpuidle
//  echo 1 | sudo tee state1/disable state2/disable  # forbid C1/C1E
//  ... run the probe ...
//  echo 0 | sudo tee state1/disable state2/disable  # restore
#define _GNU_SOURCE
#include <dirent.h>
...
static int fwait(atomic_int* a, int v) {
  return syscall(SYS_futex, a, FUTEX_WAIT_PRIVATE, v, NULL, NULL, 0);
}
static int fwake(atomic_int* a) {
  return syscall(SYS_futex, a, FUTEX_WAKE_PRIVATE, 1, NULL, NULL, 0);
}
...
static void load_states(int cpu) {
  ...
  for (int i = 0; i < MAX_STATES; i++) {
    ...
    snprintf(path, sizeof(path),
             "/sys/devices/system/cpu/cpu%d/cpuidle/state%d/name", cpu, i);
    if (read_file(path, buf, sizeof(buf)) != 0) break;
    snprintf(state_name[i], sizeof(state_name[i]), "%s", buf);
    state_lat[i] = read_long(
        "/sys/devices/system/cpu/cpu%d/cpuidle/state%d/latency", cpu, i);
    nstates++;
  }
}
...
static void snap_usage(int cpu, long* out) {
  for (int i = 0; i < nstates; i++)
    out[i] = read_long("/sys/devices/system/cpu/cpu%d/cpuidle/state%d/usage",
                       cpu, i);
}
...
static int read_proc_stat(long* idle, long* total, int max) {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (sscanf(line, "cpu%d %ld %ld %ld %ld %ld %ld %ld %ld", &cpu, &u, &n, &s,
               &id, &io, &irq, &sirq, &st) >= 9 &&
        cpu >= 0 && cpu < max) {
      idle[cpu] = id + io;
...
int main(int argc, char** argv) {
  ...
  char kernel[128] = "?", model[128] = "?", driver[64] = "none", igov[64] = "?",
       fgov[64] = "?", arch[32] = "?";
  {
    char buf[256];
    struct utsname un;
    ...
    read_file("/sys/devices/system/cpu/cpuidle/current_driver", driver,
              sizeof(driver));
    read_file("/sys/devices/system/cpu/cpuidle/current_governor", igov,
              sizeof(igov));
    char p[128];
    snprintf(p, sizeof(p),
  ...
  printf(
      "CSTATEPROBE_META|kernel=%s|arch=%s|model=%s|idle_driver=%s|idle_"
      "governor=%s|cpufreq_governor=%s|load1=%.2f|waker=%d|wakee=%d|wakee_node="
      "%d|states=",
      kernel, arch, model, driver, igov, fgov, load1, waker_cpu, wakee_cpu,
      cpu_node[wakee_cpu]);
  for (int i = 0; i < nstates; i++)
    printf("%s%s:%ld", i ? "," : "", state_name[i], state_lat[i]);
  ...
  for (int g = 0; g < 4; g++) {
    ...
    printf(
        "CSTATEPROBE_RES|gap_us=%ld|n=%d|p50=%.1f|p90=%.1f|p99=%.1f|max=%.1f|"
        "dom=%s:%ld\n",
  ...
  printf(
      "CSTATEPROBE_SUM|shallow_p90=%.1f|deep_p90=%.1f|delta_p90=%.1f|deepest_"
      "adv=%ld\n",
