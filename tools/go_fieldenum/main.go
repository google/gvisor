...
func (si *structInfo) writeTo(b *strings.Builder) {
	...
	if len(si.reprByBit) != 0 {
		fmt.Fprintf(b, "\tfields [%d]atomic.Uint32\n", numBitmaskUint32s)
	}
	fmt.Fprintf(b, "}\n\n")
	...
	if len(si.reprByBit) != 0 {
		...
		if numBitmaskUint32s == 1 {
			fmt.Fprintf(b, "\treturn fs.fields[0].Load() & (uint32(1) << uint(f)) != 0\n")
		} else {
			fmt.Fprintf(b, "\treturn fs.fields[f/32].Load() & (uint32(1) << (f%%32)) != 0\n")
		}
		fmt.Fprintf(b, "}\n\n")
		...
		if numBitmaskUint32s == 1 {
			fmt.Fprintf(b, "\tfs.fields[0].Store(fs.fields[0].Load() | (uint32(1) << uint(f)))\n")
		} else {
			fmt.Fprintf(b, "\tfs.fields[f/32].Store(fs.fields[f/32].Load() | (uint32(1) << (f%%32))\n")
		}
		fmt.Fprintf(b, "}\n\n")
		...
		if numBitmaskUint32s == 1 {
			...
			fmt.Fprintf(b, "\tfs.fields[f/32].Store(fs.fields[f/32].Load() &^ (uint32(1) << uint(f%%32)))\n")
		}
		fmt.Fprintf(b, "}\n\n")
	}
	...
	for i := 0; i < numBitmaskUint32s; i++ {
		fmt.Fprintf(b, "\tcopied.fields[%d].Store(fs.fields[%d].Load())\n", i, i)
	}
	...
	for _, fieldName := range si.reprByBit {
		...
		if numBitmaskUint32s == 1 {
			fmt.Fprintf(b, "\t\tfs.fields[0].Store(fs.fields[0].Load() | (uint32(1) << uint(%s)))\n", fieldConstName)
		} else {
			fmt.Fprintf(b, "\t\tword, bit := %s/32, %s%%32\n", fieldConstName, fieldConstName)
			fmt.Fprintf(b, "\t\tfs.fields[word].Store(fs.fields[word].Load() | (uint32(1) << bit))\n")
		}
		fmt.Fprintf(b, "\t}\n")
	}
	fmt.Fprintf(b, "}\n\n")
