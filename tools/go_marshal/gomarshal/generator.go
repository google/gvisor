...
func newMarshallableType(marshalTag string, spec *ast.TypeSpec) marshallableType {
	tags := make(map[string]bool)
	for _, tag := range strings.Fields(strings.TrimPrefix(marshalTag, "// +marshal")) {
		tags[tag] = true
	}
...
func (g *Generator) collectMarshallableTypes(a *ast.File, f *token.FileSet) []marshallableType {
	...
	for _, decl := range a.Decls {
		...
		for _, spec := range gdecl.Specs {
			...
			types = append(types, newMarshallableType(tagLine, t))
...
func (g *Generator) generateOne(t marshallableType, fset *token.FileSet) *interfaceGenerator {
	...
	case *ast.StructType:
		...
		if t.vector {
			i.emitMarshallableVectorForStruct(ty)
		}
	case *ast.Ident:
		i.validatePrimitiveNewtype(ty)
		i.emitMarshallableForPrimitiveNewtype(ty)
		if t.vector {
			i.emitMarshallableVectorForPrimitiveNewtype(ty)
		}
	case *ast.ArrayType:
		...
		if t.vector {
			abortAt(fset.Position(t.spec.Pos()), fmt.Sprintf("Array type marked as '+marshal vector', but this is not supported. Perhaps fold one of the dimensions?"))
		}
	default:
