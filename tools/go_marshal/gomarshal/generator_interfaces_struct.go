...
func (g *interfaceGenerator) emitMarshallableVectorForStruct(st *ast.StructType) {
	// Is g.t a packed struct without consideing field types?
	thisPacked := g.isStructPacked(st)

	sliceType := fmt.Sprintf("%sSlice", g.typeName())
	g.emit("type %s []%s\n\n", sliceType, g.typeName())

	g.emit("// CopyInVec implements marshal.MarshallableVector.CopyInVec.\n")
	g.recordUsedImport("marshal")
	g.recordUsedImport("usermem")
	g.emit("func (%s *%s) CopyInVec(task marshal.Task, addr usermem.Addr) (int, error) {\n", g.r, sliceType)
	g.inIndent(func() {
		g.emit("len := len(*%s)\n", g.r)
		g.emit("if len == 0 {\n")
		g.inIndent(func() {
		...
		fallback := func() {
			g.emit("// Type %s doesn't have a packed layout in memory, fall back to UnmarshalBytes.\n", g.typeName())
			g.emit("buf := task.CopyScratchBuffer(size * len)\n")
			g.emit("count, err := task.CopyInBytes(addr, buf)\n")
			g.emit("if err != nil {\n")
			g.inIndent(func() {
				g.emit("return count, err\n")
			})
			g.emit("}\n")

			g.emit("for idx := 0; idx < count; idx++ {\n")
			g.inIndent(func() {
				g.emit("(*%s)[idx].UnmarshalBytes(buf[size*idx:(size+1)*idx])\n", g.r)
			})
			g.emit("}\n")

			g.emit("return count, nil\n")
		}
		if thisPacked {
			g.recordUsedImport("reflect")
			g.recordUsedImport("runtime")
			...
			g.emit("hdr := (*reflect.SliceHeader)(unsafe.Pointer(&buf))\n")
			g.emit("hdr.Data = val\n")
			g.emit("hdr.Len = size * len\n")
			g.emit("hdr.Cap = size * len\n")
	...
	g.emit("// CopyOutVec implements marshal.MarshallableVector.CopyOutVec.\n")
	g.recordUsedImport("marshal")
	g.recordUsedImport("usermem")
	g.emit("func (%s *%s) CopyOutVec(task marshal.Task, addr usermem.Addr) (int, error) {\n", g.r, sliceType)
	g.inIndent(func() {
		g.emit("count := len(*%s)\n", g.r)
		g.emit("if count == 0 {\n")
		g.inIndent(func() {
		...
		fallback := func() {
			...
			g.emit("for idx := 0; idx < count; idx++ {\n")
			g.inIndent(func() {
				g.emit("(*%s)[idx].MarshalBytes(buf[size*idx:(size+1)*idx])\n", g.r)
			})
			g.emit("}\n")
			g.emit("return task.CopyOutBytes(addr, buf)\n")
		}
