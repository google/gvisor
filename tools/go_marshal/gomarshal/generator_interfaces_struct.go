...
func (g *interfaceGenerator) emitMarshallableVectorForStruct(st *ast.StructType, vector *vectorAPI) {
	thisPacked := g.isStructPacked(st)

	if vector.inner {
		abortAt(g.f.Position(vector.comment.Slash), fmt.Sprintf("The ':inner' argument to '+marshal vector:%s:inner' is only applicable to newtypes on primitives. Remove it from this struct declaration.", vector.ident))
	}

	g.recordUsedImport("marshal")
	g.recordUsedImport("usermem")

	g.emit("// Copy%sIn copies in a slice of %s objects from the task's memory.\n", vector.ident, g.typeName())
	...
	g.inIndent(func() {
		...
		fallback := func() {
			...
			g.inIndent(func() {
				g.emit("dst[idx].UnmarshalBytes(buf[size*idx:(size+1)*idx])\n")
			})
			g.emit("}\n")
	...
	g.inIndent(func() {
		...
		fallback := func() {
			...
			g.emit("for idx := 0; idx < count; idx++ {\n")
			g.inIndent(func() {
				g.emit("src[idx].MarshalBytes(buf[size*idx:(size+1)*idx])\n")
			})
			g.emit("}\n")
			g.emit("_, err := task.CopyOutBytes(addr, buf)\n")
			g.emit("return err\n")
