...
func (g *interfaceGenerator) emitMarshallableVectorForPrimitiveNewtype(nt *ast.Ident) {
	g.recordUsedImport("marshal")
	g.recordUsedImport("usermem")
	g.recordUsedImport("reflect")
	g.recordUsedImport("runtime")
	...
	// For primitives, define the slice on the underlying type.
	g.emit("type %s []%s\n\n", sliceType, nt.Name)
	...
	g.inIndent(func() {
		...
		g.emit("count, err := task.CopyOutBytes(addr, buf)\n")
		g.emit("// Since we bypassed the compiler's escape analysis, indicate that %s\n", g.r)
		g.emit("// must live until after the CopyOutBytes.\n")
		g.emit("runtime.KeepAlive(%s)\n", g.r)
		g.emit("return count, err\n")
