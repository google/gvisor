...
For example, if you are a cloud container hosting service, you might want to run containers from different users on different virtual machines in order to provide isolation. Using gVisor, you could run containers from different users in the same virtual machine, because gVisor will provide the security isolation necessary between containers, as well as protecting the virtual machine host OS.
...
Figure 1: Simplified design of gVisor. Disclaimer: We are kernel developers, not artists.
____
...
So far, we’ve only discussed security-related features of gVisor, and you might ask, “What about performance, compatibility and stability?” We will cover these considerations in future posts.
...
It may seem strange that we would want our own software components to distrust each other. But by limiting the trust between components, each component is forced to defend itself against potentially malicious input. As a result, there is no single security barrier for an attacker to overcome. 
...
____
Disclaimer: Previously mentioned disclaimer about not being artists applies.
...
For example, the Sentry is not allowed any syscall that creates or opens a file descriptor. All file descriptors must be donated to the sandbox from a trusted process. Preventing a compromised Sentry from being able to create or open new file descriptors eliminates entire categories of file-based attacks. More on this in a later post.

Without networking, the Sentry uses 53 host syscalls, and with networking, it uses an additional 15[^7]. The Sentry itself is only allowed access to specific [whitelisted syscalls](https://github.com/google/gvisor/blob/master/runsc/boot/config.go). If any attempts are made to call something outside the whitelist, it is immediately blocked and the sandbox is killed by the Host OS. 


### Sentry/Gofer Interface:

The Sentry communicates with the Gofer through a local unix domain socket (UDS) via a version of the 9P protocol[^8]. The UDS file descriptor is passed to the sandbox during initialization and all communication between the Sentry and Gofer happens via 9P. We will go more into how Gofers work in future posts.
...
Figure 3: Syscall Handling. The path to exploiting the Host OS is through the solid boxes. 
____
...
We will also use it to introduce Google’s Vulnerability Reward Program[^13], and other ways the community can contribute to help make gVisor safe, fast and stable.
