...
var (
	...
	coverageFilesDirectory                     = flag.String("coverage-files-directory", "", "Coverage ONLY; specifies the directory which generate 3 coverage files, they are separately generated by runsc, sandbox and gofer.")
)
...
func main() {
	...
	conf := &boot.Config{
		...
		CoverageFilesDirectory:                     *coverageFilesDirectory,
	}
	...
	if subcmdCode == subcommands.ExitSuccess {
		...
		if *coverageFilesDirectory != "" {
			return
		} else {
			// If we exit here, no coverage files will be generated.
	...
	// If we don't add return, Bazel test `main_test` will fail here,
	// but it won't affect code coverage.
	return
	os.Exit(128)
}
