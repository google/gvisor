...
func openIDMappedMount(req *OpenMountArgs) (*os.File, error) {
	...
	if err := unix.MountSetattr(fd, "", setattrFlags, attr); err != nil {
		unix.Close(fd)
		return nil, fmt.Errorf("mount_setattr(%q) failed: %w (kernel version = %+v, attr=%#+v, flags=%+v, dest=%+v)", req.Mount.Source, err, release, attr, setattrFlags, req.Mount.Destination)
	}
