...
func TestCombinedRestore(t *testing.T) {
	...
	const (
		rootName     = "root-cont"
		subName      = "sub-cont"
		overlay1Dest = "/overlay1"
		overlay2Dest = "/overlay2"
	)
...
func runCombinedRestoreTest(t *testing.T, t1Writable, t2Writable, t1Empty map[string]bool, paths []checkpoint.ResourceID, expectedRestored map[string]bool, rootName, subName, overlay1Dest, overlay2Dest string) {
	...
	getWritability := func(wMap map[string]bool, key string) bool {
		if wMap == nil {
			return true
		}
		w, ok := wMap[key]
	...
	// Extract writability flags. Unspecified mounts default to writable (true).
	rootWritableT1 := getWritability(t1Writable, "root-cont:/")
	overlay1WritableT1 := getWritability(t1Writable, "root-cont:"+overlay1Dest)
	subRootWritableT1 := getWritability(t1Writable, "sub-cont:/")
	overlay2WritableT1 := getWritability(t1Writable, "sub-cont:"+overlay2Dest)

	rootWritableT2 := getWritability(t2Writable, "root-cont:/")
	overlay1WritableT2 := getWritability(t2Writable, "root-cont:"+overlay1Dest)
	subRootWritableT2 := getWritability(t2Writable, "sub-cont:/")
	overlay2WritableT2 := getWritability(t2Writable, "sub-cont:"+overlay2Dest)

	conf.Overlay2.Set("all:dir=" + overlayDir)

	// Start two containers which sleep.
	appSleepCmd := []string{"/app", "task-tree", "--depth=0"}
	saveSpecs, ids := createSpecs(appSleepCmd, appSleepCmd)
	...
	conts1RootsCleanup, err := setupContainerRoots(saveSpecs, ids)
	if err != nil {
		t.Fatalf("failed to setup container roots 1: %v", err)
	}
	defer conts1RootsCleanup()
	...
	setupT1 := func(cont *Container, mountPath string, writable, empty bool) {
		...
		if out, err := executeCombinedOutput(conf, cont, nil, "/app", "fileOp", "--op=write", "--path="+modFile, "--content=initial"); err != nil {
			t.Fatalf("failed to setup T1 modFile in %q: err %v, out %s", mountPath, err, out)
		}
		if out, err := executeCombinedOutput(conf, cont, nil, "/app", "fileOp", "--op=write", "--path="+delFile, "--content=delete_me"); err != nil {
			t.Fatalf("failed to setup T1 delFile in %q: err %v, out %s", mountPath, err, out)
		}
	}
	...
	restoreSpecs, restoreIDs := createSpecs(appSleepCmd, appSleepCmd)
	...
	conts2RootsCleanup, err := setupContainerRoots(restoreSpecs, restoreIDs)
	if err != nil {
		t.Fatalf("failed to setup container roots 2: %v", err)
	}
	defer conts2RootsCleanup()
	...
	mutateToT2 := func(cont *Container, mountPath string, t1Writable, t1Empty, t2Writable bool) {
		...
		if out, err := executeCombinedOutput(conf, cont, nil, "/app", "fileOp", "--op=write", "--path="+newFile, "--content=new"); err != nil {
			t.Fatalf("failed to write newFile in %q: err %v, out %s", mountPath, err, out)
		}
		if t1Writable && !t1Empty {
			modFile := filepath.Join(mountPath, "file_to_modify")
			delFile := filepath.Join(mountPath, "file_to_delete")
			if out, err := executeCombinedOutput(conf, cont, nil, "/app", "fileOp", "--op=write", "--path="+modFile, "--content=modified"); err != nil {
				t.Fatalf("failed to modify modFile in %q: err %v, out %s", mountPath, err, out)
			}
			if out, err := executeCombinedOutput(conf, cont, nil, "/app", "fileOp", "--op=truncate", "--path="+delFile); err != nil {
				t.Fatalf("failed to truncate delFile in %q: err %v, out %s", mountPath, err, out)
			}
		}
	}
	...
	if t2Writable {
		if restored {
			if t1Writable && !t1Empty {
				...
				// Verify deleted file is size 0.
				verifyFileSizeZero(t, conf, cont, delFile)
			} else {
...
func verifyFileContent(t *testing.T, conf *config.Config, cont *Container, path, expected string) {
	t.Helper()
	stdout, err := executeCombinedOutput(conf, cont, nil, "/app", "fileOp", "--op=read", "--path="+path)
	if err != nil {
		t.Fatalf("failed to read %q: %v, out: %s", path, err, stdout)
	}
...
func verifyFileSizeZero(t *testing.T, conf *config.Config, cont *Container, path string) {
	...
	if len(stdout) != 0 {
		t.Errorf("file %q is not empty: got %q", path, string(stdout))
	}
}

func verifyFileNotExists(t *testing.T, conf *config.Config, cont *Container, path string) {
	t.Helper()
	out, err := executeCombinedOutput(conf, cont, nil, "/app", "fileOp", "--op=exists", "--path="+path)
	if err == nil {
		t.Errorf("file %q exists but should not (out: %s)", path, out)
	}
}
...
func TestSplitFSCheckpointRestoreMixed(t *testing.T) {
	...
	if out, err := executeCombinedOutput(conf, cont1, nil, testAppPath, "fileOp", "--op=write", "--path="+tmpfsFile, "--content=hello-tmpfs"); err != nil {
		t.Fatalf("failed to write to tmpfs: %v, out: %s", err, out)
	}

	if out, err := executeCombinedOutput(conf, cont1, nil, testAppPath, "fileOp", "--op=write", "--path="+overlayFile, "--content=hello-overlay"); err != nil {
		t.Fatalf("failed to write to overlay: %v, out: %s", err, out)
	}
	...
	if err := cont2.Restore(conf, checkpointDir1, false, true, true /* splitFSRestore */, nil); err != nil {
		t.Fatalf("failed to restore container 2: %v", err)
	}

	stdout, err := executeCombinedOutput(conf, cont2, nil, testAppPath, "fileOp", "--op=read", "--path="+tmpfsFile)
	if err != nil {
		t.Fatalf("failed to read tmpfs file: %v", err)
	}
	if got := strings.TrimSpace(string(stdout)); got != "hello-tmpfs" {
		t.Errorf("unexpected tmpfs content: got %q, want %q", got, "hello-tmpfs")
	}

	stdout, err = executeCombinedOutput(conf, cont2, nil, testAppPath, "fileOp", "--op=read", "--path="+overlayFile)
	if err != nil {
	...
	if out, err := executeCombinedOutput(conf, cont2, nil, testAppPath, "fileOp", "--op=write", "--path="+overlayFile, "--content=hello-overlay-modified"); err != nil {
		t.Fatalf("failed to modify overlay: %v, out: %s", err, out)
	}

	if out, err := executeCombinedOutput(conf, cont2, nil, testAppPath, "fileOp", "--op=write", "--path="+tmpfsFile, "--content=hello-tmpfs-modified"); err != nil {
		t.Fatalf("failed to modify tmpfs: %v, out: %s", err, out)
	}
	...
	cont3, err := New(conf, Args{
		...
		Spec:               spec,
		BundleDir:          bundleDir,
		FSRestoreImagePath: checkpointDir2,
	})
	if err != nil {
	...
	if err := cont3.Restore(conf, checkpointDir1, false, true, true /* splitFSRestore */, nil); err != nil {
		t.Fatalf("failed to restore container 3: %v", err)
	}

	stdout, err = executeCombinedOutput(conf, cont3, nil, testAppPath, "fileOp", "--op=read", "--path="+tmpfsFile)
	if err != nil {
		t.Fatalf("failed to read tmpfs file: %v", err)
	}
	if got := strings.TrimSpace(string(stdout)); got != "hello-tmpfs" {
	...
	stdout, err = executeCombinedOutput(conf, cont3, nil, testAppPath, "fileOp", "--op=read", "--path="+overlayFile)
	if err != nil {
		t.Fatalf("failed to read overlay file: %v", err)
	}
...
// TestSplitFSCheckpointRestoreMixedMmap verifies that under combined restore
// of sentry state from T1 and tmpfs filesystem from T2, the pre-checkpoint
// memory mapping retains the original T1 memory state ("hello") while the
// filesystem contains the T2 content ("world"). mmap-helper detects this
// mismatch and exits with ExitFailure (status 1).
func TestSplitFSCheckpointRestoreMixedMmap(t *testing.T) {
	...
	tmpfsMount := "/tmpfs-mount"
	targetFile := filepath.Join(tmpfsMount, "file.txt")
	readyFile := filepath.Join(tmpfsMount, "ready")
	goFile := filepath.Join(tmpfsMount, "go")
	...
	spec1 := testutil.NewSpecWithArgs(
		...
		"-go", goFile,
		"-output", outputFile,
		"-dummy", filepath.Join(tmpfsMount, "dummy.txt"),
		"-content", "hello",
		"-want", "world",
	)
	spec1.Mounts = append(spec1.Mounts, specs.Mount{
		Destination: tmpfsMount,
		Type:        "tmpfs",
	})
	...
	spec2.Mounts = append(spec2.Mounts, specs.Mount{
		Destination: tmpfsMount,
		Type:        "tmpfs",
	})
	...
	// Wait for ready file to be created inside guest.
	err = testutil.Poll(func() error {
		if ws, err := execute(conf, cont1, "/bin/sh", "-c", "test -f "+readyFile); err != nil || ws != 0 {
			return fmt.Errorf("ready file not found in container 1")
		}
		return nil
	}, 5*time.Second)
	if err != nil {
	...
	// Wait for ready file to be created inside guest.
	err = testutil.Poll(func() error {
		if ws, err := execute(conf, cont2, "/bin/sh", "-c", "test -f "+readyFile); err != nil || ws != 0 {
			return fmt.Errorf("ready file not found in container 2")
		}
		return nil
	}, 5*time.Second)
	if err != nil {
	...
	// Restore cont3 in background. It will resume waiting for "go" file.
	if err := cont3.Restore(conf, checkpointDir1, false, true, true /* splitFSRestore */, nil); err != nil {
		t.Fatalf("failed to restore container 3: %v", err)
	}

	// Create the go file inside container 3 to let the process resume and verify the mapping.
	if ws, err := execute(conf, cont3, "/bin/touch", goFile); err != nil || ws != 0 {
		t.Fatalf("failed to create go file: ws %d, err %v", ws, err)
	}
	...
	// Verify container 3 exited with ExitMmapMismatch (status 10) because mmap-helper
	// detects that the pre-checkpoint memory mapping does not match the newly
	// restored filesystem content ("world"). This distinct exit status differentiates
	// the expected mmap content mismatch from generic initialization or argument errors.
	if !ws.Exited() || ws.ExitStatus() != 10 {
		t.Errorf("container 3 wait status = %v (signaled: %v, signal: %v, exited: %v, exit status: %v), want exited with status 10 (mmap mismatch)",
			ws, ws.Signaled(), ws.Signal(), ws.Exited(), ws.ExitStatus())
	}
}
...
func TestSplitFSCheckpointRestoreMixedReadMismatched(t *testing.T) {
	...
	err = cont3.Restore(conf, checkpointDir1, false, false, true /* splitFSRestore */, nil)
	if err == nil {
...
func TestMultiContainerSplitFSCheckpointRestoreMixed(t *testing.T) {
	...
	conts3, cleanupContainers3, err := restoreContainersWithFSRestore(conf, testSpecs, newIds2, checkpointDir1, checkpointDir2)
	if err != nil {
...
func restoreContainersWithFSRestore(conf *config.Config, specs []*specs.Spec, ids []string, imagePath string, fsRestoreImagePath string) ([]*Container, func(), error) {
	...
	for i, spec := range specs {
		...
		if err := cont.Restore(conf, imagePath, false /* direct */, false /* background */, fsRestoreImagePath != "", nil /* networkArgs */); err != nil {
...
func setupContainerRoots(testSpecs []*specs.Spec, ids []string) (func(), error) {
...
func TestCombinedRestoreMountMismatch(t *testing.T) {
	...
	// T1: Read-only rootfs.
	testAppSleepArgv := []string{"/app", "task-tree", "--depth=0"}
	...
	cleanupRoots1, err := setupContainerRoots(saveSpecs, ids)
	if err != nil {
		t.Fatalf("failed to setup container roots 1: %v", err)
	}
	defer cleanupRoots1()
	...
	cleanupRoots2, err := setupContainerRoots(restoreSpecs, restoreIDs)
	if err != nil {
		t.Fatalf("failed to setup container roots 2: %v", err)
	}
	defer cleanupRoots2()
