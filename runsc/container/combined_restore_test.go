...
func TestSplitFSCheckpointRestoreMixed(t *testing.T) {
	...
	if ws, err := execute(conf, cont1, "/bin/bash", "-c", fmt.Sprintf("echo hello-tmpfs > %s", tmpfsFile)); err != nil || ws != 0 {
		t.Fatalf("failed to write to tmpfs: err %v, ws %v", err, ws)
	}

	if ws, err := execute(conf, cont1, "/bin/bash", "-c", fmt.Sprintf("echo hello-overlay > %s", overlayFile)); err != nil || ws != 0 {
	...
	if ws, err := execute(conf, cont2, "/bin/bash", "-c", fmt.Sprintf("echo hello-overlay-modified > %s", overlayFile)); err != nil || ws != 0 {
		t.Fatalf("failed to modify overlay: err %v, ws %v", err, ws)
	}

	if ws, err := execute(conf, cont2, "/bin/bash", "-c", fmt.Sprintf("echo hello-tmpfs-modified > %s", tmpfsFile)); err != nil || ws != 0 {
...
func TestSplitFSCheckpointRestoreMixedMmap(t *testing.T) {
	...
	if err := cont3.Restore(conf, checkpointDir1, false, false, true /* splitFSRestore */, nil); err != nil {
		t.Fatalf("failed to restore container 3: %v", err)
	}
