...
// sandboxRunning reports whether c's sandbox process is running.
func sandboxRunning(t *testing.T, c *Container) bool {
	t.Helper()

	running, err := c.IsSandboxRunning()
	if err != nil {
		t.Fatalf("error checking whether sandbox is running: %v", err)
	}
	return running
}
