...
func TestIDMappedMountPropagation(t *testing.T) {
	for name, conf := range configs(t, false /* noOverlay */) {
		t.Run(name, func(t *testing.T) {
			// Make our /tmp mountpoint private
			privateDir, err := os.MkdirTemp(testutil.TmpDir(), "private-mount")
			if err != nil {
				t.Fatalf("os.MkdirTemp() failed: %v", err)
			}
			err = unix.Mount("tmpfs", privateDir, "tmpfs", 0, "")
			if err != nil {
				t.Fatalf("mount root tmpfs failed: %v", err)
			}
			err = unix.Mount("", privateDir, "tmpfs", unix.MS_PRIVATE, "")
			if err != nil {
				t.Fatalf("make root tmpfs private failed: %v", err)
			}

			dir := filepath.Join(privateDir, "id-mapped-mount-prop")
