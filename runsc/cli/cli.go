...
func Run(commands map[util.SubCommand]string, helpTopics []subcommands.Command) {
	...
	if *logFD > -1 {
		...
		var err error
		errorLogger, err = specutils.OpenLogFile(conf.LogFilename, os.O_WRONLY|os.O_CREATE|os.O_APPEND, lfOpts)
		if err != nil {
...
func (b *bufferedEmitter) Emit(depth int, level log.Level, timestamp time.Time, format string, v ...any) {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.buffer = append(b.buffer, bufferedLog{depth: depth, level: level, timestamp: timestamp, format: format, v: v})
}
...
func (b *bufferedEmitter) drain(emitter log.Emitter) {
	...
	for _, l := range b.buffer {
		emitter.Emit(l.depth, l.level, l.timestamp, l.format, l.v...)
	}
	b.buffer = nil
}
