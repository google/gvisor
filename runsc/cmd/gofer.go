...
func (g *Gofer) Execute(_ context.Context, f *flag.FlagSet, args ...any) subcommands.ExitStatus {
	...
	if g.setUpRoot {
		if err := sandboxsetup.SetupRootFS(spec, conf, toMountConfs(g.mountConfs), g.devIoFD, makeRPCMountOpener(goferToHostRPC)); err != nil {
	...
	// setupRootFS().
	cleanMounts, err := sandboxsetup.ResolveMounts(conf, spec.Mounts, root, toMountConfs(g.mountConfs))
	if err != nil {
...
// toMountConfs converts boot.GoferMountConfFlags to a slice of
// sandboxsetup.MountConf, allowing the sandboxsetup functions to work
// without importing the heavy runsc/boot package.
func toMountConfs(confs boot.GoferMountConfFlags) []sandboxsetup.MountConf {
	result := make([]sandboxsetup.MountConf, len(confs))
	for i, c := range confs {
		result[i] = c
	}
	return result
