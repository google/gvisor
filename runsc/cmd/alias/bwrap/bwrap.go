...
func (c *bwrapConfig) sandboxOptions() ([]sandbox.Option, error) {
	...
	opts := []sandbox.Option{
		...
		// The sandbox is driven by the runsc we are already running as, under
		// the global flags this invocation was given.
		sandbox.WithRunscPath(specutils.ExePath),
		sandbox.WithRunscFlags(c.runscConfig.ToFlags()...),
	}
	if c.Hostname != "" {
	...
	// The Sentry isolates IPC, PID and UTS regardless, so the sandbox needs
	// only the namespaces bwrap was explicitly asked for.
...
func do(ctx context.Context, c *bwrapConfig, waitStatus *unix.WaitStatus) subcommands.ExitStatus {
	...
	exitCode, err := sandbox.Run(ctx, opts...)
	if err != nil {
		return util.Errorf("bwrap: %v", err)
	}
	// runsc already exits with the application's status, including 128+signal
	// when it was signaled. Re-encode it as a wait status so that the runsc CLI
	// exits with the same code.
	*waitStatus = unix.WaitStatus(exitCode << 8)
