...
func rdmaProxyUpdateChroot(chroot string, spec *specs.Spec, conf *config.Config) error {
	if !specutils.HasRDMADevicesInSpec(spec) {
		return nil
	}
...
func numaUpdateChroot(chroot string, spec *specs.Spec, conf *config.Config) error {
	// Collect host NUMA topology for the same workloads that need PCI data.
	// NCCL reads /sys/devices/system/node/nodeN/cpumap to populate the
	// per-NUMA-node CPU affinity bitmaps in its topology XML; without a
	// matching node hierarchy, devices' numa_node values dangle and NCCL
	// falls back to a single-rail layout.
	if !conf.NVProxy && !specutils.HasRDMADevicesInSpec(spec) {
		return nil
	}
