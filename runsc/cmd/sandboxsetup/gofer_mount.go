...
// vfioPathDir is the directory containing VFIO device nodes. This avoids
// importing the heavy pkg/sentry/devices/tpuproxy/vfio package just for
// the constant "/dev/vfio/vfio".
...
// mount configs.
func SetupRootFS(spec *specs.Spec, conf *config.Config, mountConfs []MountConf, devIoFD int, mountOpener MountOpener) error {
...
func SetupMounts(conf *config.Config, mounts []specs.Mount, root, procPath string, mountConfs []MountConf, mountOpener MountOpener) (retErr error) {
...
func ResolveMounts(conf *config.Config, mounts []specs.Mount, root string, mountConfs []MountConf) ([]specs.Mount, error) {
	mountIdx := 1 // First index is for rootfs.
