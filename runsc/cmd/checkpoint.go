...
func (c *Checkpoint) Execute(_ context.Context, f *flag.FlagSet, args ...any) subcommands.ExitStatus {
	...
	if c.filestoreSnapshotDir != "" && !c.skipFilestorePages {
		util.Fatalf("--filestore-snapshot-dir requires --skip-filestore-pages (the snapshot replaces the out-of-band capture of filestore contents)")
	}
