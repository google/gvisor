...
func (l *create) SetFlags(f *flag.FlagSet) {
	f.StringVar(&l.config, "config", "", "TODO")
}
...
func (l *create) Execute(_ context.Context, f *flag.FlagSet, args ...interface{}) subcommands.ExitStatus {
	...
	sinkFiles, err := seccheck.SetupSinks(sessionConfig.Sinks)
	if err != nil {
		return util.Errorf(err.Error())
	}
	defer func() {
	...
	if err := c.Sandbox.CreateTraceSession(sessionConfig, sinkFiles); err != nil {
