...
func TestResolveRestorePaths(t *testing.T) {
	testCases := []struct {
		name                   string
		paths                  []string
		flagFSRestoreImagePath string
		wantCheckpointDir      string
		wantFSRestoreImage     string
		wantCombined           bool
		expectErr              bool
	}{
		{
			name:                   "single path no fs flag",
			paths:                  []string{"/path/to/checkpoint"},
			flagFSRestoreImagePath: "",
			wantCheckpointDir:      "/path/to/checkpoint",
			wantFSRestoreImage:     "",
			wantCombined:           false,
		},
		{
			name:                   "single path with fs flag",
			paths:                  []string{"/path/to/checkpoint"},
			flagFSRestoreImagePath: "/path/to/fs",
			wantCheckpointDir:      "/path/to/checkpoint",
			wantFSRestoreImage:     "/path/to/fs",
			wantCombined:           true,
		},
		{
			name:                   "two paths no fs flag",
			paths:                  []string{"/path/to/checkpoint", "/path/to/fs"},
			flagFSRestoreImagePath: "",
			wantCheckpointDir:      "/path/to/checkpoint",
			wantFSRestoreImage:     "/path/to/fs",
			wantCombined:           true,
		...
		{
			name:                   "two paths exact matching fs flag",
			paths:                  []string{"/path/to/checkpoint", "/path/to/fs"},
			flagFSRestoreImagePath: "/path/to/fs",
			wantCheckpointDir:      "/path/to/checkpoint",
			wantFSRestoreImage:     "/path/to/fs",
		...
		{
			name:                   "two paths trailing slash on fs flag matches image-path",
			paths:                  []string{"/path/to/checkpoint", "/path/to/fs"},
			flagFSRestoreImagePath: "/path/to/fs/",
			wantCheckpointDir:      "/path/to/checkpoint",
			wantFSRestoreImage:     "/path/to/fs",
		...
		{
			name:                   "two paths trailing slash on image-path matches fs flag",
			paths:                  []string{"/path/to/checkpoint", "/path/to/fs/"},
			flagFSRestoreImagePath: "/path/to/fs",
			wantCheckpointDir:      "/path/to/checkpoint",
			wantFSRestoreImage:     "/path/to/fs/",
		...
		{
			name:                   "two paths unnormalized dot components matches fs flag",
			paths:                  []string{"/path/to/checkpoint", "/path/./to//fs"},
			flagFSRestoreImagePath: "/path/to/fs",
			wantCheckpointDir:      "/path/to/checkpoint",
			wantFSRestoreImage:     "/path/./to//fs",
		...
		{
			name:                   "two paths conflicting fs flag",
			paths:                  []string{"/path/to/checkpoint", "/path/to/fs1"},
			flagFSRestoreImagePath: "/path/to/fs2",
			expectErr:              true,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			checkpointDir, fsRestoreImage, combined, err := resolveRestorePaths(tc.paths, tc.flagFSRestoreImagePath)
			if (err != nil) != tc.expectErr {
				t.Fatalf("resolveRestorePaths(%v, %q) err = %v, expectErr = %v", tc.paths, tc.flagFSRestoreImagePath, err, tc.expectErr)
			}
			if !tc.expectErr {
				if checkpointDir != tc.wantCheckpointDir {
					t.Errorf("checkpointDir = %q, want %q", checkpointDir, tc.wantCheckpointDir)
				}
				if fsRestoreImage != tc.wantFSRestoreImage {
					t.Errorf("fsRestoreImage = %q, want %q", fsRestoreImage, tc.wantFSRestoreImage)
				}
				if combined != tc.wantCombined {
					t.Errorf("combined = %v, want %v", combined, tc.wantCombined)
				}
			}
		})
	}
