...
// address range above its own image. This frees their anonymous pages and
...
// Its bottom also holds this process's name (see `set_cmdline`).
char park_stack[PAGE] __attribute__((aligned(MAX_PAGE), used));

// The name this process reports in /proc/<pid>/cmdline. See `set_cmdline`.
#define PARKING_NAME "runsc-fd-parking"

// The name's bytes.
extern const char parking_name[];
__asm__(
    ".globl parking_name\n"
    "parking_name:\n"
    "  .asciz \"" PARKING_NAME "\"\n");
...
// set_cmdline points `/proc/self/cmdline` to a user-friendly name.
//
// The kernel serves the cmdline out of [mm->arg_start, mm->arg_end), which it
// set to the argv strings it wrote onto the original stack.
// `slim` will throw that stack away, so without this the sidecar would show as
// having an empty `/proc/<pid>/cmdline`, not great. This fixes that.
//
// Requires CAP_SYS_RESOURCE, otherwise falls back to not having a cmdline.
static void set_cmdline(void) {
  // Write byte-by-byte through a volatile pointer as we have no memcpy.
  volatile char* name = park_stack;
  for (unsigned long i = 0; i < sizeof(PARKING_NAME); i++) {
    name[i] = parking_name[i];
  }
  sys5(__NR_prctl, PR_SET_MM, PR_SET_MM_ARG_START, (long)park_stack, 0, 0);
  sys5(__NR_prctl, PR_SET_MM, PR_SET_MM_ARG_END,
       (long)(park_stack + sizeof(PARKING_NAME)), 0, 0);
}
...
__attribute__((noreturn, used)) void fdparking_main(void) {
  ...
  // This is a weird way to allocate a 1-second timespec, but necessary to
  // avoid it being pushed onto a new mapping.
  long one_second = 1;
  __asm__("" : "+r"(one_second));
  struct kernel_timespec grace;
  grace.tv_sec = one_second;
  grace.tv_nsec = 0;
  long n = park(&pfd, &grace);
  if (n == 0) {
    // Grace period elapsed and sandbox still running. Do the deferred work.
    set_cmdline();
    slim();
