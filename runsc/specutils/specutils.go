...
//
// noRootContainer says the spec describes a sandbox, not a container. See
// Loader.noRootContainer in runsc/boot/loader.go.
func ValidateSpec(spec *specs.Spec, conf *config.Config, noRootContainer bool) error {
	// Mandatory fields. A sandbox has no process and no rootfs; reject them
	// here, since boot.New would only ignore them.
	if noRootContainer {
		if spec.Process != nil {
...
func ReadSpec(bundleDir string, conf *config.Config, noRootContainer bool) (*specs.Spec, error) {
	...
	return ReadSpecFromFile(bundleDir, specFile, conf, noRootContainer)
}
...
func ReadSpecFromFile(bundleDir string, specFile *os.File, conf *config.Config, noRootContainer bool) (*specs.Spec, error) {
	...
	if err := ValidateSpec(&spec, conf, noRootContainer); err != nil {
		return nil, err
	}
