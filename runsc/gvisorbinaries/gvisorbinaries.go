...
func (b *Binary) WarnUnavailable(action string) {
	...
	case config.SidecarUsageLegacyEmbedded:
		log.Warningf("%s; embedded sidecar binaries are deprecated and will stop working after 2026-11. This slows down gVisor startup. Please install sidecar binaries as per https://gvisor.dev/docs/user_guide/install/", action)
	case config.SidecarUsageDefault:
		log.Warningf("%s; embedded sidecar binaries are deprecated and will be removed in a future release. Please install sidecar binaries per https://gvisor.dev/docs/user_guide/install/ or set `--sidecar-usage-policy=LEGACY_DEPRECATED_SLOW_EMBEDDED_FALLBACK` as a temporary option to restore functionality (this slows down gVisor startup and will stop working after 2026-11).", action)
	}
}
