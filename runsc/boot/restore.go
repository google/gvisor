...
type restorer struct {
	...
	// combinedFSRestore indicates if this is a combined restore.
	combinedFSRestore bool
...
func (r *restorer) restore(l *Loader) error {
	...
	// In combined restore, FSTarProvider triggers filesystems (e.g. tmpfs) to
	// wipe obsolete Sentry state mappings and reload from tar. In normal split
	// restore, it is omitted so in-memory dentries and open FDs are preserved.
	if l.fsRestore != nil && r.combinedFSRestore {
