...
func TestParseFSCheckpointPaths(t *testing.T) {
	testCases := []struct {
		...
		{
			name:  "multiple clean paths with spaces",
			input: " /tmp , /var/run , / ",
			want: []checkpoint.ResourceID{
				{Path: "/tmp"},
				{Path: "/var/run"},
		...
		{
			name:  "container prefixed clean paths",
			input: "c1:/tmp , c2:/data",
			want: []checkpoint.ResourceID{
		...
		{
			name:      "uncleaned trailing slash error",
			input:     "/tmp/",
			expectErr: true,
		},
		{
			name:      "uncleaned redundant slashes error",
			input:     "/var//run",
			expectErr: true,
		},
		{
			name:      "uncleaned dot error",
			input:     "/var/run/.",
			expectErr: true,
		},
		{
			name:      "uncleaned dot dot error",
			input:     "c2:/data/app/..",
			expectErr: true,
		},
		{
...
func TestFindByResourceID(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "error when ambiguous multiple matches",
			entries: []testResource{
				{id: checkpoint.ResourceID{ContainerName: "c1", Path: "/tmp"}, value: "val1"},
				{id: checkpoint.ResourceID{ContainerName: "c2", Path: "/tmp"}, value: "val2"},
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, found, err := findByResourceID(m, tc.query, func(r *testResource) checkpoint.ResourceID {
				return r.id
			}, "testResource")

			if (err != nil) != tc.expectErr {
				t.Fatalf("findByResourceID returned err = %v, expectErr = %v", err, tc.expectErr)
			}
