...
func (w *Walker) walkP4Ds(pgdEntry *PTE, p4dEntries *PTEs, start, end uintptr, clearP4DEntries *uint16) bool {
	for start < end {
		var (
			pudEntries *PTEs
		)
		nextBoundary := w.pageTables.p4dAddrEnd(start, end)
		p4dEntry := w.pageTables.p4dEntry(pgdEntry, p4dEntries, start)
		if !p4dEntry.Valid() {
			if !w.visitor.requiresAlloc() {
				// Skip over this entry.
				(*clearP4DEntries)++
...
func (w *Walker) iterateRangeCanonical(start, end uintptr) bool {
	...
	for start < end {
		...
		// Map the next level.
		clearP4DEntries := uint16(0)
		if !w.walkP4Ds(pgdEntry, p4dEntries, start, nextBoundary, &clearP4DEntries) {
			return false
		}
