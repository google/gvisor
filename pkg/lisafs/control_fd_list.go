...
//
//go:nosplit
func (l *controlFDList) PushFrontList(m *controlFDList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		controlFDElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		controlFDElementMapper{}.linkerFor(m.tail).SetNext(l.head)
