...
const channelSize = int(1 + flipcall.PacketHeaderBytes + MaxMessageSize)

func init() {
	maxChannels = runtime.NumCPU()
	if maxChannels < 2 {
		maxChannels = 2
	}
	if maxChannels > 4 {
...
func (ch *channel) sndFDs(fds []int) (uint8, error) {
	numFDs := len(fds)
	if numFDs == 0 {
		return 0, nil
	}
	...
	if numFDs > int(^uint8(0)) {
		// Too many FDs to donate.
		return 0, unix.EINVAL
	}

	for i, fd := range fds {
		if err := ch.fdChan.SendFD(fd); err != nil {
			log.Warningf("error occured while sending (%d/%d)th FD on channel(%p): %v", i+1, numFDs, ch, err)
			return uint8(i), nil
		}
	}
	return uint8(numFDs), nil
}
