...
//
//go:nosplit
func (l *openFDList) PushFrontList(m *openFDList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		openFDElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		openFDElementMapper{}.linkerFor(m.tail).SetNext(l.head)
...
//
//go:nosplit
func openFDRingAdd(old *OpenFD, new *OpenFD) {
	oldLinker := openFDElementMapper{}.linkerFor(old)
	newLinker := openFDElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func openFDRingRemove(e *OpenFD) {
	eLinker := openFDElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func openFDRingEmpty(e *OpenFD) bool {
	linker := openFDElementMapper{}.linkerFor(e)
