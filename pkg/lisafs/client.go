...
func NewClient(sock *unet.Socket, mountPath string) (*Client, Inode, error) {
	...
	for _, unsupportedM := range mountResp.UnsupportedMs {
		if unsupportedM > mountResp.MaxM {
			log.Warningf("server responded with invalid unsupported message ID = %d, which is greater than MaxM = %d", unsupportedM, mountResp.MaxM)
			return nil, Inode{}, unix.EINVAL
		}
	...
	channelErrs := make([]error, maxChannels)
