...
func (s *vmaSet) Remove(seg vmaIterator) vmaGapIterator {
	...
	return seg.node.rebalanceAfterRemove(vmaGapIterator(seg))
}
...
func (s *vmaSet) RemoveRangeWith(r __generics_imported0.AddrRange, f func(seg vmaIterator)) vmaGapIterator {
	seg, gap := s.Find(r.Start)
	if seg.Ok() {
		seg = s.Isolate(seg, r)
		if f != nil {
...
// panics.
func (s *vmaSet) RemoveFullRangeWith(r __generics_imported0.AddrRange, f func(seg vmaIterator)) vmaGapIterator {
...
func (s *vmaSet) MoveFrom(s2 *vmaSet) {
	*s = *s2
