...
// copy-on-write on the pinned range (even if at.Write is false), so that
// writes through the application's mappings remain coherent with the pinned
// pages, and Fork() copies pinned pages to the child immediately rather than
// making them copy-on-write.
//
// Preconditions:
//   - ar.Length() != 0.
//   - ar must be page-aligned.
