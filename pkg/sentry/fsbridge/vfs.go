...
func NewVFSFile(file *vfs.FileDescription) *vfsFile {
...
func (f *vfsFile) Type(ctx context.Context) (uint8, error) {
	stat, err := f.file.Stat(ctx, vfs.StatOptions{})
	if err != nil {
		return 0, err
	}
	return linux.FileMode(stat.Mode).DirentType(), nil
}
...
func (f *vfsFile) CheckPermission(ctx context.Context, ats vfs.AccessTypes) error {
	...
	isDir := mode.DirentType() == linux.DT_DIR
	return vfs.GenericCheckPermissions(creds, ats, isDir, uint16(mode.Permissions()), auth.KUID(stat.UID), auth.KGID(stat.GID))
}
...
func NewVFSLookup(mntns *vfs.MountNamespace, root, workingDir vfs.VirtualDentry) *vfsLookup {
	return &vfsLookup{
		mntns:      mntns,
		root:       root,
...
// default anyways.
func (l *vfsLookup) OpenPath(ctx context.Context, path string, ats vfs.AccessTypes, _ *uint, resolveFinal bool) (File, error) {
	...
	opts := &vfs.OpenOptions{Flags: accessTypeToOpenFlags(ats)}
...
func accessTypeToOpenFlags(ats vfs.AccessTypes) uint32 {
	if ats.MayRead() && ats.MayWrite() {
	...
	panic(fmt.Sprintf("Invalid AccessType to open file: %v", ats))
}
