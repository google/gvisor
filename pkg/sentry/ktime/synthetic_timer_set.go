...
const syntheticTimertrackGaps = 0

var _ = uint8(syntheticTimertrackGaps << 7) // Will fail if not zero or one.

// dynamicGap is a type that disappears if trackGaps is 0.
type syntheticTimerdynamicGap [syntheticTimertrackGaps]uint64

// Get returns the value of the gap.
//
// Precondition: trackGaps must be non-zero.
func (d *syntheticTimerdynamicGap) Get() uint64 {
	return d[:][0]
}
...
//
// Precondition: trackGaps must be non-zero.
func (d *syntheticTimerdynamicGap) Set(v uint64) {
	d[:][0] = v
}
...
const (
	...
	syntheticTimerminDegree = 3

	syntheticTimermaxDegree = 2 * syntheticTimerminDegree
)
...
//
// +stateify savable
type syntheticTimerSet struct {
	root syntheticTimernode `state:".([]syntheticTimerFlatSegment)"`
}

// IsEmpty returns true if the set contains no segments.
func (s *syntheticTimerSet) IsEmpty() bool {
...
// more efficient.
func (s *syntheticTimerSet) IsEmptyRange(r uint64Range) bool {
	switch {
	case r.Length() < 0:
...
func (s *syntheticTimerSet) Span() uint64 {
	var sz uint64
...
func (s *syntheticTimerSet) SpanRange(r uint64Range) uint64 {
	switch {
	case r.Length() < 0:
		panic(fmt.Sprintf("invalid range %v", r))
	case r.Length() == 0:
...
func (s *syntheticTimerSet) FirstSegment() syntheticTimerIterator {
	if s.root.nrSegments == 0 {
		return syntheticTimerIterator{}
	}
...
func (s *syntheticTimerSet) LastSegment() syntheticTimerIterator {
	if s.root.nrSegments == 0 {
		return syntheticTimerIterator{}
	}
...
func (s *syntheticTimerSet) FirstGap() syntheticTimerGapIterator {
	n := &s.root
	for n.hasChildren {
		n = n.children[0]
	}
	return syntheticTimerGapIterator{n, 0}
}

// LastGap returns the last gap in the set.
func (s *syntheticTimerSet) LastGap() syntheticTimerGapIterator {
	n := &s.root
	for n.hasChildren {
		n = n.children[n.nrSegments]
	}
	return syntheticTimerGapIterator{n, n.nrSegments}
}
...
func (s *syntheticTimerSet) Find(key uint64) (syntheticTimerIterator, syntheticTimerGapIterator) {
	n := &s.root
	for {
		...
		for lower < upper {
			...
			if r := n.keys[i]; key < r.End {
				if key >= r.Start {
					return syntheticTimerIterator{n, i}, syntheticTimerGapIterator{}
				}
				upper = i
			} else {
				lower = i + 1
		...
		i := lower
		if !n.hasChildren {
			return syntheticTimerIterator{}, syntheticTimerGapIterator{n, i}
		}
		n = n.children[i]
	}
}
...
func (s *syntheticTimerSet) FindSegment(key uint64) syntheticTimerIterator {
	seg, _ := s.Find(key)
	return seg
}
...
func (s *syntheticTimerSet) LowerBoundSegment(min uint64) syntheticTimerIterator {
	seg, gap := s.Find(min)
	if seg.Ok() {
		return seg
	}
...
func (s *syntheticTimerSet) UpperBoundSegment(max uint64) syntheticTimerIterator {
	seg, gap := s.Find(max)
	if seg.Ok() {
		return seg
	}
...
// terminal iterator.
func (s *syntheticTimerSet) FindGap(key uint64) syntheticTimerGapIterator {
	_, gap := s.Find(key)
	return gap
}
...
// equal to min.
func (s *syntheticTimerSet) LowerBoundGap(min uint64) syntheticTimerGapIterator {
	seg, gap := s.Find(min)
	if gap.Ok() {
		return gap
	}
...
// equal to max.
func (s *syntheticTimerSet) UpperBoundGap(max uint64) syntheticTimerGapIterator {
	seg, gap := s.Find(max)
	if gap.Ok() {
		return gap
	}
...
func (s *syntheticTimerSet) FirstLargeEnoughGap(minSize uint64) syntheticTimerGapIterator {
	if syntheticTimertrackGaps != 1 {
...
//
// Precondition: trackGaps must be 1.
func (s *syntheticTimerSet) LastLargeEnoughGap(minSize uint64) syntheticTimerGapIterator {
	if syntheticTimertrackGaps != 1 {
		panic("set is not tracking gaps")
	}
	gap := s.LastGap()
...
//
// Precondition: trackGaps must be 1.
func (s *syntheticTimerSet) LowerBoundLargeEnoughGap(min, minSize uint64) syntheticTimerGapIterator {
	if syntheticTimertrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
//
// Precondition: trackGaps must be 1.
func (s *syntheticTimerSet) UpperBoundLargeEnoughGap(max, minSize uint64) syntheticTimerGapIterator {
	if syntheticTimertrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (s *syntheticTimerSet) Insert(gap syntheticTimerGapIterator, r uint64Range, val syntheticTimerQueue) syntheticTimerIterator {
	if r.Length() <= 0 {
	...
	if prev.Ok() && prev.End() == r.Start {
		if mval, ok := (syntheticTimerSetFunctions{}).Merge(prev.Range(), prev.Value(), r, val); ok {
			shrinkMaxGap := syntheticTimertrackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Get()
			...
			if next.Ok() && next.Start() == r.End {
				val = mval
				if mval, ok := (syntheticTimerSetFunctions{}).Merge(prev.Range(), val, next.Range(), next.Value()); ok {
	...
	if next.Ok() && next.Start() == r.End {
		if mval, ok := (syntheticTimerSetFunctions{}).Merge(r, val, next.Range(), next.Value()); ok {
			shrinkMaxGap := syntheticTimertrackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Get()
...
func (s *syntheticTimerSet) InsertWithoutMerging(gap syntheticTimerGapIterator, r uint64Range, val syntheticTimerQueue) syntheticTimerIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
//   - r.Start >= gap.Start().
//   - r.End <= gap.End().
func (s *syntheticTimerSet) InsertWithoutMergingUnchecked(gap syntheticTimerGapIterator, r uint64Range, val syntheticTimerQueue) syntheticTimerIterator {
	gap = gap.node.rebalanceBeforeInsert(gap)
	splitMaxGap := syntheticTimertrackGaps != 0 && (gap.node.nrSegments == 0 || gap.Range().Length() == gap.node.maxGap.Get())
	...
	return syntheticTimerIterator{gap.node, gap.index}
}
...
func (s *syntheticTimerSet) InsertRange(r uint64Range, val syntheticTimerQueue) syntheticTimerIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (s *syntheticTimerSet) InsertWithoutMergingRange(r uint64Range, val syntheticTimerQueue) syntheticTimerIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
// additional work between finding the gap and insertion, use Insert instead.
func (s *syntheticTimerSet) TryInsertRange(r uint64Range, val syntheticTimerQueue) syntheticTimerIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *syntheticTimerSet) TryInsertWithoutMergingRange(r uint64Range, val syntheticTimerQueue) syntheticTimerIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *syntheticTimerSet) Remove(seg syntheticTimerIterator) syntheticTimerGapIterator {
	...
	syntheticTimerSetFunctions{}.ClearValue(&seg.node.values[seg.node.nrSegments-1])
	seg.node.nrSegments--
	if syntheticTimertrackGaps != 0 {
		seg.node.updateMaxGapLeaf()
	}
	return seg.node.rebalanceAfterRemove(syntheticTimerGapIterator{seg.node, seg.index})
}

// RemoveAll removes all segments from the set. All existing iterators are
// invalidated.
func (s *syntheticTimerSet) RemoveAll() {
	s.root = syntheticTimernode{}
}
...
func (s *syntheticTimerSet) RemoveRange(r uint64Range) syntheticTimerGapIterator {
...
func (s *syntheticTimerSet) RemoveFullRange(r uint64Range) syntheticTimerGapIterator {
...
func (s *syntheticTimerSet) RemoveRangeWith(r uint64Range, f func(seg syntheticTimerIterator)) syntheticTimerGapIterator {
	seg, gap := s.Find(r.Start)
	if seg.Ok() {
		seg = s.Isolate(seg, r)
		if f != nil {
...
// panics.
func (s *syntheticTimerSet) RemoveFullRangeWith(r uint64Range, f func(seg syntheticTimerIterator)) syntheticTimerGapIterator {
...
func (s *syntheticTimerSet) MoveFrom(s2 *syntheticTimerSet) {
	*s = *s2
...
func (s *syntheticTimerSet) Merge(first, second syntheticTimerIterator) syntheticTimerIterator {
...
func (s *syntheticTimerSet) MergeUnchecked(first, second syntheticTimerIterator) syntheticTimerIterator {
	if first.End() == second.Start() {
		if mval, ok := (syntheticTimerSetFunctions{}).Merge(first.Range(), first.Value(), second.Range(), second.Value()); ok {
...
// skipped.
func (s *syntheticTimerSet) MergePrev(seg syntheticTimerIterator) syntheticTimerIterator {
...
// skipped.
func (s *syntheticTimerSet) MergeNext(seg syntheticTimerIterator) syntheticTimerIterator {
...
func (s *syntheticTimerSet) Unisolate(seg syntheticTimerIterator) syntheticTimerIterator {
	if prev := seg.PrevSegment(); prev.Ok() {
		if mseg := s.MergeUnchecked(prev, seg); mseg.Ok() {
			seg = mseg
		}
...
func (s *syntheticTimerSet) MergeAll() {
	seg := s.FirstSegment()
	if !seg.Ok() {
		return
	}
...
func (s *syntheticTimerSet) MergeInsideRange(r uint64Range) {
...
func (s *syntheticTimerSet) MergeOutsideRange(r uint64Range) {
...
// Split panics.
func (s *syntheticTimerSet) Split(seg syntheticTimerIterator, split uint64) (syntheticTimerIterator, syntheticTimerIterator) {
...
func (s *syntheticTimerSet) SplitUnchecked(seg syntheticTimerIterator, split uint64) (syntheticTimerIterator, syntheticTimerIterator) {
	val1, val2 := (syntheticTimerSetFunctions{}).Split(seg.Range(), seg.Value(), split)
	end2 := seg.End()
	seg.SetEndUnchecked(split)
	seg.SetValue(val1)
	seg2 := s.InsertWithoutMergingUnchecked(seg.NextGap(), uint64Range{split, end2}, val2)
...
func (s *syntheticTimerSet) SplitBefore(seg syntheticTimerIterator, start uint64) syntheticTimerIterator {
...
func (s *syntheticTimerSet) SplitAfter(seg syntheticTimerIterator, end uint64) syntheticTimerIterator {
...
func (s *syntheticTimerSet) Isolate(seg syntheticTimerIterator, r uint64Range) syntheticTimerIterator {
...
func (s *syntheticTimerSet) LowerBoundSegmentSplitBefore(min uint64) syntheticTimerIterator {
	seg, gap := s.Find(min)
	if seg.Ok() {
...
// mutated, suitable as the initial value for a loop variable.
func (s *syntheticTimerSet) UpperBoundSegmentSplitAfter(max uint64) syntheticTimerIterator {
	seg, gap := s.Find(max)
	if seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *syntheticTimerSet) VisitRange(r uint64Range, f func(seg syntheticTimerIterator) bool) {
	for seg := s.LowerBoundSegment(r.Start); seg.Ok() && seg.Start() < r.End; seg = seg.NextSegment() {
		if !f(seg) {
			return
		}
...
func (s *syntheticTimerSet) VisitFullRange(r uint64Range, f func(seg syntheticTimerIterator) bool) {
	pos := r.Start
	seg := s.FindSegment(r.Start)
	for {
		if !seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *syntheticTimerSet) MutateRange(r uint64Range, f func(seg syntheticTimerIterator) bool) {
...
func (s *syntheticTimerSet) MutateFullRange(r uint64Range, f func(seg syntheticTimerIterator) bool) {
	seg := s.FindSegment(r.Start)
	if !seg.Ok() {
		panic(fmt.Sprintf("missing segment at %v", r.Start))
	}
...
// +stateify savable
type syntheticTimernode struct {
	...
	maxGap syntheticTimerdynamicGap

	// Nodes store keys and values in separate arrays to maximize locality in
	// the common case (scanning keys for lookup).
	keys     [syntheticTimermaxDegree - 1]uint64Range
	values   [syntheticTimermaxDegree - 1]syntheticTimerQueue
	children [syntheticTimermaxDegree]*syntheticTimernode
}

// firstSegment returns the first segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *syntheticTimernode) firstSegment() syntheticTimerIterator {
	for n.hasChildren {
		n = n.children[0]
	}
	return syntheticTimerIterator{n, 0}
}

// lastSegment returns the last segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *syntheticTimernode) lastSegment() syntheticTimerIterator {
	for n.hasChildren {
		n = n.children[n.nrSegments]
	}
	return syntheticTimerIterator{n, n.nrSegments - 1}
}

func (n *syntheticTimernode) prevSibling() *syntheticTimernode {
...
func (n *syntheticTimernode) nextSibling() *syntheticTimernode {
...
// represented by gap.
func (n *syntheticTimernode) rebalanceBeforeInsert(gap syntheticTimerGapIterator) syntheticTimerGapIterator {
	if n.nrSegments < syntheticTimermaxDegree-1 {
		return gap
	}
	if n.parent != nil {
	...
	if n.parent == nil {
		...
		left := &syntheticTimernode{
			nrSegments:  syntheticTimerminDegree - 1,
			parent:      n,
			parentIndex: 0,
			hasChildren: n.hasChildren,
		}
		right := &syntheticTimernode{
			nrSegments:  syntheticTimerminDegree - 1,
			parent:      n,
			parentIndex: 1,
			hasChildren: n.hasChildren,
		}
		copy(left.keys[:syntheticTimerminDegree-1], n.keys[:syntheticTimerminDegree-1])
		copy(left.values[:syntheticTimerminDegree-1], n.values[:syntheticTimerminDegree-1])
		copy(right.keys[:syntheticTimerminDegree-1], n.keys[syntheticTimerminDegree:])
		copy(right.values[:syntheticTimerminDegree-1], n.values[syntheticTimerminDegree:])
		n.keys[0], n.values[0] = n.keys[syntheticTimerminDegree-1], n.values[syntheticTimerminDegree-1]
		syntheticTimerzeroValueSlice(n.values[1:])
		if n.hasChildren {
			copy(left.children[:syntheticTimerminDegree], n.children[:syntheticTimerminDegree])
			copy(right.children[:syntheticTimerminDegree], n.children[syntheticTimerminDegree:])
			syntheticTimerzeroNodeSlice(n.children[2:])
			for i := 0; i < syntheticTimerminDegree; i++ {
		...
		if gap.index < syntheticTimerminDegree {
			return syntheticTimerGapIterator{left, gap.index}
		}
		return syntheticTimerGapIterator{right, gap.index - syntheticTimerminDegree}
	}
	...
	n.parent.keys[n.parentIndex], n.parent.values[n.parentIndex] = n.keys[syntheticTimerminDegree-1], n.values[syntheticTimerminDegree-1]
	...
	n.parent.nrSegments++
	copy(sibling.keys[:syntheticTimerminDegree-1], n.keys[syntheticTimerminDegree:])
	copy(sibling.values[:syntheticTimerminDegree-1], n.values[syntheticTimerminDegree:])
	syntheticTimerzeroValueSlice(n.values[syntheticTimerminDegree-1:])
	if n.hasChildren {
		copy(sibling.children[:syntheticTimerminDegree], n.children[syntheticTimerminDegree:])
		syntheticTimerzeroNodeSlice(n.children[syntheticTimerminDegree:])
		for i := 0; i < syntheticTimerminDegree; i++ {
			sibling.children[i].parent = sibling
			sibling.children[i].parentIndex = i
		}
	}
	n.nrSegments = syntheticTimerminDegree - 1
	...
	return syntheticTimerGapIterator{sibling, gap.index - syntheticTimerminDegree}
}
...
// B-tree invariant.
func (n *syntheticTimernode) rebalanceAfterRemove(gap syntheticTimerGapIterator) syntheticTimerGapIterator {
	for {
		if n.nrSegments >= syntheticTimerminDegree-1 {
			return gap
		}
		if n.parent == nil {
		...
		if sibling := n.prevSibling(); sibling != nil && sibling.nrSegments >= syntheticTimerminDegree {
			...
			syntheticTimerSetFunctions{}.ClearValue(&sibling.values[sibling.nrSegments-1])
			if n.hasChildren {
			...
			if gap.node == n {
				return syntheticTimerGapIterator{n, gap.index + 1}
			}
			return gap
		}
		if sibling := n.nextSibling(); sibling != nil && sibling.nrSegments >= syntheticTimerminDegree {
			...
			if gap.node == sibling {
				...
				return syntheticTimerGapIterator{sibling, gap.index - 1}
			}
			return gap
		}
		...
		if p.nrSegments == 1 {
			...
			if gap.node == left {
				return syntheticTimerGapIterator{p, gap.index}
			}
			if gap.node == right {
				return syntheticTimerGapIterator{p, gap.index + left.nrSegments + 1}
			}
			return gap
		}
		...
		var left, right *syntheticTimernode
		if n.parentIndex > 0 {
			left = n.prevSibling()
			right = n
		} else {
		...
		if gap.node == right {
			gap = syntheticTimerGapIterator{left, gap.index + left.nrSegments + 1}
		}
		...
		syntheticTimerSetFunctions{}.ClearValue(&p.values[p.nrSegments-1])
...
func (n *syntheticTimernode) updateMaxGapLeaf() {
	if n.hasChildren {
...
//
// Precondition: trackGaps must be 1.
func (n *syntheticTimernode) updateMaxGapLocal() {
	if !n.hasChildren {
...
func (n *syntheticTimernode) calculateMaxGapLeaf() uint64 {
	max := syntheticTimerGapIterator{n, 0}.Range().Length()
	for i := 1; i <= n.nrSegments; i++ {
		if current := (syntheticTimerGapIterator{n, i}).Range().Length(); current > max {
			max = current
		}
	}
	return max
...
func (n *syntheticTimernode) calculateMaxGapInternal() uint64 {
...
func (n *syntheticTimernode) searchFirstLargeEnoughGap(minSize uint64) syntheticTimerGapIterator {
	if n.maxGap.Get() < minSize {
		return syntheticTimerGapIterator{}
	}
	if n.hasChildren {
		...
		for i := 0; i <= n.nrSegments; i++ {
			currentGap := syntheticTimerGapIterator{n, i}
...
// in the subtree rooted by n. If not found, return a terminal gap iterator.
func (n *syntheticTimernode) searchLastLargeEnoughGap(minSize uint64) syntheticTimerGapIterator {
	if n.maxGap.Get() < minSize {
		return syntheticTimerGapIterator{}
	}
	if n.hasChildren {
...
func (seg syntheticTimerIterator) Ok() bool {
	return seg.node != nil
}

// Range returns the iterated segment's range key.
func (seg syntheticTimerIterator) Range() uint64Range {
...
func (seg syntheticTimerIterator) SetRangeUnchecked(r uint64Range) {
...
// iterators.
func (seg syntheticTimerIterator) SetRange(r uint64Range) {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (seg syntheticTimerIterator) SetStartUnchecked(start uint64) {
...
// iterators.
func (seg syntheticTimerIterator) SetStart(start uint64) {
	if start >= seg.End() {
...
func (seg syntheticTimerIterator) SetEndUnchecked(end uint64) {
...
// iterators.
func (seg syntheticTimerIterator) SetEnd(end uint64) {
	if end <= seg.Start() {
...
func (seg syntheticTimerIterator) Value() syntheticTimerQueue {
...
// invalidate any iterators.
func (seg syntheticTimerIterator) ValuePtr() *syntheticTimerQueue {
	return &seg.node.values[seg.index]
}

// SetValue mutates the iterated segment's value. This operation does not
// invalidate any iterators.
func (seg syntheticTimerIterator) SetValue(val syntheticTimerQueue) {
...
func (seg syntheticTimerIterator) PrevSegment() syntheticTimerIterator {
	if seg.node.hasChildren {
		return seg.node.children[seg.index].lastSegment()
	}
	if seg.index > 0 {
		return syntheticTimerIterator{seg.node, seg.index - 1}
	}
	if seg.node.parent == nil {
		return syntheticTimerIterator{}
	}
	return syntheticTimersegmentBeforePosition(seg.node.parent, seg.node.parentIndex)
}
...
func (seg syntheticTimerIterator) NextSegment() syntheticTimerIterator {
	if seg.node.hasChildren {
	...
	return syntheticTimersegmentAfterPosition(seg.node.parent, seg.node.parentIndex)
}

// PrevGap returns the gap immediately before the iterated segment.
func (seg syntheticTimerIterator) PrevGap() syntheticTimerGapIterator {
	if seg.node.hasChildren {

		return seg.node.children[seg.index].lastSegment().NextGap()
	}
	return syntheticTimerGapIterator{seg.node, seg.index}
}

// NextGap returns the gap immediately after the iterated segment.
func (seg syntheticTimerIterator) NextGap() syntheticTimerGapIterator {
	if seg.node.hasChildren {
		return seg.node.children[seg.index+1].firstSegment().PrevGap()
	}
	return syntheticTimerGapIterator{seg.node, seg.index + 1}
}
...
// non-terminal.
func (seg syntheticTimerIterator) PrevNonEmpty() (syntheticTimerIterator, syntheticTimerGapIterator) {
	if prev := seg.PrevSegment(); prev.Ok() && prev.End() == seg.Start() {
		return prev, syntheticTimerGapIterator{}
	}
	return syntheticTimerIterator{}, seg.PrevGap()
}
...
// non-terminal.
func (seg syntheticTimerIterator) NextNonEmpty() (syntheticTimerIterator, syntheticTimerGapIterator) {
	if next := seg.NextSegment(); next.Ok() && next.Start() == seg.End() {
		return next, syntheticTimerGapIterator{}
	}
	return syntheticTimerIterator{}, seg.NextGap()
}
...
// Unless otherwise specified, any mutation of a set invalidates all existing
// iterators into the set.
type syntheticTimerGapIterator struct {
...
// Ok returns true if the iterator is not terminal. All other methods are only
// valid for non-terminal iterators.
func (gap syntheticTimerGapIterator) Ok() bool {
	return gap.node != nil
}

// Range returns the range spanned by the iterated gap.
func (gap syntheticTimerGapIterator) Range() uint64Range {
	return uint64Range{gap.Start(), gap.End()}
}

// Start is equivalent to Range().Start, but should be preferred if only the
// start of the range is needed.
func (gap syntheticTimerGapIterator) Start() uint64 {
	if ps := gap.PrevSegment(); ps.Ok() {
		return ps.End()
	}
	return syntheticTimerSetFunctions{}.MinKey()
}

// End is equivalent to Range().End, but should be preferred if only the end of
// the range is needed.
func (gap syntheticTimerGapIterator) End() uint64 {
	if ns := gap.NextSegment(); ns.Ok() {
		return ns.Start()
	}
	return syntheticTimerSetFunctions{}.MaxKey()
}

// IsEmpty returns true if the iterated gap is empty (that is, the "gap" is
// between two adjacent segments.)
func (gap syntheticTimerGapIterator) IsEmpty() bool {
...
func (gap syntheticTimerGapIterator) PrevSegment() syntheticTimerIterator {
	return syntheticTimersegmentBeforePosition(gap.node, gap.index)
}
...
func (gap syntheticTimerGapIterator) NextSegment() syntheticTimerIterator {
	return syntheticTimersegmentAfterPosition(gap.node, gap.index)
}

// PrevGap returns the iterated gap's predecessor. If no such gap exists,
// PrevGap returns a terminal iterator.
func (gap syntheticTimerGapIterator) PrevGap() syntheticTimerGapIterator {
	seg := gap.PrevSegment()
	if !seg.Ok() {
		return syntheticTimerGapIterator{}
	}
...
// returns a terminal iterator.
func (gap syntheticTimerGapIterator) NextGap() syntheticTimerGapIterator {
	seg := gap.NextSegment()
	if !seg.Ok() {
		return syntheticTimerGapIterator{}
	}
...
//
// Precondition: trackGaps must be 1.
func (gap syntheticTimerGapIterator) NextLargeEnoughGap(minSize uint64) syntheticTimerGapIterator {
	if syntheticTimertrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (gap syntheticTimerGapIterator) nextLargeEnoughGapHelper(minSize uint64) syntheticTimerGapIterator {
	for {
...
//
// Precondition: trackGaps must be 1.
func (gap syntheticTimerGapIterator) PrevLargeEnoughGap(minSize uint64) syntheticTimerGapIterator {
	if syntheticTimertrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (gap syntheticTimerGapIterator) prevLargeEnoughGapHelper(minSize uint64) syntheticTimerGapIterator {
	for {
...
func syntheticTimersegmentBeforePosition(n *syntheticTimernode, i int) syntheticTimerIterator {
	for i == 0 {
		if n.parent == nil {
			return syntheticTimerIterator{}
		}
	...
	return syntheticTimerIterator{n, i - 1}
}
...
func syntheticTimersegmentAfterPosition(n *syntheticTimernode, i int) syntheticTimerIterator {
	for i == n.nrSegments {
		if n.parent == nil {
			return syntheticTimerIterator{}
		}
	...
	return syntheticTimerIterator{n, i}
}

func syntheticTimerzeroValueSlice(slice []syntheticTimerQueue) {

	for i := range slice {
		syntheticTimerSetFunctions{}.ClearValue(&slice[i])
	}
}

func syntheticTimerzeroNodeSlice(slice []*syntheticTimernode) {
	for i := range slice {
		slice[i] = nil
	}
}
...
func (s *syntheticTimerSet) String() string {
	return s.root.String()
}

// String stringifies a node (and all of its children) for debugging.
func (n *syntheticTimernode) String() string {
	var buf bytes.Buffer
	n.writeDebugString(&buf, "")
	return buf.String()
}

func (n *syntheticTimernode) writeDebugString(buf *bytes.Buffer, prefix string) {
...
//
// +stateify savable
type syntheticTimerFlatSegment struct {
	Start uint64
	End   uint64
...
// key order.
func (s *syntheticTimerSet) ExportSlice() []syntheticTimerFlatSegment {
	var fs []syntheticTimerFlatSegment
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		fs = append(fs, syntheticTimerFlatSegment{
			Start: seg.Start(),
			End:   seg.End(),
			Value: seg.Value(),
		})
...
func (s *syntheticTimerSet) ImportSlice(fs []syntheticTimerFlatSegment) error {
	if !s.IsEmpty() {
...
func (s *syntheticTimerSet) segmentTestCheck(expectedSegments int, segFunc func(int, uint64Range, syntheticTimerQueue) error) error {
	havePrev := false
	prev := uint64(0)
	nrSegments := 0
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
...
func (s *syntheticTimerSet) countSegments() (segments int) {
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		segments++
	}
	return segments
}
func (s *syntheticTimerSet) saveRoot() []syntheticTimerFlatSegment {
	fs := s.ExportSlice()
...
func (s *syntheticTimerSet) loadRoot(_ context.Context, fs []syntheticTimerFlatSegment) {
