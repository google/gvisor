...
func MoveMount(t *kernel.Task, sysno uintptr, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	// resolved: a path that cannot be looked up is reported as such. The
	// destination is passed ahead of the source because that is the order in
	// which Linux resolves them.
	//
	// Matches Linux [fs/namespace.c]:vfs_move_mount(), which calls
	// security_move_mount() after SYSCALL_DEFINE5(move_mount, ...) has resolved
	// the destination and then the source, and before do_move_mount() checks
