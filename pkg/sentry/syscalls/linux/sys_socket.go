...
func writeAddress(t *kernel.Task, addr linux.SockAddr, addrLen uint32, addrPtr usermem.Addr, addrLenPtr usermem.Addr) error {
	...
	addrLenP := primitive.Uint32(addrLen)
	if err := addrLenP.CopyOut(t, addrLenPtr); err != nil {
		return err
	}
...
func SocketPair(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	if err := primitive.CopyInt32SliceOut(t, socks, fds); err != nil {
...
func GetSockOpt(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	vLen := primitive.Int32((binary.Size(v)))
	if err := vLen.CopyOut(t, optLenAddr); err != nil {
		return 0, nil, err
	}
...
func RecvMMsg(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	for i := uint64(0); i < uint64(vlen); i++ {
		...
		nP := primitive.Uint32(n)
		if err = nP.CopyOut(t, lp); err != nil {
			break
		}
		count++
	}
...
func recvSingleMsg(t *kernel.Task, s socket.Socket, msgPtr usermem.Addr, flags int32, haveDeadline bool, deadline ktime.Time) (uintptr, error) {
	...
	if err := msg.CopyIn(t, msgPtr); err != nil {
		return 0, err
	}
	...
	if msg.ControlLen == 0 && msg.NameLen == 0 {
		...
		if int(msg.Flags) != mflags {
			...
			mflagsP := primitive.Uint32(mflags)
			if err := mflagsP.CopyOut(t, msgPtr+flagsOffset); err != nil {
				return 0, err
			}
		}
	...
	controlDataLen := primitive.Uint64(len(controlData))
	if err := controlDataLen.CopyOut(t, msgPtr+controlLenOffset); err != nil {
		return 0, err
	}
	...
	// Copy out the flags to the caller.
	mflagsP := primitive.Int32(mflags)
	if err := mflagsP.CopyOut(t, msgPtr+flagsOffset); err != nil {
		return 0, err
	}
