...
type reclaimSet struct {
	root reclaimnode `state:".([]reclaimFlatSegment)"`
}
...
func (s *reclaimSet) FirstLargeEnoughGap(minSize uint64) reclaimGapIterator {
...
//
// Precondition: trackGaps must be 1.
func (s *reclaimSet) LastLargeEnoughGap(minSize uint64) reclaimGapIterator {
	if reclaimtrackGaps != 1 {
		panic("set is not tracking gaps")
	}
	gap := s.LastGap()
...
//
// Precondition: trackGaps must be 1.
func (s *reclaimSet) LowerBoundLargeEnoughGap(min, minSize uint64) reclaimGapIterator {
	if reclaimtrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
//
// Precondition: trackGaps must be 1.
func (s *reclaimSet) UpperBoundLargeEnoughGap(max, minSize uint64) reclaimGapIterator {
	if reclaimtrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (s *reclaimSet) InsertRange(r __generics_imported0.FileRange, val reclaimSetValue) reclaimIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (s *reclaimSet) InsertWithoutMergingRange(r __generics_imported0.FileRange, val reclaimSetValue) reclaimIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
// additional work between finding the gap and insertion, use Insert instead.
func (s *reclaimSet) TryInsertRange(r __generics_imported0.FileRange, val reclaimSetValue) reclaimIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *reclaimSet) TryInsertWithoutMergingRange(r __generics_imported0.FileRange, val reclaimSetValue) reclaimIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *reclaimSet) RemoveFullRange(r __generics_imported0.FileRange) reclaimGapIterator {
...
// skipped.
func (s *reclaimSet) MergePrev(seg reclaimIterator) reclaimIterator {
...
// skipped.
func (s *reclaimSet) MergeNext(seg reclaimIterator) reclaimIterator {
...
func (s *reclaimSet) Unisolate(seg reclaimIterator) reclaimIterator {
	if prev := seg.PrevSegment(); prev.Ok() {
		if mseg := s.MergeUnchecked(prev, seg); mseg.Ok() {
			seg = mseg
		}
...
func (s *reclaimSet) MergeInsideRange(r __generics_imported0.FileRange) {
...
func (s *reclaimSet) MergeOutsideRange(r __generics_imported0.FileRange) {
...
func (s *reclaimSet) SplitBefore(seg reclaimIterator, start uint64) reclaimIterator {
...
func (s *reclaimSet) SplitAfter(seg reclaimIterator, end uint64) reclaimIterator {
...
func (s *reclaimSet) LowerBoundSegmentSplitBefore(min uint64) reclaimIterator {
...
// mutated, suitable as the initial value for a loop variable.
func (s *reclaimSet) UpperBoundSegmentSplitAfter(max uint64) reclaimIterator {
...
func (s *reclaimSet) VisitRange(r __generics_imported0.FileRange, f func(seg reclaimIterator) bool) {
	for seg := s.LowerBoundSegment(r.Start); seg.Ok() && seg.Start() < r.End; seg = seg.NextSegment() {
		if !f(seg) {
			return
		}
...
func (s *reclaimSet) VisitFullRange(r __generics_imported0.FileRange, f func(seg reclaimIterator) bool) {
	pos := r.Start
	seg := s.FindSegment(r.Start)
	for {
		if !seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *reclaimSet) MutateRange(r __generics_imported0.FileRange, f func(seg reclaimIterator) bool) {
...
func (s *reclaimSet) MutateFullRange(r __generics_imported0.FileRange, f func(seg reclaimIterator) bool) {
	seg := s.FindSegment(r.Start)
	if !seg.Ok() {
		panic(fmt.Sprintf("missing segment at %v", r.Start))
	}
...
func (seg reclaimIterator) PrevNonEmpty() (reclaimIterator, reclaimGapIterator) {
	if prev := seg.PrevSegment(); prev.Ok() && prev.End() == seg.Start() {
		return prev, reclaimGapIterator{}
	}
	return reclaimIterator{}, seg.PrevGap()
}
...
func (seg reclaimIterator) NextNonEmpty() (reclaimIterator, reclaimGapIterator) {
	if next := seg.NextSegment(); next.Ok() && next.Start() == seg.End() {
		return next, reclaimGapIterator{}
	}
	return reclaimIterator{}, seg.NextGap()
}
...
//
// +stateify savable
type reclaimFlatSegment struct {
	Start uint64
	End   uint64
...
// key order.
func (s *reclaimSet) ExportSlice() []reclaimFlatSegment {
	var fs []reclaimFlatSegment
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		fs = append(fs, reclaimFlatSegment{
			Start: seg.Start(),
			End:   seg.End(),
			Value: seg.Value(),
		})
...
func (s *reclaimSet) ImportSlice(fs []reclaimFlatSegment) error {
	if !s.IsEmpty() {
...
func (s *reclaimSet) saveRoot() []reclaimFlatSegment {
	fs := s.ExportSlice()
...
func (s *reclaimSet) loadRoot(_ context.Context, fs []reclaimFlatSegment) {
