...
func (f *MemoryFile) LoadFrom(ctx context.Context, r io.Reader, opts *LoadOpts) error {
	...
	if opts.PagesFile != nil {
		...
		defer func() {
			...
			if amfl.unloaded.IsEmpty() {
				// The async page loader goroutine won't log the "async page
				// loading completed" message for this MemoryFile, so handle
				// that here.
				amfl.minUnloaded.Store(math.MaxUint64)
				amfl.timeline.End()
				amfl.pf.amfls.Remove(amfl)
				log.Infof("MemoryFile(%p): async page loading completed before LoadFrom returned (%d bytes)", f, bytesLoaded)
			}
		}()
	}
	...
	durPages := time.Since(timePagesStart)
	if amfl != nil {
		log.Infof("MemoryFile(%p): loaded page file offsets in %s; async loading %d bytes", f, durPages, bytesLoaded)
	} else {
		log.Infof("MemoryFile(%p): loaded pages in %s (%d bytes, %.3f MB/s)", f, durPages, bytesLoaded, float64(bytesLoaded)*1e-6/durPages.Seconds())
	}
...
func (amfl *asyncMemoryFileLoad) awaitLoad(fr memmap.FileRange) error {
	...
	if logAwaitedLoads {
		log.Infof("MemoryFile(%p): awaitLoad goid %d waited %v: %v (%d bytes)", amfl.f, goid.Get(), time.Since(startWaitTime), fr, fr.Length())
	}
	return apfl.err()
}
	...
	for {
		...
		if apfl.qavail == aplQueueCapacity {
			...
			if ev&aplLFDone != 0 {
				...
				apfl.mu.Lock()
				log.Infof("Async page loading completed in %s (%d bytes, %.3f MB/s); %d waiters %v~%v for %d bytes", durTotal.Round(time.Millisecond), apfl.bytesLoaded, float64(apfl.bytesLoaded)*1e-6/durTotal.Seconds(), apfl.totalWaiters, apfl.durWaitedOne.Round(time.Millisecond), apfl.durWaitedTotal.Round(time.Millisecond), apfl.bytesWaited)
				apfl.mu.Unlock()
				return
			}
		...
		for _, c := range completions {
			...
			if amfl.lfDone {
				if amfl.unloaded.IsEmpty() {
					...
					now := gohacks.Nanotime()
					durAMFL := time.Duration(now - timeStartAMFL)
					// Note that the duration, total bytes, and throughput
					// logged here may not correspond exactly to this
					// MemoryFile, since awaited loads from any MemoryFile are
					// prioritized over unawaited loads from any MemoryFile.
					// Total bytes can also vary since loading will be skipped
					// for pages that are deallocated before they are loaded.
					log.Infof("MemoryFile(%p): async page loading completed in %s (%d bytes, %f MB/s)", amfl.f, durAMFL.Round(time.Millisecond), bytesAMFL, float64(bytesAMFL)*1e-6/durAMFL.Seconds())
					timeStartAMFL = now
					bytesAMFL = 0
				} else {
