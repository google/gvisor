		...
		go func() {
			...
			for {
				...
				case <-progressTicker.C:
					...
					log.Infof("MemoryFile(%p): async page loading in progress for %s (%d bytes, %f bytes/second); since last update %s ago: %d bytes, %f bytes/second; %d waiters waited %v~%v for %d bytes", f, durTotal, bytesLoaded, float64(bytesLoaded)/durTotal.Seconds(), durDelta, bytesLoadedDelta, float64(bytesLoadedDelta)/durDelta.Seconds(), totalWaiters, durWaitedOne, durWaitedTotal, bytesWaited)
					timeLast = now
	...
	for {
		...
		if g.qavail == q.Cap() {
			...
			if ev&aplLFDone != 0 {
				...
				apl.mu.Lock()
				log.Infof("MemoryFile(%p): async page loading completed in %s (%d bytes, %f bytes/second); %d waiters waited %v~%v for %d bytes", f, durTotal, apl.bytesLoaded, float64(apl.bytesLoaded)/durTotal.Seconds(), apl.totalWaiters, apl.durWaitedOne, apl.durWaitedTotal, apl.bytesWaited)
				apl.mu.Unlock()
				return
			}
