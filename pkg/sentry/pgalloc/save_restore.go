...
func (f *MemoryFile) LoadFrom(ctx context.Context, r io.Reader, opts *LoadOpts) error {
	...
	if opts.PagesFile != nil {
		...
		if opts.PagesFile.ar.NeedRegisterDestinationFD() {
			var dfrs []stateio.ClientFileRangeSetting
				...
				f.forEachChunk(memmap.FileRange{0, fileSize}, func(chunk *chunkInfo, chunkFR memmap.FileRange) bool {
					if chunk.huge {
						if f.opts.AdviseHugepage {
							dfrs = append(dfrs, stateio.ClientFileRangeSetting{
								FileRange: chunkFR,
			...
			var err error
			df, err = opts.PagesFile.ar.RegisterDestinationFD(int32(f.file.Fd()), fileSize, dfrs)
			if err != nil {
...
// file ar. It takes ownership of ar.
...
func (apfl *AsyncPagesFileLoad) enqueueCurOp() {
	...
	if len(op.frs) == 1 && len(op.iovecs) == 1 {
		...
		apfl.ar.StartRead(int(apfl.curOpID), op.off(), op.amfl.df, op.frs[0], sliceFromIovec(op.iovecs[0]))
	} else {
		apfl.ar.StartReadv(int(apfl.curOpID), op.off(), op.total, op.amfl.df, op.frs, op.iovecs)
	}
...
func (apfl *AsyncPagesFileLoad) main() {
	defer func() {
		...
		if err := apfl.ar.Close(); err != nil {
			...
			log.Warningf("Async page loading: stateio.AsyncReader.Close returned error: %v", err)
		}
		apfl.timeline.End()
	...
	for {
		...
		for _, c := range completions {
			...
			if c.Err != nil && c.Err != io.EOF {
				log.Warningf("Async page loading failed: read for MemoryFile(%p) pages %v failed: %v", op.amfl.f, op.frs, c.Err)
				apfl.errVal.Store(linuxerr.EIO)
				apfl.mu.Unlock()
				apfl.amflsMu.Unlock()
				return
			...
			if c.N != op.total {
				log.Warningf("Async page loading failed: read for MemoryFile(%p) pages %v (total %d bytes) returned %d bytes with error %v", op.amfl.f, op.frs, op.total, c.N, c.Err)
				apfl.errVal.Store(linuxerr.EIO)
				apfl.mu.Unlock()
				apfl.amflsMu.Unlock()
				return
