...
type evictableRangeSet struct {
	root evictableRangenode `state:".([]evictableRangeFlatSegment)"`
}
...
func (s *evictableRangeSet) FirstLargeEnoughGap(minSize uint64) evictableRangeGapIterator {
...
//
// Precondition: trackGaps must be 1.
func (s *evictableRangeSet) LastLargeEnoughGap(minSize uint64) evictableRangeGapIterator {
	if evictableRangetrackGaps != 1 {
		panic("set is not tracking gaps")
	}
	gap := s.LastGap()
...
//
// Precondition: trackGaps must be 1.
func (s *evictableRangeSet) LowerBoundLargeEnoughGap(min, minSize uint64) evictableRangeGapIterator {
	if evictableRangetrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
//
// Precondition: trackGaps must be 1.
func (s *evictableRangeSet) UpperBoundLargeEnoughGap(max, minSize uint64) evictableRangeGapIterator {
	if evictableRangetrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (s *evictableRangeSet) InsertRange(r EvictableRange, val evictableRangeSetValue) evictableRangeIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (s *evictableRangeSet) InsertWithoutMergingRange(r EvictableRange, val evictableRangeSetValue) evictableRangeIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
// additional work between finding the gap and insertion, use Insert instead.
func (s *evictableRangeSet) TryInsertRange(r EvictableRange, val evictableRangeSetValue) evictableRangeIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *evictableRangeSet) TryInsertWithoutMergingRange(r EvictableRange, val evictableRangeSetValue) evictableRangeIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *evictableRangeSet) RemoveFullRange(r EvictableRange) evictableRangeGapIterator {
...
func (s *evictableRangeSet) RemoveRangeWith(r EvictableRange, f func(seg evictableRangeIterator)) evictableRangeGapIterator {
	seg, gap := s.Find(r.Start)
	if seg.Ok() {
		seg = s.Isolate(seg, r)
		if f != nil {
...
// panics.
func (s *evictableRangeSet) RemoveFullRangeWith(r EvictableRange, f func(seg evictableRangeIterator)) evictableRangeGapIterator {
...
func (s *evictableRangeSet) MoveFrom(s2 *evictableRangeSet) {
	*s = *s2
...
// skipped.
func (s *evictableRangeSet) MergePrev(seg evictableRangeIterator) evictableRangeIterator {
...
// skipped.
func (s *evictableRangeSet) MergeNext(seg evictableRangeIterator) evictableRangeIterator {
...
func (s *evictableRangeSet) Unisolate(seg evictableRangeIterator) evictableRangeIterator {
	if prev := seg.PrevSegment(); prev.Ok() {
		if mseg := s.MergeUnchecked(prev, seg); mseg.Ok() {
			seg = mseg
		}
...
func (s *evictableRangeSet) MergeInsideRange(r EvictableRange) {
...
func (s *evictableRangeSet) MergeOutsideRange(r EvictableRange) {
...
func (s *evictableRangeSet) SplitBefore(seg evictableRangeIterator, start uint64) evictableRangeIterator {
...
func (s *evictableRangeSet) SplitAfter(seg evictableRangeIterator, end uint64) evictableRangeIterator {
...
func (s *evictableRangeSet) LowerBoundSegmentSplitBefore(min uint64) evictableRangeIterator {
	seg, gap := s.Find(min)
	if seg.Ok() {
...
// mutated, suitable as the initial value for a loop variable.
func (s *evictableRangeSet) UpperBoundSegmentSplitAfter(max uint64) evictableRangeIterator {
	seg, gap := s.Find(max)
	if seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *evictableRangeSet) VisitRange(r EvictableRange, f func(seg evictableRangeIterator) bool) {
	for seg := s.LowerBoundSegment(r.Start); seg.Ok() && seg.Start() < r.End; seg = seg.NextSegment() {
		if !f(seg) {
			return
		}
...
func (s *evictableRangeSet) VisitFullRange(r EvictableRange, f func(seg evictableRangeIterator) bool) {
	pos := r.Start
	seg := s.FindSegment(r.Start)
	for {
		if !seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *evictableRangeSet) MutateRange(r EvictableRange, f func(seg evictableRangeIterator) bool) {
...
func (s *evictableRangeSet) MutateFullRange(r EvictableRange, f func(seg evictableRangeIterator) bool) {
	seg := s.FindSegment(r.Start)
	if !seg.Ok() {
		panic(fmt.Sprintf("missing segment at %v", r.Start))
	}
...
func (seg evictableRangeIterator) PrevNonEmpty() (evictableRangeIterator, evictableRangeGapIterator) {
	if prev := seg.PrevSegment(); prev.Ok() && prev.End() == seg.Start() {
		return prev, evictableRangeGapIterator{}
	}
	return evictableRangeIterator{}, seg.PrevGap()
}
...
func (seg evictableRangeIterator) NextNonEmpty() (evictableRangeIterator, evictableRangeGapIterator) {
	if next := seg.NextSegment(); next.Ok() && next.Start() == seg.End() {
		return next, evictableRangeGapIterator{}
	}
	return evictableRangeIterator{}, seg.NextGap()
}
...
//
// +stateify savable
type evictableRangeFlatSegment struct {
	Start uint64
	End   uint64
...
// key order.
func (s *evictableRangeSet) ExportSlice() []evictableRangeFlatSegment {
	var fs []evictableRangeFlatSegment
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		fs = append(fs, evictableRangeFlatSegment{
			Start: seg.Start(),
			End:   seg.End(),
			Value: seg.Value(),
		})
...
func (s *evictableRangeSet) ImportSlice(fs []evictableRangeFlatSegment) error {
	if !s.IsEmpty() {
	...
	for i := range fs {
		f := &fs[i]
		r := EvictableRange{f.Start, f.End}
...
func (s *evictableRangeSet) saveRoot() []evictableRangeFlatSegment {
	fs := s.ExportSlice()
