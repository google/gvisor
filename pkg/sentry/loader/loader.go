...
func openPath(ctx context.Context, args LoadArgs) (fsbridge.File, error) {
	...
	file, err := args.Opener.OpenPath(ctx, args.Filename, vfs.MayRead|vfs.MayExec, args.RemainingTraversals, args.ResolveFinal)
	if err != nil {
		return nil, err
	}
	defer file.DecRef() // In case of error.

	direntType, err := file.Type(ctx)
	if err != nil {
		return nil, err
	}
	...
	if strings.HasSuffix(args.Filename, "/") {
		if direntType != linux.DT_DIR {
			return nil, syserror.ENOTDIR
		}
	}

	if direntType != linux.DT_REG {
		ctx.Infof("%s is not regular: %v", args.Filename, direntType)
...
func checkFile(ctx context.Context, file fsbridge.File, filename string) error {
	...
	perms := vfs.MayRead | vfs.MayExec
	if err := file.CheckPermission(ctx, perms); err != nil {
		return err
	}
	direntType, err := file.Type(ctx)
	if err != nil {
	...
	if direntType != linux.DT_REG {
		ctx.Infof("%s is not regular: %v", filename, direntType)
...
func loadExecutable(ctx context.Context, args LoadArgs) (loadedELF, arch.Context, fsbridge.File, []string, error) {
	for i := 0; i < maxLoaderAttempts; i++ {
		if args.File == nil {
			...
			if err := checkFile(ctx, args.File, args.Filename); err != nil {
				ctx.Infof("Error opening %s: %v", args.Filename, err)
				return loadedELF{}, nil, nil, nil, err
			}
		}
