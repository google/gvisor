...
func TestProcessTrackerAndPipeEnrichment(t *testing.T) {
	...
	// 1. Simulate an ExecveInfo event for process A ("cat foo").
	...
	eventA := pt.ProcessExecve(execA)
	if eventA.Process == nil {
		t.Fatalf("expected ProcessInfo for cat, got nil")
	}
	if eventA.Process.BinaryPath != "/bin/cat" {
		t.Errorf("got binary %q, want %q", eventA.Process.BinaryPath, "/bin/cat")
	}

	// 2. Simulate an ExecveInfo event for process B ("grep bar") which receives piped input from A.
	...
	eventB := pt.ProcessExecve(execB)
	if eventB.PipeInputSibling == nil {
		t.Fatalf("expected PipeInputSibling to be resolved from tracker, got nil")
	}
	...
	// 3. Simulate TaskExit for process A and verify cleanup.
	pt.ProcessTaskExit(&pb.TaskExit{
		ContextData: &pb.ContextData{
			ThreadGroupId:          100,
			ThreadGroupStartTimeNs: 1000,
		},
	...
	if info := pt.Lookup(keyA); info != nil {
		t.Errorf("expected process A to be unregistered after exit, got %v", info)
	}
}
