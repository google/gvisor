...
type SyscallFlagListener interface {
	// UpdateSecCheck is called when system-call point enablement may have changed.
	//
	// State invokes change notifications with state.registrationMu held, so
	// callbacks must be fast and must not reenter State methods that acquire
	// that mutex. Direct calls during kernel syscall-table initialization run
	// without registrationMu held.
