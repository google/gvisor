...
func (fs *filesystem) InitSend(creds *auth.Credentials, pid uint32) error {
	...
	// The only special case where t for CallAsync() can be nil.
	return fs.conn.CallAsync(nil, req)
}

// InitRecv receives a FUSE_INIT reply and process it.
func (fs *filesystem) InitRecv(res *Response, hasSysAdminCap bool) error {
	...
	return fs.initProcessReply(&out, hasSysAdminCap)
}
...
func (fs *filesystem) initProcessReply(out *linux.FUSEInitOut, hasSysAdminCap bool) error {
