...
type inode struct {
	...
	// entryTime is the time until the entry is valid.
	entryTime time.Time
...
func (i *inode) getAttr(ctx context.Context, fs *vfs.Filesystem, opts vfs.StatOptions, flags uint32, fh uint64) (linux.FUSEAttr, error) {
	...
	i.fs.conn.mu.Unlock()
	i.updateAttrs(out.Attr, time.FromTimespec(linux.Timespec{Sec: int64(out.AttrValid), Nsec: int64(out.AttrValidNsec)}))
	return out.Attr, nil
}
...
// +checklocks:i.attrMu
func (i *inode) updateAttrs(attr linux.FUSEAttr, attrValid time.Time) {
	i.fs.conn.mu.Lock()
	i.attrVersion.Store(i.fs.conn.attributeVersion.Add(1))
	i.fs.conn.mu.Unlock()
	i.attrTime = i.fs.clock.Now().AddTime(attrValid)
