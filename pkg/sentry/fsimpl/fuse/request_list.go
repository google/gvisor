...
//
//go:nosplit
func requestRingInit(e *Request) {
	linker := requestElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func requestRingAdd(old *Request, new *Request) {
	oldLinker := requestElementMapper{}.linkerFor(old)
	newLinker := requestElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func requestRingRemove(e *Request) {
	eLinker := requestElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func requestRingEmpty(e *Request) bool {
	linker := requestElementMapper{}.linkerFor(e)
