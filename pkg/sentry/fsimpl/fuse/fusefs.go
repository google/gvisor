...
// fromFUSEAttr masks attributes from linux.FUSEAttr to linux.Statx. The
// opts.Sync attribute is ignored since the synchronization is handled by the
// FUSE server.
func fromFUSEAttr(attr linux.FUSEAttr, mask, devMinor uint32) linux.Statx {
...
func (i *inode) Lookup(ctx context.Context, name string) (*vfs.Dentry, error) {
	...
	return i.newEntry(task, req, name, 0, true)
}
...
func (i *inode) Stat(ctx context.Context, fs *vfs.Filesystem, opts vfs.StatOptions) (linux.Statx, error) {
	...
	return fromFUSEAttr(out.Attr, opts.Mask, fs.Impl().(*filesystem).devMinor), nil
}
