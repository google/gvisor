...
const connectionenableLogging = false

// obj is used to customize logging. Note that we use a pointer to T so that
// we do not copy the entire object when passed as a format parameter.
var connectionobj *connection
...
// checking.
func (r *connectionRefs) InitRefs() {
...
func (r *connectionRefs) RefType() string {
	return fmt.Sprintf("%T", connectionobj)[1:]
}

// LeakMessage implements refs.CheckedObject.LeakMessage.
func (r *connectionRefs) LeakMessage() string {
	return fmt.Sprintf("[%s %p] reference count of %d instead of 0", r.RefType(), r, r.ReadRefs())
}

// LogRefs implements refs.CheckedObject.LogRefs.
func (r *connectionRefs) LogRefs() bool {
	return connectionenableLogging
}

// ReadRefs returns the current number of references. The returned count is
// inherently racy and is unsafe to use without external synchronization.
func (r *connectionRefs) ReadRefs() int64 {
...
//
//go:nosplit
func (r *connectionRefs) IncRef() {
	v := r.refCount.Add(1)
	if connectionenableLogging {
		refs.LogIncRef(r, v)
	}
	if v <= 1 {
...
//
//go:nosplit
func (r *connectionRefs) TryIncRef() bool {
...
//
//go:nosplit
func (r *connectionRefs) DecRef(destroy func()) {
	v := r.refCount.Add(-1)
	if connectionenableLogging {
		refs.LogDecRef(r, v)
	}
...
func (r *connectionRefs) afterLoad(context.Context) {
	if r.ReadRefs() > 0 {
		refs.Register(r)
	}
}
