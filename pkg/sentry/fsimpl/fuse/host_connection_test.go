...
// The connection is pre-initialized for immediate use.
...
func TestHostConnectionCallAsync(t *testing.T) {
	...
	// Server handles two requests: one async, one sync.
	asyncDone := make(chan struct{})
	go echoServer(t, serverFD, asyncDone)
	...
	go echoServer(t, serverFD, syncDone)
