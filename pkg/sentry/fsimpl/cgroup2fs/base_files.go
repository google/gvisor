...
func (cf *cgroupEvents) Generate(ctx context.Context, buf *bytes.Buffer) error {
	...
	// Reports settled state (frozen(), not freezeOrderedLocked()): 1 iff
	// effectively frozen AND every task that asked to freeze has actually
	// finished doing so, matching Linux's CGRP_FROZEN (which flips once
	// freezing completes, not when cgroup.freeze is written).
	// freezeOrderedLocked() alone -- used internally by commit()/attach()/
	// freeze() -- reports only the requested state.
	frozen := 0
	if cf.c.frozen() {
		frozen = 1
	}
