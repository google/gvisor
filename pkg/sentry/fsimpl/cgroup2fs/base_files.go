...
func (cf *cgroupEvents) Generate(ctx context.Context, buf *bytes.Buffer) error {
	...
	// the events "frozen" line reports genuinely settled state: 1 iff this
	// cgroup or any ancestor is frozen AND every task in this cgroup's own
	// subtree that is currently asked to freeze has actually finished doing
	// so. This matches Linux's CGRP_FROZEN semantics, which only flips once
	// every member task has actually entered the freezer -- not the instant
	// cgroup.freeze is written. settledFrozen(), not IsFrozen(), is used
	// here specifically: IsFrozen() alone reports only the requested state
	// and is reserved for its other two callers (task_start.go's
	// fork-inherit check, cgroup.attach's migration-refresh), which
	// genuinely want "is this task supposed to be frozen right now", not
	// "has the subtree finished settling". See settledFrozen's own comment
	// for why the two conjuncts it combines are both necessary.
	frozen := 0
	if cf.c.settledFrozen() {
		frozen = 1
	}
