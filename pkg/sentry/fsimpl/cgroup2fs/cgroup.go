...
func (c *cgroup) stealController(ctx context.Context, cType kernel.Cgroup2Ctrl) error {
	...
	if cType == kernel.Cgroup2Memory {
		...
		// since the root memory controller has been stolen/detached.
		c.updateSubtreeMemoryCgIDs()
	}
	return nil
}
...
func (c *cgroup) returnController(ctx context.Context, cType kernel.Cgroup2Ctrl) {
	...
	if cType == kernel.Cgroup2Memory {
		...
		// memory controller ID now that it has been returned/attached.
		c.updateSubtreeMemoryCgIDs()
	}
}

// +checklocks:c.fs.treeMu
func (c *cgroup) updateSubtreeMemoryCgIDs() {
	c.fs.tasksMu.Lock()
	defer c.fs.tasksMu.Unlock()

	updateTasks := func(node *cgroup) {
		var memCgID uint32
		if mem := node.closestCtrls.Load()[kernel.Cgroup2Memory]; mem != nil {
			memCgID = mem.(*memory).id
		}
		for t := range node.tasks { // +checklocksforce: c.fs.tasksMu is locked
