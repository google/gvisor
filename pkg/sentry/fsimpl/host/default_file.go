...
func (f *defaultFileFD) PRead(ctx context.Context, dst usermem.IOSequence, offset int64, opts vfs.ReadOptions) (int64, error) {
	...
	return readFromHostFD(ctx, f.hostFD, dst, offset, int(opts.Flags))
}
...
func (f *defaultFileFD) Read(ctx context.Context, dst usermem.IOSequence, opts vfs.ReadOptions) (int64, error) {
	...
	if f.wouldBlock {
		...
		// Since preadv2 is not supported, do not allow calls with preadv2 flags.
		if opts.Flags != 0 {
			return 0, syserror.ESPIPE
		}

		reader := fd.NewReadWriter(f.hostFD)
	...
	f.mu.Lock()
	n, err := readFromHostFD(ctx, f.hostFD, dst, f.offset, int(opts.Flags))
	f.offset += n
	f.mu.Unlock()
	return n, err
}
...
func (f *defaultFileFD) PWrite(ctx context.Context, src usermem.IOSequence, offset int64, opts vfs.WriteOptions) (int64, error) {
	...
	return writeToHostFD(ctx, f.hostFD, src, offset, int(opts.Flags))
}
...
func (f *defaultFileFD) Write(ctx context.Context, src usermem.IOSequence, opts vfs.WriteOptions) (int64, error) {
	...
	if f.wouldBlock {
		...
		// Since pwritev2 is not supported, do not allow calls with pwritev2 flags.
		if opts.Flags != 0 {
			return 0, syserror.ESPIPE
		}

		writer := fd.NewReadWriter(f.hostFD)
	...
	f.mu.Lock()
	n, err := writeToHostFD(ctx, f.hostFD, src, f.offset, flags)
	f.mu.Unlock()
	return n, err
}
...
func (f *defaultFileFD) seek(offset int64, whence int32) error {
	...
	case linux.SEEK_END:
		...
		if err := syscall.Fstat(f.hostFD, &s); err != nil {
			return err
		}
		size := s.Size
	...
	case linux.SEEK_DATA, linux.SEEK_HOLE:
		...
		n, err := unix.Seek(f.hostFD, offset, int(whence))
		if err != nil {
			return err
		}
		f.offset = n
