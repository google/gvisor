...
// against RDMA availability — NICs without moved netdevs should be
// excluded from the virtual PCI tree so NCCL's topology graph doesn't
// create dangling NET nodes.
func pciClassIsNIC(class string) bool {
	...
	prefix := c[:4]
	return prefix == "0200" || prefix == "0c06"
}

// rdmaPCISlotNamesWithNdevs returns the set of PCI slot names (lowercase)
// whose RDMA devices have at least one GID entry with a valid ndev. These
// are the NICs that were successfully moved into the sandbox netns and
// have usable GID tables.
func rdmaPCISlotNamesWithNdevs(rdmaData *RDMAData) map[string]bool {
	if rdmaData == nil {
		return nil
	}
...
func (fs *filesystem) newPCIDevicesSysfsEntries(ctx context.Context, creds *auth.Credentials, data *PCIDevicesData, rdmaData *RDMAData) (devicesSub, pciBusSub, busPCIDevicesSub map[string]kernfs.Inode, pciSlotToRelPath map[string]string) {
	...
	// Build a set of PCI slot names whose RDMA devices have valid ndevs
	// (i.e., their netdev was moved into the sandbox). NIC-class PCI
	// devices not in this set are excluded from the virtual sysfs tree.
	// Without this filter, NCCL creates NET nodes in the topology graph
	// for NICs it can't actually use (no GIDs / no ndev), causing
	// "Could not find NET with id N" errors during graph search.
	activeNICSlots := rdmaPCISlotNamesWithNdevs(rdmaData)

	kept, skipped := 0, 0
	for _, dev := range data.Devices {
		if pciClassIsNIC(dev.Class) {
			addr := strings.ToLower(dev.Address)
			if activeNICSlots != nil && !activeNICSlots[addr] {
				skipped++
				continue
			}
		}
	...
	log.Infof("pci sysfs: building virtual sysfs for %d device(s) (%d NIC(s) skipped — no moved ndev)", kept, skipped)
	...
	for _, dev := range data.Devices {
		// Skip NIC-class devices whose RDMA counterpart has no moved ndev.
		if pciClassIsNIC(dev.Class) {
			addr := strings.ToLower(dev.Address)
			if activeNICSlots != nil && !activeNICSlots[addr] {
				continue
		...
		// For NIC-class PCI devices with matching RDMA data, add uevent
		// and modalias so they're accessible when NCCL follows the
		// /sys/class/infiniband/<ibdev>/device symlink into this directory.
