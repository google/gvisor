...
func (fs *filesystem) AccessAt(ctx context.Context, rp *vfs.ResolvingPath, creds *auth.Credentials, mode vfs.AccessTypes) error {
	fs.mu.RLock()
	defer fs.mu.RUnlock()
	...
	return d.inode.checkPermissions(creds, mode, d.inode.isDir())
}
