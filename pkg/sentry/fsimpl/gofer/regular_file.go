...
func (fd *regularFileFD) OnClose(ctx context.Context) error {
	...
	if d.inode.fs.opts.interop == InteropModeExclusive {
		...
		haveDirtyPages := !d.inode.dirty.IsEmpty()
		d.inode.dataMu.RUnlock()
		if haveDirtyPages {
			return nil
		}
	}
...
func (fd *regularFileFD) PRead(ctx context.Context, dst usermem.IOSequence, offset int64, opts vfs.ReadOptions) (int64, error) {
	...
	if d.cachedMetadataAuthoritative() && uint64(offset) >= d.inode.size.Load() {
		return 0, io.EOF
	}
...
func (fd *regularFileFD) pwrite(ctx context.Context, src usermem.IOSequence, offset int64, opts vfs.WriteOptions) (written, finalOff int64, err error) {
	...
	if fd.vfsfd.StatusFlags()&linux.O_APPEND != 0 {
		// Holding d.metadataMu is sufficient for reading d.inode.size.
...
func (d *dentry) Evict(ctx context.Context, er pgalloc.EvictableRange) {
	...
	for mgap := d.inode.mappings.LowerBoundGap(mr.Start); mgap.Ok() && mgap.Start() < mr.End; mgap = mgap.NextGap() {
		mgapMR := mgap.Range().Intersect(mr)
		if mgapMR.Length() == 0 {
			continue
		}
		if err := fsutil.SyncDirty(ctx, mgapMR, &d.inode.cache, &d.inode.dirty, d.inode.size.Load(), mf, h.writeFromBlocksAt); err != nil {
			log.Warningf("Failed to writeback cached data %v: %v", mgapMR, err)
		}
		d.inode.cache.Drop(mgapMR, mf)
		d.inode.dirty.KeepClean(mgapMR)
	}
}
...
func (d *dentryPlatformFile) MapInternal(fr memmap.FileRange, at hostarch.AccessType) (safemem.BlockSeq, error) {
	d.inode.handleMu.RLock()
	defer d.inode.handleMu.RUnlock()
	return d.hostFileMapper.MapInternal(fr, int(d.inode.mmapFD.RacyLoad()), at.Write)
}
...
func (d *dentryPlatformFile) FD() int {
	d.inode.handleMu.RLock()
	defer d.inode.handleMu.RUnlock()
	return int(d.inode.mmapFD.RacyLoad())
}
