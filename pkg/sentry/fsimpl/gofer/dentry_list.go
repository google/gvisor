...
//
//go:nosplit
func dentryRingInit(e *dentryListElem) {
	linker := dentryElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func dentryRingAdd(old *dentryListElem, new *dentryListElem) {
	oldLinker := dentryElementMapper{}.linkerFor(old)
	newLinker := dentryElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func dentryRingRemove(e *dentryListElem) {
	eLinker := dentryElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func dentryRingEmpty(e *dentryListElem) bool {
	linker := dentryElementMapper{}.linkerFor(e)
