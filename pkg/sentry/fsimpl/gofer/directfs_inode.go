...
func (fs *filesystem) newDirectfsInode(controlFD int) (*inode, error) {
	...
	cachedInode := fs.findInode(fs.inoFromKey(inoKey))
	if cachedInode != nil {
		return cachedInode, nil
	}
	...
	i.inoKey = inoKey
	i.size.Store(uint64(stat.Size))
	i.nlink.Store(uint32(stat.Nlink))
	i.mode.Store(uint32(stat.Mode))
	i.uid.Store(uint32(stat.Uid))
	i.gid.Store(uint32(stat.Gid))
