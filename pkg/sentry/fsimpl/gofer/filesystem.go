...
func (fs *filesystem) Sync(ctx context.Context) error {
	...
	for d := range fs.syncableDentries {
		// Try to acquire reference because fs.syncableDentries doesn't hold a
		// reference on the dentries. Note that it must process files in the cache
		// because they may not have been flushed yet.
		if d.tryIncRefFromCache() {
			ds = append(ds, d)
		}
	}
	sffds := make([]*specialFileFD, 0, len(fs.specialFileFDs))
	for sffd := range fs.specialFileFDs {
		// As above, tries to acquire reference because fs.specialFileFDs doesn't
		// hold references. However, unlike dentries, an FD that has reached zero
		// references can't be resurrected, so we can use TryIncRef.
