...
func (fs *filesystem) getOrCreateInode(inoKey inoKey, createInode func() *inode) *inode {
	if cachedInode, ok := fs.inodeByKey[inoKey]; ok {
		cachedInode.fs.inodeMu.Lock()
		cachedInode.incRef()
		cachedInode.fs.inodeMu.Unlock()
		return cachedInode
	}
	inode := createInode()
	inode.fs.inodeMu.Lock()
	fs.inodeByKey[inoKey] = inode
	inode.fs.inodeMu.Unlock()
	return inode
}
...
type inode struct {
	...
	// it points to. This reference is decremented in two places: d.destroyLocked() and
	// fs.getRemoteChildLocked() when a race is detected. For both cases, fs.inodeMu must
	// be locked prior to the decrement. incRef() checks that fs.inodeMu is locked.
	refs inodeRefs
