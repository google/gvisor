...
// getInode returns the inode for the given inoKey and increments its reference count.
// If the inode is not found, then createInode is called and the new inode is returned.
// getInode locks fs.inodeMu.
// The caller is responsible for decrementing the inode's reference count when done.
func (fs *filesystem) getOrCreateInode(inoKey inoKey, createInode func() *inode) (*inode, bool) {
	fs.inodeMu.Lock()
	defer fs.inodeMu.Unlock()
	if cachedInode, ok := fs.inodeByKey[inoKey]; ok {
		cachedInode.incRef()
		return cachedInode, true
	}
	return createInode(), false
}
...
type inode struct {
	...
	// refs is the reference count. A dentry holds a reference on the inode
	// it points to.
	refs inodeRefs
