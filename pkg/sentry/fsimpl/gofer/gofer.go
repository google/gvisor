...
type filesystem struct {
	...
	inodeByIno   map[uint64]*inode `state:"nosave"`
...
func (fs *filesystem) findInode(ino uint64) *inode {
	fs.inodeByInoMu.Lock()
	defer fs.inodeByInoMu.Unlock()
	if inode, ok := fs.inodeByIno[ino]; ok {
		inode.incRef()
		return inode
	}
	return nil
...
func (i *inode) init(fs *filesystem, ino uint64, impl any) {
	i.refs.InitRefs()
	i.refs.LogRefs()
	...
	// [[[ vfs.Dentry ] dentry -> inode ] inodeImpl ]
