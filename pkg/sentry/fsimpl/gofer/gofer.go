...
func (d *dentry) setStat(ctx context.Context, creds *auth.Credentials, stat *linux.Statx, mnt *vfs.Mount) error {
	...
	if err := vfs.CheckSetStat(creds, mnt, stat, mode, auth.KUID(atomic.LoadUint32(&d.uid)), auth.KGID(atomic.LoadUint32(&d.gid))); err != nil {
		return err
	}
...
func (d *dentry) checkPermissions(creds *auth.Credentials, mnt *vfs.Mount, ats vfs.AccessTypes) error {
	return vfs.GenericCheckPermissions(creds, mnt, ats, linux.FileMode(atomic.LoadUint32(&d.mode)), auth.KUID(atomic.LoadUint32(&d.uid)), auth.KGID(atomic.LoadUint32(&d.gid)))
}
