...
func (fs *filesystem) walkLisa(ctx context.Context, dirFD lisafs.FDID, names []string) ([]lisafs.Inode, error) {
	req := lisafs.WalkReq{
		DirFD: dirFD,
...
func (fs *filesystem) walkOneLisa(ctx context.Context, dirFD lisafs.FDID, name string) (lisafs.Inode, error) {
	req := lisafs.WalkReq{
		DirFD: dirFD,
		Path:  []string{name},
	}
	...
	if err == nil {
		if n := len(resp.Inodes); n > 1 {
			panic(fmt.Sprintf("requested to walk one component, but got %d results", n))
		} else if n == 0 {
