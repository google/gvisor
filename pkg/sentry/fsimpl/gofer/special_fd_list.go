...
//
//go:nosplit
func specialFDRingInit(e *specialFileFD) {
	linker := specialFDElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func specialFDRingAdd(old *specialFileFD, new *specialFileFD) {
	oldLinker := specialFDElementMapper{}.linkerFor(old)
	newLinker := specialFDElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func specialFDRingRemove(e *specialFileFD) {
	eLinker := specialFDElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func specialFDRingEmpty(e *specialFileFD) bool {
	linker := specialFDElementMapper{}.linkerFor(e)
