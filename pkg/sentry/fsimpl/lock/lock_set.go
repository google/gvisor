...
// +stateify savable
type LockSet struct {
	root Locknode `state:".([]LockFlatSegment)"`
}
...
func (s *LockSet) Remove(seg LockIterator) LockGapIterator {
	...
	return seg.node.rebalanceAfterRemove(LockGapIterator(seg))
}
...
func (s *LockSet) MoveFrom(s2 *LockSet) {
	*s = *s2
...
func (seg LockIterator) PrevNonEmpty() (LockIterator, LockGapIterator) {
	if prev := seg.PrevSegment(); prev.Ok() && prev.End() == seg.Start() {
		return prev, LockGapIterator{}
	}
	return LockIterator{}, seg.PrevGap()
}
...
func (seg LockIterator) NextNonEmpty() (LockIterator, LockGapIterator) {
	if next := seg.NextSegment(); next.Ok() && next.Start() == seg.End() {
		return next, LockGapIterator{}
	}
	return LockIterator{}, seg.NextGap()
}
...
func (s *LockSet) ExportSlice() []LockFlatSegment {
	var fs []LockFlatSegment
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		fs = append(fs, LockFlatSegment{
			Start: seg.Start(),
			End:   seg.End(),
			Value: seg.Value(),
		})
...
func (s *LockSet) saveRoot() []LockFlatSegment {
	fs := s.ExportSlice()
