...
func (i *inode) StatFS(ctx context.Context, fs *vfs.Filesystem) (linux.Statfs, error) {
	return vfs.GenericStatFS(linux.SOCKFS_MAGIC), syserror.EFAULT
}
...
type fileDescription struct {
	...
	vfs.LockFD // FIXME: This should probably be a LockFD
...
func (*fileDescription) InvalidateUnsavable(context.Context) error {
	panic("InvalidateUnsavable not implemented")
	return nil
}
...
func (fd *fileDescription) cqEndOffset() uint64 {
	// Sizeof need an instance of the type...
	cqe := linux.IoUringCqe{}
	CQEndOff := uint64(fd.cqEntries) * uint64(unsafe.Sizeof(cqe))
...
func (fd *fileDescription) sqeEndOffset() uint64 {
	...
	SQESEndOff := uint64(fd.sqEntries) * uint64(unsafe.Sizeof(sqe))
...
func (fd *fileDescription) isInRing(maprange memmap.MappableRange) bool {
	if maprange.Start >= linux.IORING_OFF_SQ_RING && maprange.End <= fd.sqEndOffset() {
		return true
	}

	if maprange.Start >= linux.IORING_OFF_CQ_RING && maprange.End <= fd.cqEndOffset() {
		return true
	}

	if maprange.Start >= linux.IORING_OFF_SQES && maprange.End <= fd.sqeEndOffset() {
		return true
	}
