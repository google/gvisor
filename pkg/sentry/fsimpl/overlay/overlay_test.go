...
// layerIdentity returns the identity of name on the given layer root.
func (o *testOverlay) layerIdentity(t *testing.T, layerRoot vfs.VirtualDentry, name string) vfs.InodeIdentity {
	t.Helper()
	vd, err := o.vfsObj.GetDentryAt(o.ctx, o.creds, &vfs.PathOperation{
		Root:  layerRoot,
		Start: layerRoot,
	...
	if err != nil {
		t.Fatalf("failed to look up %q on the layer: %v", name, err)
	}
	defer vd.DecRef(o.ctx)
	return vd.Dentry().InodeIdentity()
}

// TestInodeIdentityDistinctFromLayers verifies that an overlay file's identity
// is neither its lower layer file's nor, once copied up, its upper layer
// file's, while staying the same across lookups and, once pinned as
// landlock_add_rule(2) pins it, across the copy-up. On Linux the overlay inode
// is distinct from both layer inodes, so a Landlock rule on a layer file never
// covers the merged file or the reverse, which the selftest
// layout2_overlay.same_content_different_file checks.
func TestInodeIdentityDistinctFromLayers(t *testing.T) {
	o := newTestOverlay(t)
	defer o.cleanup()
	const filename = "file"
	o.createLowerFile(t, filename)

	lowerID := o.layerIdentity(t, o.lowerRoot, filename)
	if !lowerID.Ok() {
		t.Fatalf("lower %q has no identity", filename)
	}
	vd := o.getDentry(t, filename)
	vd.Dentry().PinInodeIdentity()
	...
	vd.DecRef(o.ctx)
	if !id.Ok() {
		t.Fatalf("overlay %q has no identity", filename)
	}
	if id == lowerID {
		t.Errorf("overlay identity of %q equals its lower layer file's identity %+v", filename, id)
	}

	vd2 := o.getDentry(t, filename)
	id2 := vd2.Dentry().InodeIdentity()
	vd2.DecRef(o.ctx)
	if id2 != id {
		t.Errorf("identity of %q changed between lookups: %+v then %+v", filename, id, id2)
	}

	o.copyUp(t, filename)
	upperID := o.layerIdentity(t, o.upperRoot, filename)
	if !upperID.Ok() {
		t.Fatalf("upper %q has no identity", filename)
	}
	vd3 := o.getDentry(t, filename)
	id3 := vd3.Dentry().InodeIdentity()
	vd3.DecRef(o.ctx)
	if id3 != id {
		t.Errorf("identity of %q changed across copy-up: %+v then %+v", filename, id, id3)
	}
	if id3 == upperID {
		t.Errorf("overlay identity of %q equals its upper layer file's identity %+v", filename, id3)
	}
}
