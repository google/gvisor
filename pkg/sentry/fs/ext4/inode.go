...
func newInode(ctx context.Context, bgdl *goext4.BlockGroupDescriptorList, msrc *fs.MountSource, absoluteInodeNumber uint64, rs io.ReadSeeker, inodeType fs.InodeType) (*fs.Inode, error) {
	...
	var inodeOps fs.InodeOperations
	switch inodeType {
	case fs.Pipe:
		p := pipe.NewPipe(ctx, true /* isNamed */, pipe.DefaultPipeSize, usermem.PageSize)
		inodeOps = pipe.NewInodeOperations(ctx, fs.FilePermsFromMode(linux.FileMode(ext4Inode.Data().IMode)), p)
	case fs.Socket:
		return nil, syserr.ErrSocketNotSupported.ToError()
	default:
	...
	// Use ext4 absolute inode numbers for InodeID since they are unique within
	// this device.
	return fs.NewInode(
		ctx, inodeOps, msrc, fs.StableAttr{
			Type:      inodeType,
...
func (i *inodeOperations) GetFile(ctx context.Context, d *fs.Dirent, flags fs.FileFlags) (*fs.File, error) {
	return fs.NewFile(ctx, d, flags, &fileOperations{inodeOperations: i}), nil
}
...
func (i *inodeOperations) Lookup(ctx context.Context, dir *fs.Inode, name string) (*fs.Dirent, error) {
	...
	for {
		...
		return fs.NewDirent(ctx, childInode, name), nil
	}
}
