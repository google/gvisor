...
func (*overlayFileOperations) ConfigureMMap(ctx context.Context, file *File, opts *memmap.MMapOpts) error {
	...
	// FIXME: This is a copy/paste of fsutil.GenericConfigureMMap, which we
	// can't use because the overlay implementation is in package fs, so
	// depending on fs/fsutil would create a circular dependency. Move
