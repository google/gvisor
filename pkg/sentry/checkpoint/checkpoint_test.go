...
func TestResourceID(t *testing.T) {
	...
	noContainer := ResourceID{Path: "/data"}
	if !noContainer.Ok() {
		t.Errorf("ResourceID%+v.Ok() = false, want true", noContainer)
	}
	if got, want := noContainer.String(), "/data"; got != want {
		t.Errorf("ResourceID%+v.String() = %q, want %q", noContainer, got, want)
	}
