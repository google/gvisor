...
const refstrackGaps = 0

var _ = uint8(refstrackGaps << 7) // Will fail if not zero or one.

// dynamicGap is a type that disappears if trackGaps is 0.
type refsdynamicGap [refstrackGaps]uint64

// Get returns the value of the gap.
//
// Precondition: trackGaps must be non-zero.
func (d *refsdynamicGap) Get() uint64 {
	return d[:][0]
}
...
//
// Precondition: trackGaps must be non-zero.
func (d *refsdynamicGap) Set(v uint64) {
	d[:][0] = v
}
...
// +stateify savable
type refsSet struct {
	root refsnode `state:".([]refsFlatSegment)"`
}

// IsEmpty returns true if the set contains no segments.
func (s *refsSet) IsEmpty() bool {
...
// more efficient.
func (s *refsSet) IsEmptyRange(r __generics_imported0.FileRange) bool {
	switch {
	case r.Length() < 0:
...
func (s *refsSet) SpanRange(r __generics_imported0.FileRange) uint64 {
	switch {
	case r.Length() < 0:
		panic(fmt.Sprintf("invalid range %v", r))
	case r.Length() == 0:
...
func (s *refsSet) FirstSegment() refsIterator {
...
func (s *refsSet) LastSegment() refsIterator {
	if s.root.nrSegments == 0 {
		return refsIterator{}
	}
...
func (s *refsSet) FirstGap() refsGapIterator {
	n := &s.root
	for n.hasChildren {
		n = n.children[0]
	}
	return refsGapIterator{n, 0}
}

// LastGap returns the last gap in the set.
func (s *refsSet) LastGap() refsGapIterator {
	n := &s.root
	for n.hasChildren {
		n = n.children[n.nrSegments]
	}
	return refsGapIterator{n, n.nrSegments}
}
...
func (s *refsSet) Find(key uint64) (refsIterator, refsGapIterator) {
	n := &s.root
	for {
		...
		for lower < upper {
			...
			if r := n.keys[i]; key < r.End {
				if key >= r.Start {
					return refsIterator{n, i}, refsGapIterator{}
				}
				upper = i
			} else {
				lower = i + 1
		...
		i := lower
		if !n.hasChildren {
			return refsIterator{}, refsGapIterator{n, i}
		}
		n = n.children[i]
	}
}
...
func (s *refsSet) FindSegment(key uint64) refsIterator {
	seg, _ := s.Find(key)
	return seg
}
...
func (s *refsSet) LowerBoundSegment(min uint64) refsIterator {
	seg, gap := s.Find(min)
	if seg.Ok() {
		return seg
	}
...
func (s *refsSet) UpperBoundSegment(max uint64) refsIterator {
	seg, gap := s.Find(max)
	if seg.Ok() {
		return seg
	}
...
// terminal iterator.
func (s *refsSet) FindGap(key uint64) refsGapIterator {
	_, gap := s.Find(key)
	return gap
}
...
// equal to min.
func (s *refsSet) LowerBoundGap(min uint64) refsGapIterator {
	seg, gap := s.Find(min)
	if gap.Ok() {
		return gap
	}
...
// equal to max.
func (s *refsSet) UpperBoundGap(max uint64) refsGapIterator {
	seg, gap := s.Find(max)
	if gap.Ok() {
		return gap
	}
...
func (s *refsSet) FirstLargeEnoughGap(minSize uint64) refsGapIterator {
	if refstrackGaps != 1 {
...
//
// Precondition: trackGaps must be 1.
func (s *refsSet) LastLargeEnoughGap(minSize uint64) refsGapIterator {
	if refstrackGaps != 1 {
		panic("set is not tracking gaps")
	}
	gap := s.LastGap()
...
//
// Precondition: trackGaps must be 1.
func (s *refsSet) LowerBoundLargeEnoughGap(min, minSize uint64) refsGapIterator {
	if refstrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
//
// Precondition: trackGaps must be 1.
func (s *refsSet) UpperBoundLargeEnoughGap(max, minSize uint64) refsGapIterator {
	if refstrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (s *refsSet) Insert(gap refsGapIterator, r __generics_imported0.FileRange, val uint64) refsIterator {
	if r.Length() <= 0 {
	...
	if prev.Ok() && prev.End() == r.Start {
		if mval, ok := (refsSetFuncs{}).Merge(prev.Range(), prev.Value(), r, val); ok {
			shrinkMaxGap := refstrackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Get()
			...
			if next.Ok() && next.Start() == r.End {
				val = mval
				if mval, ok := (refsSetFuncs{}).Merge(prev.Range(), val, next.Range(), next.Value()); ok {
	...
	if next.Ok() && next.Start() == r.End {
		if mval, ok := (refsSetFuncs{}).Merge(r, val, next.Range(), next.Value()); ok {
			shrinkMaxGap := refstrackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Get()
...
func (s *refsSet) InsertWithoutMerging(gap refsGapIterator, r __generics_imported0.FileRange, val uint64) refsIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
//   - r.Start >= gap.Start().
//   - r.End <= gap.End().
func (s *refsSet) InsertWithoutMergingUnchecked(gap refsGapIterator, r __generics_imported0.FileRange, val uint64) refsIterator {
	gap = gap.node.rebalanceBeforeInsert(gap)
	splitMaxGap := refstrackGaps != 0 && (gap.node.nrSegments == 0 || gap.Range().Length() == gap.node.maxGap.Get())
	...
	return refsIterator{gap.node, gap.index}
}
...
func (s *refsSet) InsertRange(r __generics_imported0.FileRange, val uint64) refsIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (s *refsSet) InsertWithoutMergingRange(r __generics_imported0.FileRange, val uint64) refsIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
// additional work between finding the gap and insertion, use Insert instead.
func (s *refsSet) TryInsertRange(r __generics_imported0.FileRange, val uint64) refsIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *refsSet) TryInsertWithoutMergingRange(r __generics_imported0.FileRange, val uint64) refsIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *refsSet) Remove(seg refsIterator) refsGapIterator {
	...
	refsSetFuncs{}.ClearValue(&seg.node.values[seg.node.nrSegments-1])
	seg.node.nrSegments--
	if refstrackGaps != 0 {
		seg.node.updateMaxGapLeaf()
	}
	return seg.node.rebalanceAfterRemove(refsGapIterator{seg.node, seg.index})
}

// RemoveAll removes all segments from the set. All existing iterators are
// invalidated.
func (s *refsSet) RemoveAll() {
	s.root = refsnode{}
}
...
func (s *refsSet) RemoveRange(r __generics_imported0.FileRange) refsGapIterator {
...
func (s *refsSet) RemoveFullRange(r __generics_imported0.FileRange) refsGapIterator {
...
func (s *refsSet) RemoveRangeWith(r __generics_imported0.FileRange, f func(seg refsIterator)) refsGapIterator {
	seg, gap := s.Find(r.Start)
	if seg.Ok() {
		seg = s.Isolate(seg, r)
		if f != nil {
...
// panics.
func (s *refsSet) RemoveFullRangeWith(r __generics_imported0.FileRange, f func(seg refsIterator)) refsGapIterator {
...
func (s *refsSet) MoveFrom(s2 *refsSet) {
	*s = *s2
...
func (s *refsSet) Merge(first, second refsIterator) refsIterator {
...
func (s *refsSet) MergeUnchecked(first, second refsIterator) refsIterator {
	if first.End() == second.Start() {
		if mval, ok := (refsSetFuncs{}).Merge(first.Range(), first.Value(), second.Range(), second.Value()); ok {
...
// skipped.
func (s *refsSet) MergePrev(seg refsIterator) refsIterator {
...
// skipped.
func (s *refsSet) MergeNext(seg refsIterator) refsIterator {
...
func (s *refsSet) Unisolate(seg refsIterator) refsIterator {
	if prev := seg.PrevSegment(); prev.Ok() {
		if mseg := s.MergeUnchecked(prev, seg); mseg.Ok() {
			seg = mseg
		}
...
func (s *refsSet) MergeInsideRange(r __generics_imported0.FileRange) {
...
func (s *refsSet) MergeOutsideRange(r __generics_imported0.FileRange) {
...
// Split panics.
func (s *refsSet) Split(seg refsIterator, split uint64) (refsIterator, refsIterator) {
...
func (s *refsSet) SplitUnchecked(seg refsIterator, split uint64) (refsIterator, refsIterator) {
	val1, val2 := (refsSetFuncs{}).Split(seg.Range(), seg.Value(), split)
	end2 := seg.End()
...
func (s *refsSet) SplitBefore(seg refsIterator, start uint64) refsIterator {
...
func (s *refsSet) SplitAfter(seg refsIterator, end uint64) refsIterator {
...
func (s *refsSet) Isolate(seg refsIterator, r __generics_imported0.FileRange) refsIterator {
...
func (s *refsSet) LowerBoundSegmentSplitBefore(min uint64) refsIterator {
	seg, gap := s.Find(min)
	if seg.Ok() {
...
// mutated, suitable as the initial value for a loop variable.
func (s *refsSet) UpperBoundSegmentSplitAfter(max uint64) refsIterator {
	seg, gap := s.Find(max)
	if seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *refsSet) VisitRange(r __generics_imported0.FileRange, f func(seg refsIterator) bool) {
	for seg := s.LowerBoundSegment(r.Start); seg.Ok() && seg.Start() < r.End; seg = seg.NextSegment() {
		if !f(seg) {
			return
		}
...
func (s *refsSet) VisitFullRange(r __generics_imported0.FileRange, f func(seg refsIterator) bool) {
	pos := r.Start
	seg := s.FindSegment(r.Start)
	for {
		if !seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *refsSet) MutateRange(r __generics_imported0.FileRange, f func(seg refsIterator) bool) {
...
func (s *refsSet) MutateFullRange(r __generics_imported0.FileRange, f func(seg refsIterator) bool) {
	seg := s.FindSegment(r.Start)
	if !seg.Ok() {
		panic(fmt.Sprintf("missing segment at %v", r.Start))
	}
...
type refsnode struct {
	...
	maxGap refsdynamicGap

	// Nodes store keys and values in separate arrays to maximize locality in
	// the common case (scanning keys for lookup).
	keys     [refsmaxDegree - 1]__generics_imported0.FileRange
	values   [refsmaxDegree - 1]uint64
	children [refsmaxDegree]*refsnode
}

// firstSegment returns the first segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *refsnode) firstSegment() refsIterator {
	for n.hasChildren {
		n = n.children[0]
	}
	return refsIterator{n, 0}
}

// lastSegment returns the last segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *refsnode) lastSegment() refsIterator {
	for n.hasChildren {
		n = n.children[n.nrSegments]
	}
	return refsIterator{n, n.nrSegments - 1}
}

func (n *refsnode) prevSibling() *refsnode {
...
func (n *refsnode) nextSibling() *refsnode {
...
// represented by gap.
func (n *refsnode) rebalanceBeforeInsert(gap refsGapIterator) refsGapIterator {
	if n.nrSegments < refsmaxDegree-1 {
		return gap
	}
	if n.parent != nil {
	...
	if n.parent == nil {
		...
		copy(left.keys[:refsminDegree-1], n.keys[:refsminDegree-1])
		copy(left.values[:refsminDegree-1], n.values[:refsminDegree-1])
		copy(right.keys[:refsminDegree-1], n.keys[refsminDegree:])
		copy(right.values[:refsminDegree-1], n.values[refsminDegree:])
		n.keys[0], n.values[0] = n.keys[refsminDegree-1], n.values[refsminDegree-1]
		refszeroValueSlice(n.values[1:])
		if n.hasChildren {
			copy(left.children[:refsminDegree], n.children[:refsminDegree])
			copy(right.children[:refsminDegree], n.children[refsminDegree:])
			refszeroNodeSlice(n.children[2:])
			for i := 0; i < refsminDegree; i++ {
		...
		if gap.index < refsminDegree {
			return refsGapIterator{left, gap.index}
		}
		return refsGapIterator{right, gap.index - refsminDegree}
	}
	...
	n.parent.keys[n.parentIndex], n.parent.values[n.parentIndex] = n.keys[refsminDegree-1], n.values[refsminDegree-1]
	...
	n.parent.nrSegments++
	copy(sibling.keys[:refsminDegree-1], n.keys[refsminDegree:])
	copy(sibling.values[:refsminDegree-1], n.values[refsminDegree:])
	refszeroValueSlice(n.values[refsminDegree-1:])
	if n.hasChildren {
		copy(sibling.children[:refsminDegree], n.children[refsminDegree:])
		refszeroNodeSlice(n.children[refsminDegree:])
		for i := 0; i < refsminDegree; i++ {
			sibling.children[i].parent = sibling
			sibling.children[i].parentIndex = i
		}
	}
	n.nrSegments = refsminDegree - 1
	...
	return refsGapIterator{sibling, gap.index - refsminDegree}
}
...
// B-tree invariant.
func (n *refsnode) rebalanceAfterRemove(gap refsGapIterator) refsGapIterator {
	for {
		if n.nrSegments >= refsminDegree-1 {
			return gap
		}
		if n.parent == nil {
		...
		if sibling := n.prevSibling(); sibling != nil && sibling.nrSegments >= refsminDegree {
			...
			refsSetFuncs{}.ClearValue(&sibling.values[sibling.nrSegments-1])
			if n.hasChildren {
			...
			if gap.node == n {
				return refsGapIterator{n, gap.index + 1}
			}
			return gap
		}
		if sibling := n.nextSibling(); sibling != nil && sibling.nrSegments >= refsminDegree {
			...
			if gap.node == sibling {
				...
				return refsGapIterator{sibling, gap.index - 1}
			}
			return gap
		}
		...
		if p.nrSegments == 1 {
			...
			if gap.node == left {
				return refsGapIterator{p, gap.index}
			}
			if gap.node == right {
				return refsGapIterator{p, gap.index + left.nrSegments + 1}
			}
			return gap
		}
		...
		if gap.node == right {
			gap = refsGapIterator{left, gap.index + left.nrSegments + 1}
		}
		...
		refsSetFuncs{}.ClearValue(&p.values[p.nrSegments-1])
...
func (n *refsnode) updateMaxGapLeaf() {
	if n.hasChildren {
...
//
// Precondition: trackGaps must be 1.
func (n *refsnode) updateMaxGapLocal() {
	if !n.hasChildren {
...
func (n *refsnode) calculateMaxGapLeaf() uint64 {
	max := refsGapIterator{n, 0}.Range().Length()
	for i := 1; i <= n.nrSegments; i++ {
		if current := (refsGapIterator{n, i}).Range().Length(); current > max {
			max = current
		}
	}
	return max
...
func (n *refsnode) calculateMaxGapInternal() uint64 {
...
func (n *refsnode) searchFirstLargeEnoughGap(minSize uint64) refsGapIterator {
	if n.maxGap.Get() < minSize {
		return refsGapIterator{}
	}
	if n.hasChildren {
		...
		for i := 0; i <= n.nrSegments; i++ {
			currentGap := refsGapIterator{n, i}
...
// in the subtree rooted by n. If not found, return a terminal gap iterator.
func (n *refsnode) searchLastLargeEnoughGap(minSize uint64) refsGapIterator {
	if n.maxGap.Get() < minSize {
		return refsGapIterator{}
	}
	if n.hasChildren {
...
func (seg refsIterator) Ok() bool {
	return seg.node != nil
}

// Range returns the iterated segment's range key.
func (seg refsIterator) Range() __generics_imported0.FileRange {
...
func (seg refsIterator) SetRangeUnchecked(r __generics_imported0.FileRange) {
...
// iterators.
func (seg refsIterator) SetRange(r __generics_imported0.FileRange) {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (seg refsIterator) SetStartUnchecked(start uint64) {
...
// iterators.
func (seg refsIterator) SetStart(start uint64) {
	if start >= seg.End() {
...
func (seg refsIterator) SetEndUnchecked(end uint64) {
...
// iterators.
func (seg refsIterator) SetEnd(end uint64) {
	if end <= seg.Start() {
...
func (seg refsIterator) Value() uint64 {
...
// invalidate any iterators.
func (seg refsIterator) ValuePtr() *uint64 {
	return &seg.node.values[seg.index]
}

// SetValue mutates the iterated segment's value. This operation does not
// invalidate any iterators.
func (seg refsIterator) SetValue(val uint64) {
...
func (seg refsIterator) PrevSegment() refsIterator {
	if seg.node.hasChildren {
		return seg.node.children[seg.index].lastSegment()
	}
	if seg.index > 0 {
		return refsIterator{seg.node, seg.index - 1}
	}
	if seg.node.parent == nil {
		return refsIterator{}
	}
	return refssegmentBeforePosition(seg.node.parent, seg.node.parentIndex)
}
...
func (seg refsIterator) NextSegment() refsIterator {
	if seg.node.hasChildren {
	...
	return refssegmentAfterPosition(seg.node.parent, seg.node.parentIndex)
}

// PrevGap returns the gap immediately before the iterated segment.
func (seg refsIterator) PrevGap() refsGapIterator {
	if seg.node.hasChildren {

		return seg.node.children[seg.index].lastSegment().NextGap()
	}
	return refsGapIterator{seg.node, seg.index}
}

// NextGap returns the gap immediately after the iterated segment.
func (seg refsIterator) NextGap() refsGapIterator {
	if seg.node.hasChildren {
		return seg.node.children[seg.index+1].firstSegment().PrevGap()
	}
	return refsGapIterator{seg.node, seg.index + 1}
}
...
// non-terminal.
func (seg refsIterator) PrevNonEmpty() (refsIterator, refsGapIterator) {
	if prev := seg.PrevSegment(); prev.Ok() && prev.End() == seg.Start() {
		return prev, refsGapIterator{}
	}
	return refsIterator{}, seg.PrevGap()
}
...
// non-terminal.
func (seg refsIterator) NextNonEmpty() (refsIterator, refsGapIterator) {
	if next := seg.NextSegment(); next.Ok() && next.Start() == seg.End() {
		return next, refsGapIterator{}
	}
	return refsIterator{}, seg.NextGap()
}
...
// Ok returns true if the iterator is not terminal. All other methods are only
// valid for non-terminal iterators.
func (gap refsGapIterator) Ok() bool {
	return gap.node != nil
}

// Range returns the range spanned by the iterated gap.
func (gap refsGapIterator) Range() __generics_imported0.FileRange {
	return __generics_imported0.FileRange{gap.Start(), gap.End()}
}

// Start is equivalent to Range().Start, but should be preferred if only the
// start of the range is needed.
func (gap refsGapIterator) Start() uint64 {
	if ps := gap.PrevSegment(); ps.Ok() {
		return ps.End()
	}
	return refsSetFuncs{}.MinKey()
}

// End is equivalent to Range().End, but should be preferred if only the end of
// the range is needed.
func (gap refsGapIterator) End() uint64 {
	if ns := gap.NextSegment(); ns.Ok() {
		return ns.Start()
	}
	return refsSetFuncs{}.MaxKey()
}

// IsEmpty returns true if the iterated gap is empty (that is, the "gap" is
// between two adjacent segments.)
func (gap refsGapIterator) IsEmpty() bool {
...
func (gap refsGapIterator) PrevSegment() refsIterator {
	return refssegmentBeforePosition(gap.node, gap.index)
}
...
func (gap refsGapIterator) NextSegment() refsIterator {
	return refssegmentAfterPosition(gap.node, gap.index)
}

// PrevGap returns the iterated gap's predecessor. If no such gap exists,
// PrevGap returns a terminal iterator.
func (gap refsGapIterator) PrevGap() refsGapIterator {
	seg := gap.PrevSegment()
	if !seg.Ok() {
		return refsGapIterator{}
	}
...
// returns a terminal iterator.
func (gap refsGapIterator) NextGap() refsGapIterator {
	seg := gap.NextSegment()
	if !seg.Ok() {
		return refsGapIterator{}
	}
...
//
// Precondition: trackGaps must be 1.
func (gap refsGapIterator) NextLargeEnoughGap(minSize uint64) refsGapIterator {
	if refstrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (gap refsGapIterator) nextLargeEnoughGapHelper(minSize uint64) refsGapIterator {
	for {
...
//
// Precondition: trackGaps must be 1.
func (gap refsGapIterator) PrevLargeEnoughGap(minSize uint64) refsGapIterator {
	if refstrackGaps != 1 {
		panic("set is not tracking gaps")
	}
...
func (gap refsGapIterator) prevLargeEnoughGapHelper(minSize uint64) refsGapIterator {
	for {
...
func refssegmentBeforePosition(n *refsnode, i int) refsIterator {
	for i == 0 {
		if n.parent == nil {
			return refsIterator{}
		}
...
func refssegmentAfterPosition(n *refsnode, i int) refsIterator {
	for i == n.nrSegments {
		if n.parent == nil {
			return refsIterator{}
		}
	...
	return refsIterator{n, i}
}

func refszeroValueSlice(slice []uint64) {

	for i := range slice {
		refsSetFuncs{}.ClearValue(&slice[i])
	}
}

func refszeroNodeSlice(slice []*refsnode) {
	for i := range slice {
		slice[i] = nil
	}
}
...
func (s *refsSet) String() string {
	return s.root.String()
}

// String stringifies a node (and all of its children) for debugging.
func (n *refsnode) String() string {
	var buf bytes.Buffer
	n.writeDebugString(&buf, "")
	return buf.String()
}

func (n *refsnode) writeDebugString(buf *bytes.Buffer, prefix string) {
...
// key order.
func (s *refsSet) ExportSlice() []refsFlatSegment {
	var fs []refsFlatSegment
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		fs = append(fs, refsFlatSegment{
			Start: seg.Start(),
			End:   seg.End(),
			Value: seg.Value(),
		})
...
func (s *refsSet) ImportSlice(fs []refsFlatSegment) error {
	if !s.IsEmpty() {
...
func (s *refsSet) segmentTestCheck(expectedSegments int, segFunc func(int, __generics_imported0.FileRange, uint64) error) error {
	havePrev := false
	prev := uint64(0)
	nrSegments := 0
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
...
func (s *refsSet) countSegments() (segments int) {
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		segments++
	}
	return segments
}
func (s *refsSet) saveRoot() []refsFlatSegment {
	fs := s.ExportSlice()
...
func (s *refsSet) loadRoot(_ context.Context, fs []refsFlatSegment) {
