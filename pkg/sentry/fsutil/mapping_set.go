...
type mappingSet struct {
	root mappingnode `state:".([]mappingFlatSegment)"`
}
...
// more efficient.
func (s *mappingSet) IsEmptyRange(r __generics_imported0.FileRange) bool {
	switch {
	case r.Length() < 0:
...
func (s *mappingSet) SpanRange(r __generics_imported0.FileRange) uint64 {
	switch {
	case r.Length() < 0:
		panic(fmt.Sprintf("invalid range %v", r))
	case r.Length() == 0:
...
func (s *mappingSet) Insert(gap mappingGapIterator, r __generics_imported0.FileRange, val mapping) mappingIterator {
	if r.Length() <= 0 {
	...
	if prev.Ok() && prev.End() == r.Start {
		if mval, ok := (mappingSetFuncs{}).Merge(prev.Range(), prev.Value(), r, val); ok {
			...
			if next.Ok() && next.Start() == r.End {
				val = mval
				if mval, ok := (mappingSetFuncs{}).Merge(prev.Range(), val, next.Range(), next.Value()); ok {
	...
	if next.Ok() && next.Start() == r.End {
		if mval, ok := (mappingSetFuncs{}).Merge(r, val, next.Range(), next.Value()); ok {
			shrinkMaxGap := mappingtrackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Get()
...
func (s *mappingSet) InsertWithoutMerging(gap mappingGapIterator, r __generics_imported0.FileRange, val mapping) mappingIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
//   - r.Start >= gap.Start().
//   - r.End <= gap.End().
func (s *mappingSet) InsertWithoutMergingUnchecked(gap mappingGapIterator, r __generics_imported0.FileRange, val mapping) mappingIterator {
...
func (s *mappingSet) InsertRange(r __generics_imported0.FileRange, val mapping) mappingIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (s *mappingSet) InsertWithoutMergingRange(r __generics_imported0.FileRange, val mapping) mappingIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
// additional work between finding the gap and insertion, use Insert instead.
func (s *mappingSet) TryInsertRange(r __generics_imported0.FileRange, val mapping) mappingIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *mappingSet) TryInsertWithoutMergingRange(r __generics_imported0.FileRange, val mapping) mappingIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *mappingSet) Remove(seg mappingIterator) mappingGapIterator {
	...
	mappingSetFuncs{}.ClearValue(&seg.node.values[seg.node.nrSegments-1])
	seg.node.nrSegments--
	if mappingtrackGaps != 0 {
		seg.node.updateMaxGapLeaf()
	}
	return seg.node.rebalanceAfterRemove(mappingGapIterator(seg))
}
...
func (s *mappingSet) RemoveRange(r __generics_imported0.FileRange) mappingGapIterator {
...
func (s *mappingSet) RemoveFullRange(r __generics_imported0.FileRange) mappingGapIterator {
...
func (s *mappingSet) RemoveRangeWith(r __generics_imported0.FileRange, f func(seg mappingIterator)) mappingGapIterator {
	seg, gap := s.Find(r.Start)
	if seg.Ok() {
		seg = s.Isolate(seg, r)
		if f != nil {
...
// panics.
func (s *mappingSet) RemoveFullRangeWith(r __generics_imported0.FileRange, f func(seg mappingIterator)) mappingGapIterator {
...
func (s *mappingSet) MoveFrom(s2 *mappingSet) {
	*s = *s2
...
func (s *mappingSet) MergeUnchecked(first, second mappingIterator) mappingIterator {
	if first.End() == second.Start() {
		if mval, ok := (mappingSetFuncs{}).Merge(first.Range(), first.Value(), second.Range(), second.Value()); ok {
...
func (s *mappingSet) MergeInsideRange(r __generics_imported0.FileRange) {
...
func (s *mappingSet) MergeOutsideRange(r __generics_imported0.FileRange) {
...
func (s *mappingSet) SplitUnchecked(seg mappingIterator, split uint64) (mappingIterator, mappingIterator) {
	val1, val2 := (mappingSetFuncs{}).Split(seg.Range(), seg.Value(), split)
	end2 := seg.End()
...
func (s *mappingSet) Isolate(seg mappingIterator, r __generics_imported0.FileRange) mappingIterator {
...
//
// N.B. f must not invalidate iterators into s.
func (s *mappingSet) VisitRange(r __generics_imported0.FileRange, f func(seg mappingIterator) bool) {
	for seg := s.LowerBoundSegment(r.Start); seg.Ok() && seg.Start() < r.End; seg = seg.NextSegment() {
		if !f(seg) {
			return
		}
...
func (s *mappingSet) VisitFullRange(r __generics_imported0.FileRange, f func(seg mappingIterator) bool) {
	pos := r.Start
	seg := s.FindSegment(r.Start)
	for {
		if !seg.Ok() {
...
//
// N.B. f must not invalidate iterators into s.
func (s *mappingSet) MutateRange(r __generics_imported0.FileRange, f func(seg mappingIterator) bool) {
...
func (s *mappingSet) MutateFullRange(r __generics_imported0.FileRange, f func(seg mappingIterator) bool) {
	seg := s.FindSegment(r.Start)
	if !seg.Ok() {
		panic(fmt.Sprintf("missing segment at %v", r.Start))
	}
...
type mappingnode struct {
	...
	keys     [mappingmaxDegree - 1]__generics_imported0.FileRange
	values   [mappingmaxDegree - 1]mapping
...
func (n *mappingnode) rebalanceAfterRemove(gap mappingGapIterator) mappingGapIterator {
	for {
		...
		if sibling := n.prevSibling(); sibling != nil && sibling.nrSegments >= mappingminDegree {
			...
			mappingSetFuncs{}.ClearValue(&sibling.values[sibling.nrSegments-1])
			if n.hasChildren {
		...
		mappingSetFuncs{}.ClearValue(&p.values[p.nrSegments-1])
...
func (seg mappingIterator) Range() __generics_imported0.FileRange {
...
func (seg mappingIterator) SetRangeUnchecked(r __generics_imported0.FileRange) {
...
// iterators.
func (seg mappingIterator) SetRange(r __generics_imported0.FileRange) {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (seg mappingIterator) Value() mapping {
...
// invalidate any iterators.
func (seg mappingIterator) ValuePtr() *mapping {
	return &seg.node.values[seg.index]
}

// SetValue mutates the iterated segment's value. This operation does not
// invalidate any iterators.
func (seg mappingIterator) SetValue(val mapping) {
...
func (seg mappingIterator) PrevNonEmpty() (mappingIterator, mappingGapIterator) {
	if prev := seg.PrevSegment(); prev.Ok() && prev.End() == seg.Start() {
		return prev, mappingGapIterator{}
	}
	return mappingIterator{}, seg.PrevGap()
}
...
func (seg mappingIterator) NextNonEmpty() (mappingIterator, mappingGapIterator) {
	if next := seg.NextSegment(); next.Ok() && next.Start() == seg.End() {
		return next, mappingGapIterator{}
	}
	return mappingIterator{}, seg.NextGap()
}
...
func (gap mappingGapIterator) Range() __generics_imported0.FileRange {
...
func mappingzeroValueSlice(slice []mapping) {

	for i := range slice {
		mappingSetFuncs{}.ClearValue(&slice[i])
	}
}
...
func (s *mappingSet) ExportSlice() []mappingFlatSegment {
	var fs []mappingFlatSegment
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		fs = append(fs, mappingFlatSegment{
			Start: seg.Start(),
			End:   seg.End(),
			Value: seg.Value(),
		})
...
func (s *mappingSet) segmentTestCheck(expectedSegments int, segFunc func(int, __generics_imported0.FileRange, mapping) error) error {
	havePrev := false
	prev := uint64(0)
	nrSegments := 0
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
...
func (s *mappingSet) saveRoot() []mappingFlatSegment {
	fs := s.ExportSlice()
