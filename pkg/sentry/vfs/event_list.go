...
//
//go:nosplit
func (l *eventList) PushFrontList(m *eventList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		eventElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		eventElementMapper{}.linkerFor(m.tail).SetNext(l.head)
...
//
//go:nosplit
func eventRingAdd(old *Event, new *Event) {
	oldLinker := eventElementMapper{}.linkerFor(old)
	newLinker := eventElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func eventRingEmpty(e *Event) bool {
	linker := eventElementMapper{}.linkerFor(e)
