...
type PosixACL struct {
	...
	// Users is an immutable list of named users in the ACL.
	Users []ACLUser

	// Groups is an immutable list of named groups in the ACL.
	Groups []ACLGroup
}
...
func (a *PosixACL) checkPermissions(creds *auth.Credentials, ats AccessTypes, kuid auth.KUID, kgid auth.KGID) bool {
	if creds.EffectiveKUID == kuid {
		// First, are we the owning user?
		if ats.checkPerms(a.UserPerms()) {
			return true
		}
	} else if found, passes := a.checkUserWithMask(creds, ats); found {
		// If we're not the owning user, are we a named user?
		// If so, check against the permissions specified there.
		if passes {
			return true
		}
	} else if found, passes := a.checkGroupsWithMask(creds, ats); creds.InGroup(kgid) || found {
		// Otherwise, are we *either* in the owning group or a named group?
		// If so, if any group grants access, allow the access.
		...
		// Otherwise, use the permissions granted to other.
		return true
	}
...
func ParsePosixACL(src []byte, userns *auth.UserNamespace) (PosixACL, error) {
	...
	seenKUIDs := make(map[auth.KUID]struct{})
	seenKGIDs := make(map[auth.KGID]struct{})
	for _, entry := range acl.Entries {
		...
		case linux.ACL_USER:
			...
			if _, ok := seenKUIDs[kuid]; ok {
				// UIDs must be unique
				return PosixACL{}, linuxerr.EINVAL
			}
			seenKUIDs[kuid] = struct{}{}
		...
		case linux.ACL_GROUP:
			...
			if _, ok := seenKGIDs[kgid]; ok {
				// GIDs must be unique
				return PosixACL{}, linuxerr.EINVAL
			}
			seenKGIDs[kgid] = struct{}{}
