...
//
//go:nosplit
func (l *epollInterestList) PushFrontList(m *epollInterestList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		epollInterestElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		epollInterestElementMapper{}.linkerFor(m.tail).SetNext(l.head)
