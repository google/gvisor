...
var _ = uint8(DevAddrtrackGaps << 7) // Will fail if not zero or one.
...
type DevAddrSet struct {
	root DevAddrnode `state:".([]DevAddrFlatSegment)"`
}
...
func (s *DevAddrSet) LastGap() DevAddrGapIterator {
	...
	return DevAddrGapIterator{n, n.nrSegments}
}
	...
	for {
		...
		for lower < upper {
			...
			if r := n.keys[i]; key < r.End {
				if key >= r.Start {
					return DevAddrIterator{n, i}, DevAddrGapIterator{}
				}
				upper = i
			} else {
				lower = i + 1
		...
		i := lower
		if !n.hasChildren {
			return DevAddrIterator{}, DevAddrGapIterator{n, i}
		}
		n = n.children[i]
	}
}
...
func (s *DevAddrSet) Insert(gap DevAddrGapIterator, r DevAddrRange, val __generics_imported0.PinnedRange) DevAddrIterator {
	if r.Length() <= 0 {
	...
	if prev.Ok() && prev.End() == r.Start {
		if mval, ok := (devAddrSetFuncs{}).Merge(prev.Range(), prev.Value(), r, val); ok {
			shrinkMaxGap := DevAddrtrackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Get()
			...
			if next.Ok() && next.Start() == r.End {
				val = mval
				if mval, ok := (devAddrSetFuncs{}).Merge(prev.Range(), val, next.Range(), next.Value()); ok {
	...
	if next.Ok() && next.Start() == r.End {
		if mval, ok := (devAddrSetFuncs{}).Merge(r, val, next.Range(), next.Value()); ok {
			shrinkMaxGap := DevAddrtrackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Get()
...
func (s *DevAddrSet) InsertWithoutMerging(gap DevAddrGapIterator, r DevAddrRange, val __generics_imported0.PinnedRange) DevAddrIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
func (s *DevAddrSet) InsertWithoutMergingUnchecked(gap DevAddrGapIterator, r DevAddrRange, val __generics_imported0.PinnedRange) DevAddrIterator {
	...
	splitMaxGap := DevAddrtrackGaps != 0 && (gap.node.nrSegments == 0 || gap.Range().Length() == gap.node.maxGap.Get())
	...
	return DevAddrIterator{gap.node, gap.index}
}
...
func (s *DevAddrSet) InsertRange(r DevAddrRange, val __generics_imported0.PinnedRange) DevAddrIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
...
// additional work between finding the gap and insertion, use Insert instead.
func (s *DevAddrSet) TryInsertRange(r DevAddrRange, val __generics_imported0.PinnedRange) DevAddrIterator {
	if r.Length() <= 0 {
		panic(fmt.Sprintf("invalid segment range %v", r))
	}
	seg, gap := s.Find(r.Start)
...
func (s *DevAddrSet) Remove(seg DevAddrIterator) DevAddrGapIterator {
	...
	devAddrSetFuncs{}.ClearValue(&seg.node.values[seg.node.nrSegments-1])
	seg.node.nrSegments--
	if DevAddrtrackGaps != 0 {
		seg.node.updateMaxGapLeaf()
	}
	return seg.node.rebalanceAfterRemove(DevAddrGapIterator{seg.node, seg.index})
}
...
func (s *DevAddrSet) MoveFrom(s2 *DevAddrSet) {
	*s = *s2
...
func (s *DevAddrSet) SplitUnchecked(seg DevAddrIterator, split uint64) (DevAddrIterator, DevAddrIterator) {
	val1, val2 := (devAddrSetFuncs{}).Split(seg.Range(), seg.Value(), split)
	end2 := seg.End()
...
type DevAddrnode struct {
	...
	maxGap DevAddrdynamicGap

	// Nodes store keys and values in separate arrays to maximize locality in
	// the common case (scanning keys for lookup).
	keys     [DevAddrmaxDegree - 1]DevAddrRange
	values   [DevAddrmaxDegree - 1]__generics_imported0.PinnedRange
	children [DevAddrmaxDegree]*DevAddrnode
}

// firstSegment returns the first segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *DevAddrnode) firstSegment() DevAddrIterator {
	for n.hasChildren {
		n = n.children[0]
	}
	return DevAddrIterator{n, 0}
}

// lastSegment returns the last segment in the subtree rooted by n.
//
// Preconditions: n.nrSegments != 0.
func (n *DevAddrnode) lastSegment() DevAddrIterator {
	for n.hasChildren {
		n = n.children[n.nrSegments]
	}
	return DevAddrIterator{n, n.nrSegments - 1}
}
...
func (n *DevAddrnode) nextSibling() *DevAddrnode {
...
// represented by gap.
func (n *DevAddrnode) rebalanceBeforeInsert(gap DevAddrGapIterator) DevAddrGapIterator {
	if n.nrSegments < DevAddrmaxDegree-1 {
		return gap
	}
	if n.parent != nil {
	...
	if n.parent == nil {
		...
		copy(left.keys[:DevAddrminDegree-1], n.keys[:DevAddrminDegree-1])
		copy(left.values[:DevAddrminDegree-1], n.values[:DevAddrminDegree-1])
		copy(right.keys[:DevAddrminDegree-1], n.keys[DevAddrminDegree:])
		copy(right.values[:DevAddrminDegree-1], n.values[DevAddrminDegree:])
		n.keys[0], n.values[0] = n.keys[DevAddrminDegree-1], n.values[DevAddrminDegree-1]
		DevAddrzeroValueSlice(n.values[1:])
		if n.hasChildren {
			copy(left.children[:DevAddrminDegree], n.children[:DevAddrminDegree])
			copy(right.children[:DevAddrminDegree], n.children[DevAddrminDegree:])
			DevAddrzeroNodeSlice(n.children[2:])
			for i := 0; i < DevAddrminDegree; i++ {
		...
		if gap.index < DevAddrminDegree {
			return DevAddrGapIterator{left, gap.index}
		}
	...
	n.parent.nrSegments++
	copy(sibling.keys[:DevAddrminDegree-1], n.keys[DevAddrminDegree:])
	copy(sibling.values[:DevAddrminDegree-1], n.values[DevAddrminDegree:])
	DevAddrzeroValueSlice(n.values[DevAddrminDegree-1:])
	if n.hasChildren {
		copy(sibling.children[:DevAddrminDegree], n.children[DevAddrminDegree:])
		DevAddrzeroNodeSlice(n.children[DevAddrminDegree:])
		for i := 0; i < DevAddrminDegree; i++ {
	...
	return DevAddrGapIterator{sibling, gap.index - DevAddrminDegree}
}
...
// B-tree invariant.
func (n *DevAddrnode) rebalanceAfterRemove(gap DevAddrGapIterator) DevAddrGapIterator {
	for {
		if n.nrSegments >= DevAddrminDegree-1 {
			return gap
		}
		if n.parent == nil {
		...
		if sibling := n.prevSibling(); sibling != nil && sibling.nrSegments >= DevAddrminDegree {
			...
			if gap.node == n {
				return DevAddrGapIterator{n, gap.index + 1}
			}
			return gap
		}
		if sibling := n.nextSibling(); sibling != nil && sibling.nrSegments >= DevAddrminDegree {
			...
			if gap.node == sibling {
				...
				return DevAddrGapIterator{sibling, gap.index - 1}
			}
			return gap
		}
		...
		if p.nrSegments == 1 {
			...
			if gap.node == right {
				return DevAddrGapIterator{p, gap.index + left.nrSegments + 1}
			}
			return gap
		}
		...
		devAddrSetFuncs{}.ClearValue(&p.values[p.nrSegments-1])
...
func (n *DevAddrnode) updateMaxGapLeaf() {
	if n.hasChildren {
...
func (n *DevAddrnode) calculateMaxGapLeaf() uint64 {
	max := DevAddrGapIterator{n, 0}.Range().Length()
	for i := 1; i <= n.nrSegments; i++ {
		if current := (DevAddrGapIterator{n, i}).Range().Length(); current > max {
			max = current
		}
	}
	return max
...
func (n *DevAddrnode) calculateMaxGapInternal() uint64 {
...
func (n *DevAddrnode) searchFirstLargeEnoughGap(minSize uint64) DevAddrGapIterator {
	...
	if n.hasChildren {
		...
		for i := 0; i <= n.nrSegments; i++ {
			currentGap := DevAddrGapIterator{n, i}
...
// in the subtree rooted by n. If not found, return a terminal gap iterator.
func (n *DevAddrnode) searchLastLargeEnoughGap(minSize uint64) DevAddrGapIterator {
	if n.maxGap.Get() < minSize {
		return DevAddrGapIterator{}
	}
	if n.hasChildren {
...
func (seg DevAddrIterator) Value() __generics_imported0.PinnedRange {
...
// invalidate any iterators.
func (seg DevAddrIterator) ValuePtr() *__generics_imported0.PinnedRange {
	return &seg.node.values[seg.index]
}

// SetValue mutates the iterated segment's value. This operation does not
// invalidate any iterators.
func (seg DevAddrIterator) SetValue(val __generics_imported0.PinnedRange) {
...
func (seg DevAddrIterator) PrevSegment() DevAddrIterator {
	...
	if seg.index > 0 {
		return DevAddrIterator{seg.node, seg.index - 1}
	}
	if seg.node.parent == nil {
		return DevAddrIterator{}
	}
	return DevAddrsegmentBeforePosition(seg.node.parent, seg.node.parentIndex)
}
...
func (seg DevAddrIterator) NextSegment() DevAddrIterator {
	...
	return DevAddrsegmentAfterPosition(seg.node.parent, seg.node.parentIndex)
}
...
func (seg DevAddrIterator) PrevGap() DevAddrGapIterator {
	...
	return DevAddrGapIterator{seg.node, seg.index}
}
...
func (seg DevAddrIterator) NextGap() DevAddrGapIterator {
	...
	return DevAddrGapIterator{seg.node, seg.index + 1}
}
...
func (seg DevAddrIterator) PrevNonEmpty() (DevAddrIterator, DevAddrGapIterator) {
	if prev := seg.PrevSegment(); prev.Ok() && prev.End() == seg.Start() {
		return prev, DevAddrGapIterator{}
	}
	return DevAddrIterator{}, seg.PrevGap()
}
...
func (seg DevAddrIterator) NextNonEmpty() (DevAddrIterator, DevAddrGapIterator) {
	if next := seg.NextSegment(); next.Ok() && next.Start() == seg.End() {
		return next, DevAddrGapIterator{}
	}
	return DevAddrIterator{}, seg.NextGap()
}
...
func (gap DevAddrGapIterator) Range() DevAddrRange {
	return DevAddrRange{gap.Start(), gap.End()}
}
...
func (gap DevAddrGapIterator) PrevSegment() DevAddrIterator {
	return DevAddrsegmentBeforePosition(gap.node, gap.index)
}
...
func (gap DevAddrGapIterator) NextSegment() DevAddrIterator {
	return DevAddrsegmentAfterPosition(gap.node, gap.index)
}
...
func (n *DevAddrnode) writeDebugString(buf *bytes.Buffer, prefix string) {
...
func (s *DevAddrSet) ExportSlice() []DevAddrFlatSegment {
	var fs []DevAddrFlatSegment
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		fs = append(fs, DevAddrFlatSegment{
			Start: seg.Start(),
			End:   seg.End(),
			Value: seg.Value(),
		})
...
func (s *DevAddrSet) segmentTestCheck(expectedSegments int, segFunc func(int, DevAddrRange, __generics_imported0.PinnedRange) error) error {
	havePrev := false
	prev := uint64(0)
	nrSegments := 0
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
...
func (s *DevAddrSet) countSegments() (segments int) {
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		segments++
	}
	return segments
