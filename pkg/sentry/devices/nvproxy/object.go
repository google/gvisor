...
type object struct {
	...
	// relationships as equivalent to other dependencies.
...
type objectImpl interface {
	...
	// is freed. It returns a function that must be called after nvproxy locks
	// are released.
	//
...
func (nvp *nvproxy) objAdd(ctx context.Context, client *rootClient, h nvgpu.Handle, c nvgpu.ClassID, oi objectImpl, parentH nvgpu.Handle, deps ...nvgpu.Handle) {
	...
	if client == nil {
		log.Traceback("nvproxy: new object %v (class %v) has no client", h, c)
		return
	}
...
func (nvp *nvproxy) objFree(ctx context.Context, client *rootClient, h nvgpu.Handle) []func() {
	if client == nil {
		ctx.Warningf("nvproxy: freeing object handle %v with unknown client", h)
		return nil
	}
...
func (o *osDescMem) Release(ctx context.Context) func() {
	if o.m != 0 {
		// This is a time-taking operation that can be done in the background.
		go func() {
			unix.RawSyscall(unix.SYS_MUNMAP, o.m, o.len, 0)
		}()
	}
	// Unpin pages (which takes MM locks) without holding nvproxy locks. This
	// changes pgalloc.MemoryFile refcounts and needs to synchronize with
	// checkpoint/restore. Hence, do it synchronously.
	return func() {
