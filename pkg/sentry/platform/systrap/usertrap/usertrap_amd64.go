...
// trapSize is the size of one trap that must be a power of two.
// The trap table is page aligned, so a power of two stride makes every trap
// entry trapSize aligned, and is what lets RewindTrampolineIP find the
// beginning of a trap from an address inside it without knowing where
// the table has been mapped first.
const trapSize = 64

const (
	// trapCodeOffset is the offset of the first instruction of a trampoline
	// within its trap entry. The eight bytes before it hold the address of
	// the offset instruction.
	trapCodeOffset = 8

	// trapCodeSize is the size of the trampoline code generated by
	// addTrapLocked.
	trapCodeSize = 44
)
...
func (s *State) newTrapLocked(ctx context.Context, mm memoryManager) (hostarch.Addr, error) {
	...
	// trapSize is a power of two and the table is page aligned, so a trap should
	// never straddle a page boundary.
	addr := s.trapAddr(trap)
	if addr/hostarch.PageSize != (addr+trapSize-1)/hostarch.PageSize {
		return 0, fmt.Errorf("trap %d at %#x straddles a page boundary", trap, addr)
	}
	hdr = header{
		nextTrap: s.nextTrap,
	}
...
// RewindTrampolineIP rewinds ip to the beginning of the syscall trampoline if it
// is in the trampoline.
//
// The address of the trampoline's first instruction is derived from ip by
// alignment, so this doesn't need the trap table lock, and it works for all
// address spaces, including those that have just been forked and have
// yet to load the trap table.
func RewindTrampolineIP(ip uintptr) (uintptr, bool) {
	if !trapTableAddrRange.Contains(hostarch.Addr(ip)) {
		return ip, false
	}
	// The app can't map anything in trapTableAddrRange because it is
	// below MinUserAddress, so ip can only be in the trap table.
	start := ip&^uintptr(trapSize-1) + trapCodeOffset
	if ip <= start || ip >= start+trapCodeSize {
		return ip, false
	}
	return start, true
}
