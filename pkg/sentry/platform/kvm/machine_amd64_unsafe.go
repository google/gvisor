...
func (c *vCPU) setUserRegisters(uregs *userRegs) unix.Errno {
	if errno := rawsyscall.SyscallErrno(
		unix.SYS_IOCTL,
		uintptr(c.fd),
		KVM_SET_REGS,
...
func (c *vCPU) getUserRegisters(uregs *userRegs) unix.Errno {
	if errno := rawsyscall.SyscallErrno( // escapes: no.
		unix.SYS_IOCTL,
		uintptr(c.fd),
		KVM_GET_REGS,
		uintptr(unsafe.Pointer(uregs))); errno != 0 {
...
func seccompMmapSyscall(context unsafe.Pointer) (uintptr, uintptr, unix.Errno) {
	...
	addr, _, e := rawsyscall.Syscall6(uintptr(ctx.Rax), uintptr(ctx.Rdi), uintptr(ctx.Rsi),
