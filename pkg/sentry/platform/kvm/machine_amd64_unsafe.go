...
func (c *vCPU) setPAT() error {
	...
	registers.entries[0].data |= _PA_WB << (hostarch.MemoryTypeWriteBack * 8)
	registers.entries[0].data |= _PA_WC << (hostarch.MemoryTypeWriteCombine * 8)
	registers.entries[0].data |= _PA_UC << (hostarch.MemoryTypeUncached * 8)
	if errno := hostsyscall.RawSyscallErrno(
		unix.SYS_IOCTL,
		uintptr(c.fd),
		KVM_SET_MSRS,
