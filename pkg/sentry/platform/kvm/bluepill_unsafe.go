...
func printHex(title []byte, val uint64) {
	...
	str[0] = ' '
	str[17] = '\n'
	rawsyscall.SyscallErrno(unix.SYS_WRITE, uintptr(unix.Stderr), uintptr(unsafe.Pointer(&title[0])), uintptr(len(title)))
	rawsyscall.SyscallErrno(unix.SYS_WRITE, uintptr(unix.Stderr), uintptr(unsafe.Pointer(&str)), 18)
}
