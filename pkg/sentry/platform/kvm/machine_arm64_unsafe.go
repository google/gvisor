...
func updateVectorTable() {
	...
	// We only need to move the 1st 2K part of the code.
	// Currently, I filled the 2nd 2K part of vector-table with NOP(no operation) instructions.
	var entry *uint32
	var entry_from *uint32
	for i := 1; i <= 0x200; i++ {
		entry = (*uint32)(unsafe.Pointer(toLocation + 0x800 - uintptr(i)*4))
		entry_from = (*uint32)(unsafe.Pointer(fromLocation + 0x800 - uintptr(i)*4))
		*entry = *entry_from
	}
