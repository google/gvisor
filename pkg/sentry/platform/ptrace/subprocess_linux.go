...
func forkStub(flags uintptr, instrs []bpf.Instruction) (*thread, error) {
	...
	// will go on to run the sandboxed application's code under ptrace,
	// including code restored from a checkpoint taken in a different
	// process with different (kernel-randomized, per-process) PAC keys;
	// without this, a PAC-signed pointer from before checkpoint can fail
	// authentication after restore and raise SIGILL. See pac.DisableHostPAC
	// for the full rationale. This must run before the seccomp filter
	// below is installed, since the filter does not allow the prctl(2)
	// call this performs.
	pac.DisableHostPAC()
