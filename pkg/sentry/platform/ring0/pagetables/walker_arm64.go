...
//
// Precondition: the start, end must be within a canonical range.
//
//go:nosplit
func IterateRangeCanonical(pageTables *PageTables, visitor Visitor, start, end uintptr) {
	pgdEntryIndex := pageTables.root
	if start >= upperBottom {
		pgdEntryIndex = pageTables.archPageTables.root
	}
	...
	for pgdIndex := (uint16((start & pgdMask) >> pgdShift)); start < end && pgdIndex < entriesPerPage; pgdIndex++ {
		...
		if !pgdEntry.Valid() {
			...
			// Allocate a new pgd.
			pudEntries = pageTables.Allocator.NewPTEs()
			pgdEntry.setPageTable(pageTables.pudEntries)
		} else {
			pudEntries = pageTables.Allocator.LookupPTEs(pgdEntry.Address())
		}
		...
		for pudIndex := uint16((start & pudMask) >> pudShift); start < end && pudIndex < entriesPerPage; pudIndex++ {
			...
			if !pudEntry.Valid() {
				...
				if start&(pudSize-1) == 0 && end-start >= pudSize {
					pudEntry.SetSect()
					visitor.visit(uintptr(start), pudEntry, pudSize-1)
					if pudEntry.Valid() {
						start = next(start, pudSize)
						continue
					}
				...
				// Allocate a new pud.
				pmdEntries = pageTables.Allocator.NewPTEs()
				pudEntry.setPageTable(pageTables.pmdEntries)

			} else if pudEntry.IsSect() {
				// Does this page need to be split?
				if visitor.requiresSplit() && (start&(pudSize-1) != 0 || end < next(start, pudSize)) {
				...
				pmdEntries = pageTables.Allocator.LookupPTEs(pudEntry.Address())
			}
			...
			for pmdIndex := uint16((start & pmdMask) >> pmdShift); start < end && pmdIndex < entriesPerPage; pmdIndex++ {
				...
				if !pmdEntry.Valid() {
					...
					if start&(pmdSize-1) == 0 && end-start >= pmdSize {
						pmdEntry.SetSect()
						visitor.visit(uintptr(start), pmdEntry, pmdSize-1)
						if pmdEntry.Valid() {
							start = next(start, pmdSize)
							continue
						}
					...
					// Allocate a new pmd.
					pteEntries = pageTables.Allocator.NewPTEs()
					pmdEntry.setPageTable(pageTables.pteEntries)

				} else if pmdEntry.IsSect() {
					// Does this page need to be split?
					if visitor.requiresSplit() && (start&(pmdSize-1) != 0 || end < next(start, pmdSize)) {
						// Install the relevant entries.
						pteEntries = pageTables.Allocator.NewPTEs()
						for index := uint16(0); index < entriesPerPage; index++ {
					...
					pteEntries = pageTables.Allocator.LookupPTEs(pmdEntry.Address())
				}
				...
				for pteIndex := uint16((start & pteMask) >> pteShift); start < end && pteIndex < entriesPerPage; pteIndex++ {
					...
					if !pteEntry.Valid() && !visitor.requiresAlloc() {
						clearPTEEntries++
						start += pteSize
						continue
					}
					...
					visitor.visit(uintptr(start), pteEntry, pteSize-1)
					if !pteEntry.Valid() {
						if visitor.requiresAlloc() {
				...
				if clearPTEEntries == entriesPerPage {
					pmdEntry.Clear()
					pageTables.Allocator.FreePTEs(pteEntries)
					clearPMDEntries++
				}
			}
			...
			if clearPMDEntries == entriesPerPage {
				pudEntry.Clear()
				pageTables.Allocator.FreePTEs(pmdEntries)
				clearPUDEntries++
			}
		}
		...
		if clearPUDEntries == entriesPerPage {
			pgdEntry.Clear()
			pageTables.Allocator.FreePTEs(pudEntries)
		}
	}
}
