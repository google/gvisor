...
// Pagetable for user-space.
//go:nosplit
func (p *PageTables) TTBR0_EL1(noFlush bool, asid uint16) uint64 {
	return uint64(p.rootPhysical) | (uint64(asid)&ttbr_asid_mask)<<ttbr_asid_offset
}

// Pagetable for kernel-space.
//go:nosplit
func (p *PageTables) TTBR1_EL1(noFlush bool, asid uint16) uint64 {
	return uint64(p.archPageTables.rootPhysical) | (uint64(asid)&ttbr_asid_mask)<<ttbr_asid_offset
}
...
const (
	...
	pte_table_bit = 0x1 << 1
	pte_type_mask = 0x3 << 0
	present       = pte_valid | pte_table_bit
	user          = 0x1 << 6 /* AP[1] */
	rdonly        = 0x1 << 7 /* AP[2] */
	accessed      = 0x1 << 10
	dbm           = 0x1 << 51
...
const (
	...
	prot_default   = accessed | shared | mt_normal
)
...
func (p *PTE) SetSect() {
	...
	atomic.StoreUintptr((*uintptr)(p), type_sect)
}

// IsSect returns true if this page is a sect page.
//
//go:nosplit
func (p *PTE) IsSect() bool {
	return atomic.LoadUintptr((*uintptr)(p))&pte_type_mask == type_sect
}
...
func (p *PTE) Set(addr uintptr, opts MapOpts) {
	...
	v := (addr &^ optionMask) | prot_default | nG | rdonly
...
func (p *PTE) setPageTable(pt *PageTables, ptes *PTEs) {
	...
	v := addr | type_table | prot_default
	atomic.StoreUintptr((*uintptr)(p), v)
}
