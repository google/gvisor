...
// or the init container if no ID is provided. Caller must DecRef the returned mntns when done.
func (f *Fs) mountNamespaceForContainer(containerID string) (*vfs.MountNamespace, error) {
	...
	if mntns == nil {
		return nil, fmt.Errorf("mount namespace is nil for %s", containerID)
	}
	return mntns, nil
}
