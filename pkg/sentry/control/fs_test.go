...
func TestFdReader_Seekable(t *testing.T) {
	...
	fd, cleanup := setupMockFD(ctx, t, data /* isSeekable= */, true)
	defer cleanup()
	...
	if n != 5 || string(buf0) != "hello" {
		t.Fatalf("got %d bytes '%s', want 5 bytes 'hello'", n, string(buf0))
	}
	...
	if n != 5 || string(buf6[:n]) != "world" {
		t.Fatalf("got %d bytes '%s', want 5 bytes 'world'", n, string(buf6[:n]))
	}
...
func TestFdReader_NonSeekable(t *testing.T) {
	ctx := contexttest.Context(t)
	data := []byte("hello world")
	fd, cleanup := setupMockFD(ctx, t, data /* isSeekable= */, false)
	defer cleanup()
	...
	if n != 11 || string(buf0[:n]) != "hello world" {
		t.Fatalf("got %d bytes '%s', want 11 bytes 'hello world'", n, string(buf0[:n]))
	}
