...
type masqueradeTarget struct {
	...
	// as received from iptables. MASQUERADE revision 0 has multiple on-wire layouts,
	// including nf_nat_ipv4_multi_range_compat (56B) and the larger nf_nat_range
	// layout emitted by kube-proxy's `--random-fully` rules. We never interpret the
	// range (the source address comes from the egress interface), so we retain the
	// raw bytes to marshal them back verbatim for a faithful iptables-save.
	raw []byte
}
...
func (*masqueradeTargetMaker) unmarshal(buf []byte, filter stack.IPHeaderFilter) (target, *syserr.Error) {
	...
	// iptables sends several range layouts (nf_nat_ipv4_multi_range_compat and
	// nf_nat_range, the latter carrying kube-proxy's --random-fully flag), so we do
	// NOT parse or validate the range;
