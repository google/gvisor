...
func (p *Protocol) getChain(nft *nftables.NFTables, attrs map[uint16]nlmsg.BytesView, family stack.AddressFamily, msgFlags uint16, ms *nlmsg.MessageSet) *syserr.AnnotatedError {
	...
	if chain.IsBaseChain() {
		err := dumpBaseChainHook(chain, family, m)
		if err != nil {
			return err
		}
...
// dumpBaseChainHook creates a NFTA_CHAIN_HOOK attribute with all the corresponding nested attributes.
func dumpBaseChainHook(chain *nftables.Chain, family stack.AddressFamily, m *nlmsg.Message) *syserr.AnnotatedError {
	baseChainInfo := chain.GetBaseChainInfo()
