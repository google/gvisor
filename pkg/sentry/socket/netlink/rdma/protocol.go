...
// Meanwhile, rNIC continues to DMA into the same physical page, now in the
// child's address space. libibverbs addresses this issue with ibv_fork_init,
// but it has multiple problems (see man pages). Linux 5.12 made it so that
// DMA-pinned pages would be copied on fork, solving this issue, but gVisor
// currently doesn't implement this. The SYS_GET request is used to see if
// copy-on-fork support exists, so we always return 0 inside gVisor.
