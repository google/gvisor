...
// dumpLinks handles RTM_GETLINK requests.
...
// dumpAddrs handles RTM_GETADDR requests.
...
// fillRoute returns the Route using LPM algorithm.
// Refer to Linux's net/ipv4/route.c:rt_fill_info().
...
// parseForDestination parses a message as format of RtMsg-RtAttr-dst.
func parseForDestination(data []byte) ([]byte, *syserr.Error) {
	var rtMsg linux.RtMsg
	if len(data) < linux.SizeofRtMsg {
		return []byte{}, syserr.ErrInvalidArgument
	}
	binary.Unmarshal(data[:linux.SizeofRtMsg], usermem.ByteOrder, &rtMsg)
	...
	if rtMsg.Flags != 0 && rtMsg.Flags != linux.RTM_F_LOOKUP_TABLE {
		return []byte{}, syserr.ErrNotSupported
	}

	data = data[linux.SizeofRtMsg:]
