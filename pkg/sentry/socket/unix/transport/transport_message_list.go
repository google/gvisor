...
//
//go:nosplit
func (l *messageList) PushFrontList(m *messageList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		messageElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		messageElementMapper{}.linkerFor(m.tail).SetNext(l.head)
