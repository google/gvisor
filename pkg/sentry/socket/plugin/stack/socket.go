...
func (s *socketOperations) Readiness(mask waiter.EventMask) waiter.EventMask {
	...
	for {
		...
		// Use the cache only for requests with IN/OUT events, and only if
		// all of those events are present.
		if iomask == 0 || ready&iomask != iomask {
		...
		if s.eventInfo.Ready.CompareAndSwap(ready, ready&^iomask) {
