...
func int2err(from int) *syserr.Error {
	if from >= 0 {
		return nil
	}
...
func translateReturn(ret int64) (uint64, error) {
	if ret < 0 {
		return 0, int2err(int(ret)).ToError()
	} else if ret == 0 {
		return 0, nil
	} else {
...
func copyAddrOut(ifr *linux.IFReq, ifaceAddr *inet.InterfaceAddr) {
	...
	if ifaceAddr.Family == linux.AF_INET {
		...
		copy(ifr.Data[4:20], ifaceAddr.Addr[:16])
	}
}
...
func iovecsFromBlockSeq(bs safemem.BlockSeq, rw *pluginStackRW) []syscall.Iovec {
	...
	if rw != nil {
		iovs = rw.iovs[:0] // Use the old buffer, and set length to zero.
	}
