...
// time. Only one goroutine may call any of StartRead, StartReadv, or Wait at a
// time. However, RegisterDestinationFD and any one of StartRead, StartReadv,
// or Wait may be called concurrently. Close may not be called concurrently
// with any other methods, and no methods may be called after Close.
type AsyncReader interface {
	...
	// passed in calls to StartReadv, which must be strictly positive. All
	// calls to MaxRanges for a given AsyncReader must return the same value.
	MaxRanges() int
	...
	// NeedRegisterDestinationFD returns false, a DestinationFile of nil may be
	// passed to calls to StartRead and StartReadv. All calls to
	...
	// StartRead enqueues a read of size dstFR.Length() bytes, from the file
	...
	StartRead(id int, off int64, dstFile DestinationFile, dstFR memmap.FileRange, dstMap []byte)

	// StartReadv enqueues a read of size total, from the file starting at the
	// given offset, to the dstFile ranges in dstFRs. dstMaps must be a mapping
	// of dstFRs. The AsyncReader may retain dstFRs and dstMaps, which should
	// not be mutated by the caller, until the corresponding completion is
	// returned by Wait.
	//
	// Preconditions:
	// - 0 <= id < MaxParallel().
	...
	// - No previous call to Wait() has returned a non-nil error.
	StartReadv(id int, off int64, total uint64, dstFile DestinationFile, dstFRs []memmap.FileRange, dstMaps []unix.Iovec)
	...
	// enqueued reads until a call to Wait.
	//
	// Preconditions:
...
type Completion struct {
	// ID is the ID passed to AsyncReader.StartRead or AsyncReader.StartReadv.
	ID int
