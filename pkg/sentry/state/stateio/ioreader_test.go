...
func TestIOReaderRead(t *testing.T) {
	...
	for done < dataLen {
		for ids != ^uint32(0) && off < dataLen {
			...
			ids |= uint32(1) << id
			r.StartRead(id, int64(off), nil, memmap.FileRange{uint64(off), uint64(off) + chunkSize}, buf[off:off+chunkSize])
			off += chunkSize
		}
...
func TestIOReaderReadv(t *testing.T) {
	...
	for done < dataLen {
		for ids != ^uint32(0) && off < dataLen {
			...
			r.StartReadv(id, int64(off), 2*chunkSize, nil, frs, iovecs)
			off += 2 * chunkSize
		}
		cs, err := r.Wait(cs[:0], 1 /* minCompletions */)
		if err != nil {
