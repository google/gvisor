...
func NewIOReader(src io.ReaderAt, maxReadBytes uint64, maxRanges, maxParallel int) *IOReader {
	...
	r := &IOReader{
		...
		maxRanges:    maxRanges,
		subs:         make(chan Submission, maxParallel),
...
func (r *IOReader) StartRead(id int, off int64, dstFile DestinationFile, dstFR memmap.FileRange, dstMap []byte) {
	r.subs <- Submission{
		ID:     id,
		Offset: off,
		// We don't use Total.
...
func (r *IOReader) StartReadv(id int, off int64, total uint64, dstFile DestinationFile, dstFRs []memmap.FileRange, dstMaps []unix.Iovec) {
	r.subs <- Submission{
		ID:     id,
		Offset: off,
		// We don't use Total.
...
func (r *IOReader) workerMain() {
	...
	for {
		...
		case sub := <-r.subs:
			var done uint64
			var doneErr error
			for _, dst := range sub.Mappings {
				n, err := r.src.ReadAt(dst, sub.Offset)
				sub.Offset += int64(n)
				done += uint64(n)
				if err != nil {
					doneErr = err
