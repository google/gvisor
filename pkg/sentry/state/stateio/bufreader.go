...
// maximum size is unspecified. If NewBufReader succeeds, it takes ownership of
// ar.
func NewBufReader(ar AsyncReader, size int) (*BufReader, error) {
	bytesPerChunk, numChunks := getBufReaderParams(ar.MaxReadBytes(), ar.MaxParallel(), size)
	bufSize := bytesPerChunk * numChunks
	memfd, buf, err := CreateMemFD("stateio.BufReader", bufSize)
	if err != nil {
		return nil, err
	}
...
func getBufReaderParams(maxReadBytes64 uint64, maxParallel, maxSize int) (bytesPerChunk, numChunks int) {
	// Arbitrary limits:
	const (
		maxChunks     = 32
		maxChunkBytes = 32 << 20
	)
	maxReadBytes := int(min(maxReadBytes64, maxChunkBytes))
	maxParallel = min(maxParallel, maxChunks)
	if maxSize <= 0 {
		return maxReadBytes, maxParallel
	}
	// Can we accomodate at least two reads of size maxReadBytes? We don't want
	// to set the read size below maxReadBytes unless necessary, and two is the
	// minimum number of chunks required to have any amount of prefetching.
	if maxReadBytes*2 <= maxSize {
		return maxReadBytes, min(maxSize/maxReadBytes, maxParallel)
	}
	if maxSize > 1 && maxParallel > 1 {
		return maxSize / 2, 2
	}
	// maxSize == 1 || maxParallel == 1:
	return min(maxReadBytes, maxSize), 1
}
	...
	for {
		for {
			...
			if chunk.readyLen == 0 && chunk.err == nil {
				// Wait for this chunk to become ready.
				if r.waitErr != nil {
					return done, r.waitErr
				}
				break
			}
			r.bufOff = r.chunkLen * r.curChunk
			r.bufEnd = r.bufOff + chunk.readyLen
			r.bufErr = chunk.err
		...
		if !r.stopReading && r.chunksRefillable != 0 {
			...
			for range r.chunksRefillable {
				...
				r.ar.StartRead(chunkToRefill, r.refillOffset, r.df, memmap.FileRange{uint64(chunkStart), uint64(chunkEnd)}, r.buf[chunkStart:chunkEnd])
				chunkToRefill++
