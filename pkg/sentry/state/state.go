...
func (opts *SaveOpts) Save(ctx context.Context, k *kernel.Kernel, w *watchdog.Watchdog) error {
	...
	if err != nil {
		...
		// Save the kernel.
		err = k.SaveTo(ctx, wc, opts.PagesMetadata, opts.PagesFile, opts.AppMFExcludeCommittedZeroPages, opts.PrivateMFExternalContent, opts.Resume, opts.FilestoreSnapshots, opts.FilestoreSidecar) // transfers ownership of wc, opts.PagesMetadata, opts.PagesFile
