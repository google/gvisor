...
//
//go:nosplit
func (l *sessionList) PushFrontList(m *sessionList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		sessionElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		sessionElementMapper{}.linkerFor(m.tail).SetNext(l.head)
...
//
//go:nosplit
func sessionRingInit(e *Session) {
	linker := sessionElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func sessionRingAdd(old *Session, new *Session) {
	oldLinker := sessionElementMapper{}.linkerFor(old)
	newLinker := sessionElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func sessionRingRemove(e *Session) {
	eLinker := sessionElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func sessionRingEmpty(e *Session) bool {
	linker := sessionElementMapper{}.linkerFor(e)
