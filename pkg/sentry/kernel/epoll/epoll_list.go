...
//
//go:nosplit
func (l *pollEntryList) PushFrontList(m *pollEntryList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		pollEntryElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		pollEntryElementMapper{}.linkerFor(m.tail).SetNext(l.head)
