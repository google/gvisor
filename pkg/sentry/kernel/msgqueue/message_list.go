...
//
//go:nosplit
func msgRingAdd(old *Message, new *Message) {
	oldLinker := msgElementMapper{}.linkerFor(old)
	newLinker := msgElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func msgRingEmpty(e *Message) bool {
	linker := msgElementMapper{}.linkerFor(e)
