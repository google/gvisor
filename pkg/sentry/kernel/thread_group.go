...
type ThreadGroup struct {
	...
	// These help serialize execve(2) with PTRACE_ATTACH. Since checkpointing precludes holding onto
	// a sync.Mutex across taskRunStates, and execve(2) spans multiple taskRunStates, we implement
	// locking by way of the ptraceExecveMutexStop internal stop.
	// See Task.ptraceExecveMutexStartLock().
	ptraceExecveMutexMu      sync.Mutex `state:"nosave"`
	ptraceExecveMutexLocked  bool
	ptraceExecveMutexWaiters map[*Task]struct{}
}
...
func (k *Kernel) NewThreadGroup(pidns *PIDNamespace, sh *SignalHandlers, terminationSignal linux.Signal, limits *limits.LimitSet) *ThreadGroup {
	...
	tg := &ThreadGroup{
		...
		ptraceExecveMutexWaiters: make(map[*Task]struct{}),
		ioUsage:                  &usage.IO{},
		limits:                   limits,
	}
