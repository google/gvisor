...
func (t *Task) Clone(args *linux.CloneArgs) (ThreadID, *SyscallControl, error) {
	...
	if seccheck.Enabled(seccheck.PointClone) {
		req := seccheck.CloneReq()
		info := seccheck.CloneInfo{
			Credentials: creds,
			NewThreadID: int32(tid),
		}
		var mask seccheck.CloneFieldSet
		mask.Add(seccheck.CloneFieldCredentials)
		mask.Add(seccheck.CloneFieldNewThreadID)
		t.k.tasks.mu.RLock()
		if req.Contains(seccheck.CloneFieldThreadID) {
			info.ThreadID = int32(t.k.tasks.Root.tids[t])
			mask.Add(seccheck.CloneFieldThreadID)
		}
		if req.Contains(seccheck.CloneFieldThreadGroupID) {
			info.ThreadGroupID = int32(t.k.tasks.Root.tgids[t.tg])
			mask.Add(seccheck.CloneFieldThreadGroupID)
		}
		if req.Contains(seccheck.CloneFieldThreadGroupStartTime) {
			info.ThreadGroupStartTime = t.tg.leader.startTime
			mask.Add(seccheck.CloneFieldThreadGroupStartTime)
		}
		if req.Contains(seccheck.CloneFieldNewThreadGroupID) {
			info.NewThreadGroupID = int32(t.k.tasks.Root.tgids[nt.tg])
			mask.Add(seccheck.CloneFieldNewThreadGroupID)
		}
		if req.Contains(seccheck.CloneFieldNewStartTime) {
			info.NewStartTime = nt.startTime
			mask.Add(seccheck.CloneFieldNewStartTime)
		}
		t.k.tasks.mu.RUnlock()
		if req.Contains(seccheck.CloneFieldArgs) {
			info.Args = *args
			mask.Add(seccheck.CloneFieldArgs)
		}
		if err := seccheck.Clone(t, mask, &info); err != nil {
