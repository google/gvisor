...
func loadPrivateMemoryFiles(ctx context.Context, r io.Reader, mfmap map[checkpoint.ResourceID]*pgalloc.MemoryFile, opts *pgalloc.LoadOpts) error {
	...
	if FSRestoreFromContext(ctx) {
		...
		for fsID := range mfmap {
			...
			if fsCheckpointed != nil {
				_, inFS = fsCheckpointed[cleanID]
			}
			_, inSentry := ownersMap[cleanID]
			if !inFS && !inSentry {
				return fmt.Errorf("private memory file %q was neither in FS checkpoint nor in Sentry checkpoint", cleanID)
			}
		}
		...
		for _, fsID := range meta.owners {
			...
			mf, ok := mfmap[cleanID]
			if !ok {
				return fmt.Errorf("saved private memory file for %q was not configured on restore", cleanID)
			}
			if _, inFS := fsCheckpointed[cleanID]; inFS {
				discardOpts := *opts
				discardOpts.Discard = true
				if err := mf.LoadFrom(ctx, r, &discardOpts); err != nil {
					return fmt.Errorf("failed to discard MemoryFile %p fsID %q: %w", mf, cleanID, err)
				}
			...
			if err != nil {
				return fmt.Errorf("failed to load MemoryFile %p fsID %q from Sentry state: %w", mf, cleanID, err)
			}
		}
		return nil
	}
	...
	for _, fsID := range meta.owners {
		...
		err := mf.LoadFrom(ctx, r, opts)
		if err != nil {
			return fmt.Errorf("failed to load MemoryFile %p fsID %q: %w", mf, cleanID, err)
		}
	}
	return nil
}
