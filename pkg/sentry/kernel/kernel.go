...
func (k *Kernel) SaveTo(ctx context.Context, w wire.Writer, pw io.Writer) error {
	saveStart := time.Now()
...
func (k *Kernel) LoadFrom(ctx context.Context, r wire.Reader, pr io.Reader, timeReady chan struct{}, net inet.Stack, clocks sentrytime.Clocks, vfsOpts *vfs.CompleteRestoreOptions) error {
	loadStart := time.Now()
