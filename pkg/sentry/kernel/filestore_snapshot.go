...
// written next to in-window filestore snapshots. The schema is shared with
// runsc/container's adoption-time verification (filestore_snapshot.go there).
...
func (k *Kernel) snapshotFilestores(mfsToSave map[checkpoint.ResourceID]*pgalloc.MemoryFile, snapshots []checkpoint.FilestoreSnapshot) ([]filestoreSidecarEntry, error) {
	...
	entries := make([]filestoreSidecarEntry, 0, len(snapshots))
	for _, s := range snapshots {
		mf := mfsToSave[s.ID]
		if mf == nil {
		...
		entries = append(entries, filestoreSidecarEntry{
			File:              s.Name,
