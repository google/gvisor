...
//
//  1. Acquiring a lock that serializes with a concurrent PTRACE_ATTACH, for correct creds computation.
//     * The next taskRunState is runSyscallAfterPtraceLock
//     * The task will wait in the ptraceExecveMutexStop stop till it can acquire the lock.
//  2. Killing all other tasks in its thread group and switching to the new TaskImage.
//     * The next run state is runSyscallAfterExecStop
//     * The task will wait in the execStop stop till it its siblings have exited.
//
...
func (t *Task) Execve(argv, envv []string, flags int32, pathname string, executable *vfs.FileDescription, closeOnExec bool) *SyscallControl {
	...
	t.ptraceExecveMutexStartLock(t.tg)
...
// The runSyscallAfterPtraceLock state continues execve(2) after the lock serializing
// PTRACE_ATACH and execve has been acquired.
//
// +stateify savable
type runSyscallAfterPtraceLock struct {
	argv, envv  []string
	pathname    string
	flags       int32
...
func (r *runSyscallAfterPtraceLock) execute(t *Task) taskRunState {
	defer func() {
		if r.executable != nil {
	...
	if t.killed() {
		// UnlockPtraceExecveMutex() will not pass us the lock after we were killed, so there is no
		// race in loading the ptraceExecveMutexOwner value without a lock.
		if t.ptraceExecveMutexOwner != nil {
			t.ptraceExecveMutexUnlock()
		}
	...
	// Cannot gain privileges if the FSContext is shared outside of the thread group.
	stopPrivGain = stopPrivGain || t.fsContext.CheckAndPreventSharingOutsideTG(t.tg)
...
func (r *runSyscallAfterPtraceLock) execveWithImage(t *Task, newImage *TaskImage, newCreds *auth.Credentials, secureExec bool) taskRunState {
	...
	cu.Release()
	return &runSyscallAfterExecStop{newImage, newCreds, secureExec}
}
...
func (r *runSyscallAfterExecStop) execute(t *Task) taskRunState {
	...
	if r.newCreds.EffectiveKUID != oldCreds.EffectiveKUID ||
		...
		!oldCreds.PermittedCaps.IsSubsetOf(r.newCreds.PermittedCaps) {
		r.image.MemoryManager.SetDumpability(mm.NotDumpable) // suid_dumpable is not implemented
		t.parentDeathSignal = 0
	}
...
// ptraceExecveMutexStop is a TaskStop that a task enters if it fails to acquire the lock that
// serializes PTRACE_ATACH and execve immediately. The task exits the stop when the lock owner
// eventually calls endInternalStop on it in Task.ptraceExecveMutexUnlock.
//
// +stateify savable
type ptraceExecveMutexStop struct{}

// Killable implements TaskStop.Killable.
func (*ptraceExecveMutexStop) Killable() bool { return true }

// ptraceExecveMutexStartLock attempts to acquire the lock that serializes an execve(2) with a
// PTRACE_ATTACH. Such an exclusion is required to ensure correct creds computation in execve(2).
// Will push the calling Task into the ptraceExecveMutexStop stop if the lock is busy.
...
func (t *Task) ptraceExecveMutexStartLock(tg *ThreadGroup) {
	tg.ptraceExecveMutexMu.Lock()
	defer tg.ptraceExecveMutexMu.Unlock()
	t.tg.signalHandlers.mu.Lock()
	defer t.tg.signalHandlers.mu.Unlock()
	if t.ptraceExecveMutexOwner != nil {
		panic("Task already holds a ptraceExecveMutex lock")
	}
	if t.killedLocked() {
		return // No point in taking the lock when we'd give it up first thing come the next runState.
	}
	if !tg.ptraceExecveMutexLocked {
		tg.ptraceExecveMutexLocked = true
		t.ptraceExecveMutexOwner = tg
		return
	}
	t.beginInternalStopLocked((*ptraceExecveMutexStop)(nil))
	tg.ptraceExecveMutexWaiters[t] = struct{}{}
}

// ptraceExecveMutexUnlock releases the mutex that serializes an execve(2) with a PTRACE_ATTACH.
// The caller must have called ptraceExecveMutexStartLock() earlier. If there are tasks waiting on
// the lock, one will be pulled out of the ptraceExecveMutexStop stop it is in and handed the lock.
//
// Preconditions: Must be called from the task goroutine. The caller must have called
// ptraceExecveMutexStartLock() earlier.
func (t *Task) ptraceExecveMutexUnlock() {
	if t.ptraceExecveMutexOwner == nil {
		panic("calling Task does not hold hold any ptraceExecveMutex lock")
	}
	tg := t.ptraceExecveMutexOwner

	tg.ptraceExecveMutexMu.Lock()
	defer tg.ptraceExecveMutexMu.Unlock()
	if !tg.ptraceExecveMutexLocked {
		panic("unlock of unlocked ThreadGroup ptraceExecveMutex")
	}
	tg.ptraceExecveMutexLocked = false
	t.ptraceExecveMutexOwner = nil

	// Find a waiting task whose ptraceExecveMutexStop hasn't been killed and pass on the lock.
	for t2 := range tg.ptraceExecveMutexWaiters {
		delete(tg.ptraceExecveMutexWaiters, t2)
		if t.passPtraceExecveMutexLocked(tg, t2) {
			break
		}
	}
}

// Tries to pass the ptraceExecveMutex lock in tg to t2. Returns true if the lock was passed.
//
// preconditions: tg.ptraceExecveMutexMu must be locked.
func (t *Task) passPtraceExecveMutexLocked(tg *ThreadGroup, t2 *Task) bool {
	...
	// allowing the post-kill taskRunState::execute() to access Task.ptraceExecveMutexOwner
	// without anything racing with it.
	if _, ok := t2.stop.(*ptraceExecveMutexStop); ok {
		tg.ptraceExecveMutexLocked = true // tg.ptraceExecveMutexMu being locked is a precondition
		if t2.ptraceExecveMutexOwner != nil {
			panic("Task already holds a ptraceExecveMutex lock")
		}
		// The write to t2.ptraceExecveMutexOwner is safe because we know t2 is in a stop and we hold
		// the signalHandlers.mu lock, so it cannot exit the stop from under us.
		t2.ptraceExecveMutexOwner = tg
...
// Preconditions: The caller must have called ptraceExecveMutexStartLock() earlier.
...
// be shared again via clone(2). The caller must have called ptraceExecveMutexStartLock() and
