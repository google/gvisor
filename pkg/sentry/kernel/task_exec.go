...
//     * The next taskRunState is runExecveAfterPtraceLock
...
// The runExecveAfterPtraceLock state continues execve(2) after the lock serializing
// PTRACE_ATACH and execve has been acquired.
//
// +stateify savable
type runExecveAfterPtraceLock struct {
	argv, envv  []string
	pathname    string
	flags       int32
...
func (r *runExecveAfterPtraceLock) execute(t *Task) taskRunState {
	defer func() {
		if r.executable != nil {
	...
	if t.killed() {
		// ptraceExecveMutexUnlock() will not pass us the lock after we were killed, so there is no
...
func (r *runExecveAfterPtraceLock) execveWithImage(t *Task, newImage *TaskImage, newCreds *auth.Credentials, secureExec bool) taskRunState {
...
// serializes PTRACE_ATTACH and execve immediately. The task exits the stop when the lock owner
