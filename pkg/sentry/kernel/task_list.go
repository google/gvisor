...
//
//go:nosplit
func (l *taskList) PushFrontList(m *taskList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		taskElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		taskElementMapper{}.linkerFor(m.tail).SetNext(l.head)
...
//
//go:nosplit
func taskRingAdd(old *Task, new *Task) {
	oldLinker := taskElementMapper{}.linkerFor(old)
	newLinker := taskElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func taskRingEmpty(e *Task) bool {
	linker := taskElementMapper{}.linkerFor(e)
