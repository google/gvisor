...
func CapsFromVfsCaps(vfsCaps linux.VfsNsCapData, creds *Credentials) (*Credentials, bool, error) {
	...
	if !rootIDOwnsCurrentUserns(creds, rootID) {
		// In Linux, security/commoncap.c:get_vfs_caps_from_disk() returns ENODATA
		// in this situation, which causes get_file_caps() to skip vfs caps.
		return creds, false, nil
	}
	// Note that ambient capabilities are not yet supported in gVisor.
	// P'(permitted) = (P(inheritable) & F(inheritable)) | (F(permitted) & P(bounding)) | P'(ambient)
	permittedCaps := (CapabilitySet(vfsCaps.Permitted()) & creds.BoundingCaps) |
	...
	// bit is set, see security/commoncap.c:bprm_caps_from_vfs_caps().
	if effective && (CapabilitySet(vfsCaps.Permitted()) & ^permittedCaps) != 0 {
		return nil, true, linuxerr.EPERM
	}
	// If the capabilities didn't change, return the original creds.
...
// It is analogous to security/commoncap.c:handle_privileged_root().
func HandlePrivilegedRoot(creds *Credentials, hasVFSCaps bool, filename string) *Credentials {
	...
	permittedCaps := creds.PermittedCaps
	if creds.RealKUID == root || creds.EffectiveKUID == root {
		// P'(permitted) = P(inheritable) | P(bounding)
		permittedCaps = creds.BoundingCaps | creds.InheritableCaps
	}
	effectiveCaps := creds.EffectiveCaps
