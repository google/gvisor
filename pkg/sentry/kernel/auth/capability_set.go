...
// IsSubsetOf returns true if the given capability set is a subset of another.
...
// elevated privileges.
//
// The stopPrivGain param is used to deny privilege elevation for reasons beyond NO_NEW_PRIVS.
// The filename param is used for logging.
//
// Note: gVisor does not support Ambient capabilities.
func ComputeCredsForExec(c *Credentials, f FilePrivileges, filename string, stopPrivGain bool) (*Credentials, bool, error) {
	if c.NoNewPrivs {
		f.SetUserID = NoID
		f.SetGroupID = NoID
	}
	// gvisor did not always support setuid/setgid binaries. During the time that it did not,
	// apps might have come to rely on tasks preserving their effective uids when execveing into
	// binaries owned by nobody.
	if f.SetUserID == NobodyKUID {
		f.SetUserID = NoID
	}
	if f.SetGroupID == NobodyKGID {
		f.SetGroupID = NoID
	}
	...
	if !c.UserNamespace.MapFromKUID(f.SetUserID).Ok() {
		f.SetUserID = NoID
	}
	if !c.UserNamespace.MapFromKGID(f.SetGroupID).Ok() {
		f.SetGroupID = NoID
	}
	...
	// or when executed by a process that resides in a descendant of such a namespace." - capabilities(7)
	...
	if (gainedID || gainedCaps) && (c.NoNewPrivs || stopPrivGain) {
		if c.NoNewPrivs || !c.HasCapability(linux.CAP_SETUID) {
