...
func TestUsage(t *testing.T) {
	...
	simpleWorkFnTwoTimePoints := func(_ int, op *Operation, ts []*TimePoint) {
		cap := op.CaptureStart()
		ts[0].Record(cap)
	...
	tests := []testCase{
		{
			...
			successRate:      1.0,
			numRuns:          100,
			workFn:           simpleWorkFnTwoTimePoints,
			checkFn: func(t *testing.T, op *Operation, getValues metricGetValues) {
				op.FlushToMetrics()
				...
				totalAvg := getValues(op.MetricNameForTotalAvg())
				if len(totalAvg) == 0 || totalAvg[0] == 0 {
					t.Fatalf("did not generate valid metric '%s'", op.MetricNameForTotalAvg())
				}
				if m[0] != totalAvg[0] {
					t.Errorf("expected totalAvg == m, got totalAvg=%d, m=%d", totalAvg, m)
				}
			},
		},
		{
		...
		{
			name:             "totalAvgBiggerThanInnerMetrics",
			numTimepoints:    3,
			captureThreshold: 100,
			successRate:      1.0,
			numRuns:          100,
			...
			checkFn: func(t *testing.T, op *Operation, getValues metricGetValues) {
				...
				totalAvg := getValues(op.MetricNameForTotalAvg())
				if len(totalAvg) == 0 || totalAvg[0] == 0 {
					t.Errorf("did not generate valid metric '%s'", op.MetricNameForTotalAvg())
				}
				if m1[0] >= totalAvg[0] || m2[0] >= totalAvg[0] {
					t.Errorf("expected totalAvg > m1 and totalAvg > m2, got totalAvg=%d, m1=%d, m2=%d", totalAvg, m1, m2)
				}
			},
		},
		{
