...
//  2. It is a _sequential_ series of steps that has a well-known start and end;
...
// to report delta between timepoint indexed at idx, and the one preceding it.
...
func (op *Operation) Activate(register MetricRegistration) error {
	...
	op.metrics = make([]MetricSetValue, len(op.bufs)-1)
	for i, b := range op.bufs {
		b.ts = make([]int64, op.maxCaptures)
		if i > 0 {
			metricName := op.MetricNameForTimepoints(i)
			op.metrics[i-1] = register(metricName)
		}
	}
...
func (op *Operation) flushToMetricsLocked() {
	...
	if op.numGenerations == 0 {
		slices.SortFunc(op.bufs, func(a, b *TimePoint) int {
			if a.ts[first] < b.ts[first] {
				return -1
			} else if a.ts[first] > b.ts[first] {
				return 1
			}
			return 0
		})
	...
	for _, tp := range op.bufs {
		tp.generation = op.numGenerations
	}
	op.numCaptures = 0
