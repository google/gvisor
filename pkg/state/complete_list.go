...
//
//go:nosplit
func completeRingInit(e *objectDecodeState) {
	linker := completeElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func completeRingAdd(old *objectDecodeState, new *objectDecodeState) {
	oldLinker := completeElementMapper{}.linkerFor(old)
	newLinker := completeElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func completeRingRemove(e *objectDecodeState) {
	eLinker := completeElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func completeRingEmpty(e *objectDecodeState) bool {
	linker := completeElementMapper{}.linkerFor(e)
