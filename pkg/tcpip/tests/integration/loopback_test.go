...
func TestExternalLoopbackTraffic(t *testing.T) {
	...
	loopbackSourcedICMPv6 := func(e *channel.Endpoint) {
		utils.RxICMPv6EchoRequest(e, ipv6Loopback, utils.Ipv6Addr.Address)
	}
	...
	loopbackDestinedICMPv6 := func(e *channel.Endpoint) {
		utils.RxICMPv6EchoRequest(e, utils.RemoteIPv6Addr, ipv6Loopback)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := s.AddAddress(nicID2, ipv6.ProtocolNumber, ipv6Loopback); err != nil {
				t.Fatalf("AddAddress(%d, %d, %s): %s", nicID2, ipv6.ProtocolNumber, ipv6Loopback, err)
			}
			...
			s.SetRouteTable([]tcpip.Route{
				...
				tcpip.Route{
					Destination: ipv6Loopback.WithPrefix().Subnet(),
					NIC:         nicID2,
				},
			})
