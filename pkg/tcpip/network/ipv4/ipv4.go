...
type endpoint struct {
	...
	mu struct {
		sync.RWMutex
		addressableEndpoint      stack.AddressableEndpoint
...
func (p *protocol) NewEndpoint(nic stack.NetworkInterface, _ stack.LinkAddressCache, _ stack.NUDHandler, dispatcher stack.TransportDispatcher, linkEP stack.LinkEndpoint, st *stack.Stack) stack.NetworkEndpoint {
	...
	e.mu.addressableEndpoint = stack.NewAddressableEndpointWithLock(&e.mu)
...
func (e *endpoint) PrimaryEndpoints() []stack.AddressEndpoint {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.mu.addressableEndpoint.PrimaryEndpoints()
}
...
func (e *endpoint) AllEndpoints() []stack.AddressEndpoint {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.mu.addressableEndpoint.AllEndpoints()
}
...
func (e *endpoint) GetAssignedEndpoint(localAddr tcpip.Address, allowAnyInSubnet, allowTemp bool, tempPEB stack.PrimaryEndpointBehavior) stack.AddressEndpoint {
	...
	// associated subnet.
	if r := e.mu.addressableEndpoint.GetAssignedEndpoint(localAddr, allowAnyInSubnet || e.nic.IsLoopback(), allowTemp, tempPEB); r != nil {
		return r
	}

	eps := e.mu.addressableEndpoint.AllEndpoints()
	for _, r := range eps {
