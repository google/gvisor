...
type endpoint struct {
	...
	// mu is used to synchronize writes that touch multiple fields of the
	...
	// accesses with (enabled, addressableEndpointState, igmp) have their own
	// dedicated methods for synchronization.
	//
	// Given the above assumption, we can allow endpoint internal methods that
	// only perform read operations to be re-entrant (e.g. read addresses state
	// from anywhere without worrying about if the endpoint is locked).
	mu sync.RWMutex
