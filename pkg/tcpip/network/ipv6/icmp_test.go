...
func TestICMPCounts(t *testing.T) {
	...
	ep := netProto.NewEndpoint(&testInterface{}, &stubNUDHandler{}, &stubDispatcher{})
	defer ep.Close()
...
func TestCallsToNeighborCache(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ep := netProto.NewEndpoint(&testInterface{LinkEndpoint: channel.New(0, header.IPv6MinimumMTU, linkAddr0)}, nudHandler, &stubDispatcher{})
			defer ep.Close()
