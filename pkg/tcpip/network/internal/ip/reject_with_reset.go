...
func buildResetPayloadV4(ttl uint8, src, dst tcpip.Address, tcpHdr header.TCP, seq, ack uint32, flags header.TCPFlags) []byte {
	totalHdrLen := header.IPv4MinimumSize + header.TCPMinimumSize
	buf := make([]byte, totalHdrLen)
...
func buildResetPayloadV6(hopLimit uint8, src, dst tcpip.Address, tcpHdr header.TCP, seq, ack uint32, flags header.TCPFlags) []byte {
...
func RejectWithTCPReset(pkt *stack.PacketBuffer, netProto tcpip.NetworkProtocolNumber, stk *stack.Stack, deliveredLocally bool) tcpip.Error {
	...
	tcpHdr, ok := func(pkt *stack.PacketBuffer) (header.TCP, bool) {
		...
		if len(transportHdr) >= header.TCPMinimumSize {
			return header.TCP(transportHdr), true
		}
	...
	// Ref: pkg/tcpip/transport/tcp/protocol.go:replyWithReset
	route, err := stk.FindRoute(pkt.NICID, localAddr, src, netProto, false /* multicastLoop */)
	if err != nil {
		return err
	}
	defer route.Release()
	...
	if netProto == header.IPv4ProtocolNumber {
		buf = buildResetPayloadV4(ttl, src, dst, tcpHdr, seq, ack, flags)
	} else {
		buf = buildResetPayloadV6(ttl, src, dst, tcpHdr, seq, ack, flags)
	}
