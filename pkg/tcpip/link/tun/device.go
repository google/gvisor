...
func (d *Device) encodePkt(info *channel.PacketInfo) (buffer.View, bool) {
	...
	if !d.hasFlags(linux.IFF_NO_PI) {
		...
		hdr.Encode(&PacketInfoFields{
			Protocol: info.Pkt.NetworkProtocolNumber,
		})
	...
	if info.Pkt.LinkHeader().View().IsEmpty() && info.Pkt.LinkPacketInfo.RemoteLinkAddress == "" {
		return nil, false
	}
	...
	if d.hasFlags(linux.IFF_TAP) {
		...
		if info.Pkt.LinkHeader().View().IsEmpty() {
			d.endpoint.AddHeader(info.Pkt.LinkPacketInfo.LocalLinkAddress, info.Pkt.LinkPacketInfo.RemoteLinkAddress, info.Pkt.NetworkProtocolNumber, info.Pkt)
		}
