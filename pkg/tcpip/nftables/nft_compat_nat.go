...
// This file implements the "SNAT" and "DNAT" XTables compatibility target operations.
...
func (op *compatNATTarget) evaluate(regs *registerSet, evalCtx opEvalCtx) {
	opInfo := &op.info
	if opInfo.netProto != 0 && evalCtx.pkt.NetworkProtocolNumber != opInfo.netProto {
		return
	}
...
func (info *natTargetInfo) init(name string, tab *Table, flags uint32, minAddrSlice []byte, minProto, maxProto uint16) *syserr.AnnotatedError {
	...
	if info.changePort {
		...
		if minPort == 0 || minPort > maxPort {
			return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, fmt.Sprintf("invalid port range: min (%d) > max (%d) or min is 0", minPort, maxPort))
		}
...
// unmarshalRev0 parses nf_nat_ipv4_multi_range_compat (revision 0) and fills natTargetInfo.
func (info *natTargetInfo) unmarshalRev0(name string, tab *Table, infoData []byte) *syserr.AnnotatedError {
	if tab.GetAddressFamily() != stack.IP {
		return syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "NAT revision 0 is only valid for IPv4")
	}
...
func parseNATTarget(name string, tab *Table, rev uint32, infoData []byte) (*natTargetInfo, *syserr.AnnotatedError) {
	...
	default:
		return nil, syserr.NewAnnotatedError(syserr.ErrNotSupported, fmt.Sprintf("NAT revision %d not supported", rev))
	}
	if err != nil {
		return nil, err
	}
