...
func (nft *NFTables) DeepCopy() *NFTables {
	nftCopy := &NFTables{
		clock:              nft.clock,
	...
	nftCopy.tableHandleCounter.Store(nft.tableHandleCounter.Load())
	for i, filter := range nft.filters {
		if filter == nil {
			continue
		}
...
func (nft *NFTables) ReplaceNFTables(nftCopy *NFTables) {
	nft.filters = nftCopy.filters
}
