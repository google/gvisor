...
// It evaluates connection tracking state and direction against packet conntrack info.
...
func (op *compatCTMatch) match(pkt *stack.PacketBuffer) bool {
	...
	if !ctOk {
		...
		// Assume invalid state if ctInfo is not found.
	...
	if opInfo.checkDirection {
		matched := (ctInfo.Direction == stack.ConnTrackDirectionReply) != opInfo.invertDirectionMatch
		if !matched {
			return false
		}
	}
...
func parseConntrackMatch(rev uint32, infoData []byte) (*ctMatchInfo, *syserr.AnnotatedError) {
	...
	default:
		return nil, syserr.NewAnnotatedError(syserr.ErrNotSupported, fmt.Sprintf("conntrack revision %d not supported", rev))
	}
	if err != nil {
		return nil, err
	}
