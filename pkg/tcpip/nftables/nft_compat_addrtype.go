...
// Ref: net/netfilter/xt_addrtype.c:addrtype_mt_checkentry_v1()
func parseAddrtypeMatch(rev uint32, infoData []byte) (*addrTypeMatchInfo, *syserr.AnnotatedError) {
	info := &addrTypeMatchInfo{}
	...
	default:
		return nil, syserr.NewAnnotatedError(syserr.ErrNotSupported, fmt.Sprintf("addrtype revision %d not supported", rev))
	}
	if err != nil {
		return nil, err
	}
