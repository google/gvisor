...
func TestEvaluateComparison(t *testing.T) {
	for _, test := range []struct {
		...
		{
			tname: "compare new register != 4-byte data, true",
		...
		{
			tname: "compare new register != 4-byte data, false",
		...
		{
			tname: "compare new register < 4-byte data, false eq",
		...
		{
			tname: "compare new register < 4-byte data, false gt",
		...
		{
			tname: "compare new register > 4-byte data, false lt",
		...
		{
			tname: "compare new register <= 4-byte data, true lt",
		...
		{
			tname: "compare new register <= 4-byte data, true eq",
		...
		{
			tname: "compare new register >= 4-byte data, true gt",
...
func TestJumpGotoLoopBehavior(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.tname, func(t *testing.T) {
			...
			// Creates all chains in the test struct.
			...
			// Note: All chains are added first so loop checking is triggered.
			for chainName, chainInit := range test.chains {
				...
				for _, rule := range chainInit.rules {
					...
					if err := chain.AddRule(rule); err != nil {
						if test.verdict.ChainName != "error" {
							t.Fatalf("unexpected error for AddRule: %v", err)
						}
						return
					}
				}
