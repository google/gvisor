...
func (nf *NFTables) EvaluateHook(family stack.AddressFamily, hook stack.NFHook, pkt *stack.PacketBuffer, route *stack.Route) (stack.NFVerdict, *syserr.AnnotatedError) {
	...
	// Pick the base chain or extra evaluator to evaluate based on priority
	// until one of them is exhausted. Standard merge logic.
	for ei < numExtraHooks && ci < numBaseChains {
		bc := baseChains[ci]
		// If the extra evaluator has a lower priority.
		if extraEvaluators[ei].priority < bc.GetBaseChainInfo().Priority.GetValue() {
	...
	// Evaluate the remaining extra evaluators.
	for ei < numExtraHooks {
		if err := extraEvaluators[ei].handle(); err != nil {
			return stack.NFVerdict{}, err
		}
		if v := regs.Verdict(); isTerminalCode(v.Code) {
	...
	// Evaluate the remaining base chains.
	for ci < numBaseChains {
		bc := baseChains[ci]
		// Doesn't evaluate chain if it's table is flagged as dormant.
		if _, dormant := bc.table.flagSet[TableFlagDormant]; dormant {
			ci++
...
// InitNAT enables NAT functionality in NFTables.
func (nf *NFTables) InitNAT() {
	nf.natInitialized = true
}
...
// removeBaseChain removes a base chain with the specified name from the list.
// Returns false if the chain doesn't exist.
func removeBaseChain(list []*Chain, name string) ([]*Chain, bool) {
