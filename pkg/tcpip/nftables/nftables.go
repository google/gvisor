...
func (nf *NFTables) EvaluateHook(family stack.AddressFamily, hook stack.NFHook, pkt *stack.PacketBuffer) (stack.NFVerdict, *syserr.Error) {
...
func (c *Chain) evaluateFromRule(rIdx int, jumpDepth int, regs *registerSet, pkt *stack.PacketBuffer) *syserr.Error {
	if jumpDepth >= nestedJumpLimit {
		return syserr.ErrTooManyLinks
	}
...
func (c *Chain) evaluate(regs *registerSet, pkt *stack.PacketBuffer) *syserr.Error {
...
func (r *Rule) evaluate(regs *registerSet, pkt *stack.PacketBuffer) *syserr.Error {
...
func (nf *NFTables) FlushAddressFamily(family stack.AddressFamily) *syserr.Error {
	// Ensures address family is valid.
	if err := validateAddressFamily(family); err != nil {
		return err
	}
...
func (nf *NFTables) GetTable(family stack.AddressFamily, tableName string, portID uint32) (*Table, *syserr.Error) {
	// Ensures address family is valid.
	if err := validateAddressFamily(family); err != nil {
		return nil, err
	}
	...
	if nf.filters[family] == nil || nf.filters[family].tables == nil {
		return nil, syserr.ErrNoFileOrDir
	}
...
func (nf *NFTables) AddTable(family stack.AddressFamily, name string,
	errorOnDuplicate bool) (*Table, *syserr.Error) {
	// Ensures address family is valid.
	if err := validateAddressFamily(family); err != nil {
		return nil, err
	}
...
func (nf *NFTables) getNewTableHandle() uint64 {
	for {
		oldVal := nf.tableHandleCounter.Load()
		newVal := oldVal + 1
		if nf.tableHandleCounter.CompareAndSwap(oldVal, newVal) {
			return oldVal
		}
	}
}
...
func (nf *NFTables) CreateTable(family stack.AddressFamily, name string) (*Table, *syserr.Error) {
...
func (nf *NFTables) DeleteTable(family stack.AddressFamily, tableName string) (bool, *syserr.Error) {
	// Ensures address family is valid.
	if err := validateAddressFamily(family); err != nil {
		return false, err
	}
...
func (nf *NFTables) GetChain(family stack.AddressFamily, tableName string, chainName string) (*Chain, *syserr.Error) {
	// Gets and checks the table.
	t, err := nf.GetTable(family, tableName, 0)
	if err != nil {
		return nil, err
...
func (nf *NFTables) AddChain(family stack.AddressFamily, tableName string, chainName string, info *BaseChainInfo, comment string, errorOnDuplicate bool) (*Chain, *syserr.Error) {
	// Gets and checks the table.
	t, err := nf.GetTable(family, tableName, 0)
	if err != nil {
		return nil, err
...
// Note: this interface mirrors the difference between the create and add
// commands within the nft binary.
func (nf *NFTables) CreateChain(family stack.AddressFamily, tableName string, chainName string, info *BaseChainInfo, comment string) (*Chain, *syserr.Error) {
...
func (nf *NFTables) DeleteChain(family stack.AddressFamily, tableName string, chainName string) (bool, *syserr.Error) {
	// Gets and checks the table.
	t, err := nf.GetTable(family, tableName, 0)
	if err != nil {
		return false, err
...
// is not updated.
func (t *Table) SetOwner(nlpid uint32) {
	if t.HasOwner() {
		return
	}
...
func (t *Table) GetLinuxFlagSet() uint32 {
	var flags uint32 = 0
	for flag := range t.flagSet {
		...
		default:
			panic(fmt.Sprintf("only dormant flag is supported, received %v", flag))
		}
	}
...
func (t *Table) GetLinuxUserFlagSet() uint8 {
	return uint8(t.GetLinuxFlagSet() & linux.NFT_TABLE_F_MASK)
}

// GetChain returns the chain with the specified name if it exists, error
// otherwise.
func (t *Table) GetChain(chainName string) (*Chain, *syserr.Error) {
...
func (t *Table) AddChain(name string, info *BaseChainInfo, comment string, errorOnDuplicate bool) (*Chain, *syserr.Error) {
...
func (c *Chain) SetBaseChainInfo(info *BaseChainInfo) *syserr.Error {
...
func (c *Chain) RegisterRule(rule *Rule, index int) *syserr.Error {
...
func (c *Chain) UnregisterRuleByIndex(index int) (*Rule, *syserr.Error) {
...
func (c *Chain) GetRule(index int) (*Rule, *syserr.Error) {
...
func (c *Chain) checkLoops(source *Chain, depth int) *syserr.Error {
...
func (r *Rule) addOperation(op operation) *syserr.Error {
...
func (hfStack *hookFunctionStack) detachBaseChain(name string) *syserr.Error {
