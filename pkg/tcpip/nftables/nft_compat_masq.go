...
func parseMasqTarget(tab *Table, rev uint32, infoData []byte) (*masqTargetInfo, *syserr.AnnotatedError) {
	...
	if rev != 0 {
		return nil, syserr.NewAnnotatedError(syserr.ErrNotSupported, fmt.Sprintf("masquerade revision %d not supported", rev))
	}
	info := &masqTargetInfo{}
	...
	default:
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, fmt.Sprintf("masquerade target unsupported address family: %v", tab.GetAddressFamily()))
	}
	if err != nil {
		return nil, err
	}
