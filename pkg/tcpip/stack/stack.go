...
type Stack struct {
	...
	*ports.PortManager
	...
	// removeConf indicates whether to remove NICs and routes and terminate active
	// connections before saving. This flag will be set to true only for restore.
...
// SetRemoveConf sets the removeConf in stack to true during save/restore.
func (s *Stack) SetRemoveConf() {
	s.mu.Lock()
	defer s.mu.Unlock()
