...
// passed protocol.
func (s *Stack) SetForwarding(protocolNum tcpip.NetworkProtocolNumber, enable bool) {
	// If this stack does not support the protocol for forwarding, do nothing.
...
func (s *Stack) FindNetworkEndpoint(netProto tcpip.NetworkProtocolNumber, address tcpip.Address) (NetworkEndpoint, *tcpip.Error) {
	...
	for _, nic := range s.nics {
		addressEndpoint := nic.getAddressOrCreateTemp(netProto, address, NeverPrimaryEndpoint, none)
		if addressEndpoint == nil {
			continue
		}
