...
func (epsByNIC *endpointsByNIC) handlePacket(id TransportEndpointID, pkt *PacketBuffer) {
	...
	if isInboundMulticastOrBroadcast(pkt.NetworkPacketInfo) {
...
func (d *transportDemuxer) deliverPacket(protocol tcpip.TransportProtocolNumber, pkt *PacketBuffer, id TransportEndpointID) bool {
	...
	// transport endpoints.
	if protocol == header.UDPProtocolNumber && isInboundMulticastOrBroadcast(pkt.NetworkPacketInfo) {
		eps.mu.RLock()
	...
	// addresses.
	if protocol == header.TCPProtocolNumber && (!isSpecified(pkt.NetworkPacketInfo.LocalAddress) || !isSpecified(pkt.NetworkPacketInfo.RemoteAddress) || isInboundMulticastOrBroadcast(pkt.NetworkPacketInfo)) {
