...
//
//go:nosplit
func (l *PacketBufferList) PushFrontList(m *PacketBufferList) {
	if l.head == nil {
		l.head = m.head
		l.tail = m.tail
	} else if m.head != nil {
		PacketBufferElementMapper{}.linkerFor(l.head).SetPrev(m.tail)
		PacketBufferElementMapper{}.linkerFor(m.tail).SetNext(l.head)
