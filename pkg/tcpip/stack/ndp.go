...
type ndpState struct {
	...
	// The SLAAC enabled prefixes.
	autoGenPrefixes map[tcpip.Subnet]autoGenPrefixState
...
// autoGenPrefixState holds state associated with a SLAAC enabled prefix.
type autoGenPrefixState struct {
	...
	// A reference to the referencedNetworkEndpoint of the permanent address
	// generated for the SLAAC enabled prefix.
...
func (ndp *ndpState) handleAutonomousPrefixInformation(pi header.NDPPrefixInformation) {
	...
	// Check if we already know about this SLAAC enabled prefix.
	_, ok := ndp.autoGenPrefixes[prefix]
	if ok {
		// Refresh the SLAAC enabled prefix's lifetimes as outlined in RFC 4862
		// section 5.5.3.e.
		ndp.refreshSLAACPrefixLifetimes(prefix, pl, vl)
		return
	}

	// prefix is a new SLAAC enabled prefix. Do the work as outlined by RFC 4862
	// section 5.5.3.d if ndp is configured to auto-generate new addresses via
	// SLAAC.
...
func (ndp *ndpState) doSLAAC(prefix tcpip.Subnet, pl, vl time.Duration) {
	...
	// deprecated.
	deprecated := pl == 0
	ref := ndp.newSLAACAddr(prefix, deprecated)
	if ref == nil {
	...
	state := autoGenPrefixState{
		deprecationTimer: tcpip.MakeCancellableTimer(&ndp.nic.mu, func() {
			prefixState, ok := ndp.autoGenPrefixes[prefix]
			if !ok {
				log.Fatalf("ndp: must have an autoGenPrefixes entry for the SLAAC enabled prefix %s", prefix)
			}

			prefixState.ref.deprecated = true
			ep := prefixState.ref.ep
			ndp.notifyAutoGenAddressDeprecated(tcpip.AddressWithPrefix{
				Address:   ep.ID().LocalAddress,
				PrefixLen: ep.PrefixLen(),
			})
		}),
		invalidationTimer: tcpip.MakeCancellableTimer(&ndp.nic.mu, func() {
			ndp.invalidateAutoGenPrefix(prefix, true)
		}),
		ref: ref,
	}
	...
	ndp.autoGenPrefixes[prefix] = state
}

// newSLAACAddr generates a new SLAAC address for prefix.
func (ndp *ndpState) newSLAACAddr(prefix tcpip.Subnet, deprecated bool) *referencedNetworkEndpoint {
...
// refreshSLAACPrefixLifetimes refreshes the lifetimes of a SLAAC enabled
// prefix.
//
// pl is the new preferred lifetime. vl is the new valid lifetime.
func (ndp *ndpState) refreshSLAACPrefixLifetimes(prefix tcpip.Subnet, pl, vl time.Duration) {
	...
	defer func() { ndp.autoGenPrefixes[prefix] = prefixState }()

	// If the preferred lifetime is zero, then the prefix should be considered
	// deprecated.
	deprecated := pl == 0
	wasDeprecated := prefixState.ref.deprecated
	prefixState.ref.deprecated = deprecated

	// Only send the deprecation event for the auto-generated address if the
	// deprecated status for prefix just changed from non-deprecated to
	// deprecated.
...
// has been deprecated.
func (ndp *ndpState) notifyAutoGenAddressDeprecated(addr tcpip.AddressWithPrefix) {
	if ndpDisp := ndp.nic.stack.ndpDisp; ndpDisp != nil {
		ndpDisp.OnAutoGenAddressDeprecated(ndp.nic.ID(), addr)
	}
}

// invalidateAutoGenPrefix invalidates a SLAAC enabled prefix.
//
// The NIC that ndp belongs to MUST be locked.
func (ndp *ndpState) invalidateAutoGenPrefix(prefix tcpip.Subnet, removeAddr bool) {
	state, ok := ndp.autoGenPrefixes[prefix]
	if !ok {
		return
	}
	...
	delete(ndp.autoGenPrefixes, prefix)
...
// cleanupAutoGenAddrResourcesAndNotify cleans up an invalidated SLAAC
// address's resources from ndp.
//
// The NIC that ndp belongs to MUST be lockexod.
func (ndp *ndpState) cleanupAutoGenAddrResourcesAndNotify(addr tcpip.AddressWithPrefix) {
	ndp.invalidateAutoGenPrefix(addr.Subnet(), false)
}
...
func (ndp *ndpState) cleanupState(hostOnly bool) {
	...
	linkLocalPrefixes := 0
	for prefix := range ndp.autoGenPrefixes {
	...
	if got := len(ndp.autoGenPrefixes); got != linkLocalPrefixes {
		log.Fatalf("ndp: still have non-linklocal SLAAC enabled prefixes after cleaning up; found = %d prefixes, of which %d are link-local", got, linkLocalPrefixes)
	}
