...
type PacketBufferOptions struct {
	...
	// OnRelease is a function to run when the packet buffer is no longer
...
// ResetHeaders clears headers in the underlying buffer, resets all header
// fields, and prepends reserved space for new headers in pk.
func (pk *PacketBuffer) ResetHeaders(reserved int) {
	pk.buf.Remove(0, pk.dataOffset())
	pk.headers = [numHeaderType]headerInfo{}
	pk.consumed = 0
	pk.pushed = 0
	pk.reserved = reserved
	pk.buf.Prepend(make([]byte, reserved))
}
