...
func unknownToIncomplete(e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock) error {
	if err := func() error {
		...
		if e.neigh.State != Unknown {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Unknown)
		}
		e.handlePacketQueuedLocked(entryTestAddr2)
		if e.neigh.State != Incomplete {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Incomplete)
		}
		return nil
	}(); err != nil {
		return err
	...
	{
		nudDisp.mu.Lock()
		diff := cmp.Diff(wantEvents, nudDisp.events)
		nudDisp.events = nil
		nudDisp.mu.Unlock()
		if diff != "" {
...
func unknownToStale(e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock) error {
	if err := func() error {
		...
		if e.neigh.State != Stale {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Stale)
		}
		return nil
	}(); err != nil {
		return err
...
func TestEntryIncompleteToIncompleteDoesNotChangeUpdatedAt(t *testing.T) {
	...
	for i := uint32(1); i < c.MaxMulticastProbes; i++ {
		...
		e.mu.Lock()
		if got, want := e.neigh.UpdatedAt, startedAt; got != want {
	...
	nudDisp.mu.Lock()
	if diff := cmp.Diff(wantEvents, nudDisp.events); diff != "" {
		t.Errorf("nud dispatcher events mismatch (-want, +got):\n%s", diff)
	}
	nudDisp.mu.Unlock()
}
...
func incompleteToReachableWithFlags(e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock, flags ReachabilityConfirmationFlags) error {
	if err := func() error {
		...
		if e.neigh.State != Reachable {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Reachable)
		}
		if e.neigh.LinkAddr != entryTestLinkAddr1 {
			return fmt.Errorf("got e.neigh.LinkAddr = %q, want = %q", e.neigh.LinkAddr, entryTestLinkAddr1)
		}
		return nil
	}(); err != nil {
		return err
...
func TestEntryIncompleteToStaleWhenUnsolicitedConfirmation(t *testing.T) {
	...
	if e.isRouter {
		t.Errorf("got e.mu.isRouter = %t, want = false", e.isRouter)
	}
	e.mu.Unlock()
...
func incompleteToUnreachable(c NUDConfigurations, e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock) error {
	{
		...
		e.mu.Unlock()
		if state != Incomplete {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", state, Incomplete)
		}
	}
...

	{
		...
		if state != Unreachable {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", state, Unreachable)
		}
	}
...
func TestEntryReachableToReachableClearsRouterWhenConfirmationWithoutRouter(t *testing.T) {
	...
	// No events should have been dispatched.
	nudDisp.mu.Lock()
	diff := cmp.Diff([]testEntryEventInfo(nil), nudDisp.events)
	nudDisp.mu.Unlock()
	if diff != "" {
		t.Errorf("nud dispatcher events mismatch (-want, +got):\n%s", diff)
	}
...
func reachableToStale(c NUDConfigurations, e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock) error {
...

	{
		...
		e.mu.Unlock()
		if state != Reachable {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", state, Reachable)
		}
	}
...

	{
		...
		e.mu.Unlock()
		if state != Stale {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", state, Stale)
		}
	}
...
func staleToDelay(e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock) error {
	if err := func() error {
		...
		if e.neigh.State != Delay {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Delay)
		}
		return nil
	}(); err != nil {
		return err
...
func delayToProbe(c NUDConfigurations, e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock) error {
	{
		...
		e.mu.Unlock()
		if state != Delay {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", state, Delay)
		}
	}
...

	{
		...
		e.mu.Unlock()
		if state != Probe {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", state, Probe)
		}
	}
...
func probeToReachableWithFlags(e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock, linkAddr tcpip.LinkAddress, flags ReachabilityConfirmationFlags) error {
	if err := func() error {
		...
		prevLinkAddr := e.neigh.LinkAddr
		if e.neigh.State != Probe {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Probe)
		}
		...
		if e.neigh.LinkAddr != linkAddr {
			return fmt.Errorf("got e.neigh.LinkAddr = %q, want = %q", e.neigh.LinkAddr, linkAddr)
		}
		return nil
	}(); err != nil {
		return err
...
func unreachableToIncomplete(e *neighborEntry, nudDisp *testNUDDispatcher, linkRes *entryTestLinkResolver, clock *faketime.ManualClock) error {
	if err := func() error {
		...
		if e.neigh.State != Unreachable {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Unreachable)
		}
		e.handlePacketQueuedLocked(entryTestAddr2)
		if e.neigh.State != Incomplete {
			return fmt.Errorf("got e.neigh.State = %q, want = %q", e.neigh.State, Incomplete)
