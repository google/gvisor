...
type fwdTestNetworkProtocol struct {
	...
	onLinkAddressResolved  func(neigh *neighborCache, addr tcpip.Address, _ tcpip.LinkAddress)
...
func (f *fwdTestNetworkProtocol) NewEndpoint(nic NetworkInterface, _ NUDHandler, dispatcher TransportDispatcher) NetworkEndpoint {
...
func (f *fwdTestNetworkProtocol) LinkAddressRequest(addr, _ tcpip.Address, remoteLinkAddr tcpip.LinkAddress, _ NetworkInterface) *tcpip.Error {
	if f.onLinkAddressResolved != nil {
		time.AfterFunc(f.addrResolveDelay, func() {
			f.onLinkAddressResolved(f.neigh, addr, remoteLinkAddr)
		})
	}
	return nil
}
...
func TestForwardingWithFakeResolver(t *testing.T) {
	proto := &fwdTestNetworkProtocol{
		...
		onLinkAddressResolved: func(neigh *neighborCache, addr tcpip.Address, remoteLinkAddr tcpip.LinkAddress) {
			t.Helper()
