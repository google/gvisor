...
func (n *NIC) getRefOrCreateTempInner(protocol tcpip.NetworkProtocolNumber, address tcpip.Address, createTemp bool, peb PrimaryEndpointBehavior) *referencedNetworkEndpoint {
	if protocol == 0 {
		for p, ep := range n.networkEndpoints {
			if nep := ep.GetAssignedEndpoint(address, false /* allowAnyInSubnet */, createTemp, peb); nep != nil {
...
func (n *NIC) AddAddress(protocolAddress tcpip.ProtocolAddress, peb PrimaryEndpointBehavior) *tcpip.Error {
	...
	_, err := ep.AddAddress(protocolAddress.AddressWithPrefix, AddAddressOptions{
		Deprecated: false,
...
func (n *NIC) RemoveAddress(addr tcpip.Address) *tcpip.Error {
	for _, ep := range n.networkEndpoints {
		if err := ep.RemoveAddress(addr); err == tcpip.ErrBadLocalAddress {
			continue
		} else {
			return err
		}
...
func (n *NIC) leaveGroup(protocol tcpip.NetworkProtocolNumber, addr tcpip.Address) *tcpip.Error {
	...
	if _, err := gep.LeaveGroup(addr, false /* force */); err != nil {
		return err
	}
