...
// setEnabledLocked sets the enabled status for the NIC.
//
// Precondition: n.mu must be write locked.
func (n *NIC) setEnabledLocked(v bool) {
	if v {
		atomic.StoreUint32(&n.atomics.enabled, 1)
	} else {
		atomic.StoreUint32(&n.atomics.enabled, 0)
	}
}
...
// remove detaches NIC from the link endpoint and releases NetworkEndpoint
...
// getAddressEpOrCreateTemp returns the AssignableAddressEndpoint for the given
...
func (n *NIC) DeliverNetworkPacket(remote, local tcpip.LinkAddress, protocol tcpip.NetworkProtocolNumber, pkt *PacketBuffer) {
	...
	if n.stack.Forwarding(protocol) {
		...
		// Found a NIC.
		n := r.nic
		addressEndpoint := n.getAddressOrCreateTempInner(protocol, dst, false, NeverPrimaryEndpoint)
		ok := addressEndpoint != nil && n.isValidForOutgoing(addressEndpoint)
		if ok {
