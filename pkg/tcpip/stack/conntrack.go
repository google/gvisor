...
// ConnTrackTable contains a map of connections created as per nat rules.
...
func parseHeaders(pkt *PacketBuffer) {
	...
	headerView = headerView[netHeader.HeaderLength():]
	...
	case header.UDPProtocolNumber:
		if newPkt.TransportHeader == nil {
			...
			hdr := headerView[:header.UDPMinimumSize]
			newPkt.TransportHeader = buffer.View(header.UDP(hdr))
		}
	case header.TCPProtocolNumber:
		if newPkt.TransportHeader == nil {
			...
			hdr := headerView[:header.TCPMinimumSize]
			newPkt.TransportHeader = buffer.View(header.TCP(hdr))
		}
	}
