...
type AddressableEndpoint interface {
	...
	//
	// If allowInSubnet is true, the endpoint will be forced to consider itself
	// bound to all addresses in associated subnets. Note, passing a value of
	// false does not mean that an address not explicitly bound to the endpoint
	// won't be returned - the endpoint may still consider itself bound to all IPs
	// in a subnet anyways.
	//
