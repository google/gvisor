...
//
//go:nosplit
func sockErrorRingInit(e *SockError) {
	linker := sockErrorElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func sockErrorRingAdd(old *SockError, new *SockError) {
	oldLinker := sockErrorElementMapper{}.linkerFor(old)
	newLinker := sockErrorElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func sockErrorRingRemove(e *SockError) {
	eLinker := sockErrorElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func sockErrorRingEmpty(e *SockError) bool {
	linker := sockErrorElementMapper{}.linkerFor(e)
