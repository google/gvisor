...
func (l *listenContext) createEndpointAndPerformHandshake(s *segment, opts *header.TCPSynOptions, queue *waiter.Queue) (*endpoint, *tcpip.Error) {
	...
	if err := h.execute(); err != nil {
		ep.Close()
		// Wake up any waiters. This is strictly not required normally as a socket that was never
		// accepted can't really have any registered waiters except when stack.Wait() is called
		// which waits for all registered endpoints to stop and expects a EventHUp.
