...
func (l *listenContext) createConnectingEndpoint(s *segment, iss seqnum.Value, irs seqnum.Value, rcvdSynOpts *header.TCPSynOptions, queue *waiter.Queue) (*endpoint, *tcpip.Error) {
	...
	route, err := l.stack.FindRoute(s.nicID, s.networkPacketInfo.LocalAddress, s.networkPacketInfo.RemoteAddress, s.netProto, false /* multicastLoop */)
	if err != nil {
		return nil, err
	}
	route.ResolveWith(s.linkPacketInfo.RemoteLinkAddress)
...
func (e *endpoint) handleListenSegment(ctx *listenContext, s *segment) {
	...
	case s.flags == header.TCPFlagSyn:
		...
		if ctx.synRcvdCount.inc() {
			...
			route, err := e.stack.FindRoute(s.nicID, s.networkPacketInfo.LocalAddress, s.networkPacketInfo.RemoteAddress, s.netProto, false /* multicastLoop */)
			if err != nil {
				return
			}
			route.ResolveWith(s.linkPacketInfo.RemoteLinkAddress)
