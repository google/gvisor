...
func (rc *rackControl) update(seg *segment, ackSeg *segment) {
	rtt := rc.snd.ep.sinceNowMonotonicTimeNS(seg.xmitMonotonicTimeNS)
	...
	if seg.xmitCount > 1 {
		if ackSeg.parsedOptions.TS && ackSeg.parsedOptions.TSEcr != 0 {
			if ackSeg.parsedOptions.TSEcr < tcpTimeStamp(nsToMS(seg.xmitMonotonicTimeNS), tsOffset) {
				return
			}
		}
		if rtt < rc.minRTT {
	...
	if rc.XmitMonotonicTimeNS < seg.xmitMonotonicTimeNS || (seg.xmitMonotonicTimeNS == rc.XmitMonotonicTimeNS && rc.EndSequence.LessThan(endSeq)) {
		rc.XmitMonotonicTimeNS = seg.xmitMonotonicTimeNS
...
func (rc *rackControl) detectLoss(rcvMonotonicTimeNS int64) int {
	...
	for seg := rc.snd.writeList.Front(); seg != nil && seg.xmitCount != 0; seg = seg.Next() {
		...
		endSeq := seg.sequenceNumber.Add(seqnum.Size(seg.data.Size()))
		if seg.xmitMonotonicTimeNS < rc.XmitMonotonicTimeNS || (seg.xmitMonotonicTimeNS == rc.XmitMonotonicTimeNS && rc.EndSequence.LessThan(endSeq)) {
			timeRemaining := nsTimeDifference(seg.xmitMonotonicTimeNS, rcvMonotonicTimeNS) + rc.RTT + rc.ReoWnd
			if timeRemaining <= 0 {
				seg.lost = true
				numLost++
...
func (rc *rackControl) reorderTimerExpired() tcpip.Error {
	...
	numLost := rc.detectLoss(rc.snd.ep.monotonicTimeNS())
	if numLost == 0 {
		return nil
	}
