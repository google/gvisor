...
func buildTCPHdr(r *stack.Route, tf tcpFields, pkt *stack.PacketBuffer, gso stack.GSO) {
	if pkt.TransportHeader().View() != nil {
		return
	}
	optLen := len(tf.opts)
...
func sendTCP(r *stack.Route, tf tcpFields, pkt *stack.PacketBuffer, gso stack.GSO, owner tcpip.PacketOwner) tcpip.Error {
	...
	// We need to create a new packet because WritePacket can modify pkt's data,
	// and pkt is held by a segment that could be reprocessed later on.
	{
		sendPkt := pkt.Clone()
		defer sendPkt.DecRef()
		if sendPkt.TransportHeader().View() == nil {
			sendPkt.ReserveHeaderBytes(header.TCPMinimumSize + int(r.MaxHeaderLength()) + optLen)
		}
		pkt = sendPkt
	}
