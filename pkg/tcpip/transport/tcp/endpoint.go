...
type endpoint struct {
	...
	// goroutine what it was notified.
	notifyFlags atomicbitops.Uint32 `state:"nosave"`
...
func (e *endpoint) fetchNotifications() uint32 {
	return e.notifyFlags.Swap(0)
}

func (e *endpoint) notifyProtocolGoroutine(n uint32) {
	for {
		v := e.notifyFlags.Load()
		if v&n == n {
			// The flags are already set.
			return
		}

		if e.notifyFlags.CompareAndSwap(v, v|n) {
			if v == 0 {
