...
//
//go:nosplit
func segmentRingInit(e *segment) {
	linker := segmentElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func segmentRingAdd(old *segment, new *segment) {
	oldLinker := segmentElementMapper{}.linkerFor(old)
	newLinker := segmentElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func segmentRingRemove(e *segment) {
	eLinker := segmentElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func segmentRingEmpty(e *segment) bool {
	linker := segmentElementMapper{}.linkerFor(e)
