...
func (e *endpoint) HandlePacket(pkt *stack.PacketBuffer) {
	...
	if e.bound {
		...
		if e.BindAddr != "" && e.BindAddr != pkt.NetworkPacketInfo.RemoteAddress {
			e.rcvMu.Unlock()
			return
		}
	}
	...
	// connected to.
	if e.connected && e.route.RemoteAddress != pkt.NetworkPacketInfo.RemoteAddress {
		e.rcvMu.Unlock()
		return
	}
	...
	packet := &rawPacket{
		senderAddr: tcpip.FullAddress{
			NIC:  pkt.NICID,
			Addr: pkt.NetworkPacketInfo.RemoteAddress,
		},
	}
