...
//
//go:nosplit
func udpPacketRingInit(e *udpPacket) {
	linker := udpPacketElementMapper{}.linkerFor(e)
	linker.SetNext(e)
	linker.SetPrev(e)
}
...
//
//go:nosplit
func udpPacketRingAdd(old *udpPacket, new *udpPacket) {
	oldLinker := udpPacketElementMapper{}.linkerFor(old)
	newLinker := udpPacketElementMapper{}.linkerFor(new)
	next := oldLinker.Next()
	prev := old
...
//
//go:nosplit
func udpPacketRingRemove(e *udpPacket) {
	eLinker := udpPacketElementMapper{}.linkerFor(e)
	next := eLinker.Next()
	prev := eLinker.Prev()
	next.SetPrev(prev)
...
//
//go:nosplit
func udpPacketRingEmpty(e *udpPacket) bool {
	linker := udpPacketElementMapper{}.linkerFor(e)
