...
func verifyChecksum(hdr header.UDP, pkt *stack.PacketBuffer) bool {
	if pkt.LinkPacketInfo.InterfaceCapabilities&stack.CapabilityRXChecksumOffload == 0 &&
		(hdr.Checksum() != 0 || pkt.NetworkProtocolNumber == header.IPv6ProtocolNumber) {
		xsum := header.PseudoHeaderChecksum(ProtocolNumber, pkt.NetworkPacketInfo.LocalAddress, pkt.NetworkPacketInfo.RemoteAddress, hdr.Length())
...
func (e *endpoint) HandlePacket(id stack.TransportEndpointID, pkt *stack.PacketBuffer) {
	...
	packet.packetInfo.LocalAddr = pkt.NetworkPacketInfo.LocalAddress
	packet.packetInfo.DestinationAddr = pkt.NetworkPacketInfo.LocalAddress
