...
func (e *endpoint) write(p tcpip.Payloader, opts tcpip.WriteOptions) (int64, tcpip.Error) {
	...
	u, err := e.prepareForWrite(p, opts)
	if err != nil {
		return 0, err
	}
	...
	if _, err := e.net.Write(func(netProto tcpip.NetworkProtocolNumber, src, dst tcpip.Address, maxHeaderLength int, requiresTXChecksum bool) (*stack.PacketBuffer, tcpip.Error) {
		...
		if requiresTXChecksum &&
			(!e.ops.GetNoChecksum() || netProto == header.IPv6ProtocolNumber) {
			udp.SetChecksum(^udp.CalculateChecksum(header.ChecksumCombine(
				header.PseudoHeaderChecksum(ProtocolNumber, src, dst, length),
...
func (e *endpoint) Connect(addr tcpip.FullAddress) tcpip.Error {
	...
	if err := e.net.ConnectAndThen(addr, func(netProto tcpip.NetworkProtocolNumber, previousID, nextID stack.TransportEndpointID) tcpip.Error {
...
func (e *endpoint) bindLocked(addr tcpip.FullAddress) tcpip.Error {
	...
	if err := e.net.BindAndThen(addr, func(boundNetProto tcpip.NetworkProtocolNumber, boundAddr tcpip.Address) tcpip.Error {
