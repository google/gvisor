...
func (b IPv4) IsValid(pktSize int, verifyChecksum bool) bool {
	...
	if verifyChecksum && !b.IsChecksumValid() {
		return false
	}
...
// IsChecksumValid performs checksum validation on the packet.
func (b IPv4) IsChecksumValid() bool {
	// There has been some confusion regarding verifying checksums. We need just
	// look for negative 0 (0xffff) as the checksum, as it's not possible to get
	// positive 0 (0) for the checksum. Some bad implementations could get it when
	// doing entry replacement in the early days of the Internet, however the lore
	// that one needs to check for both persists.
	//
