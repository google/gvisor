...
func (s *Set) Insert(gap GapIterator, r Range, val Value) Iterator {
	...
	if prev.Ok() && prev.End() == r.Start {
		if mval, ok := (Functions{}).Merge(prev.Range(), prev.Value(), r, val); ok {
			shrinkMaxGap := gap.Range().Length() == gap.node.maxGap
...
func (s *Set) InsertWithoutMergingUnchecked(gap GapIterator, r Range, val Value) Iterator {
	...
	splitMaxGap := gap.node.nrSegments == 0 || gap.Range().Length() == gap.node.maxGap
...
func (s *Set) Remove(seg Iterator) GapIterator {
	...
	if seg.node.hasChildren {
		...
		// must have been modified by update seg.Range() to victim.Range().
		nextAdjacentNode := seg.NextSegment().node
		nextAdjacentNode.updateMaxGapLeaf()
		// At this moment the maxGap of the set may still not be well maintained
		// and it will be fixed after the Remove of victim.
...
func (n *node) rebalanceBeforeInsert(gap GapIterator) GapIterator {
	...
	if n.parent == nil {
		...
		// but the left and right children should be updated locally as they are newly split from n.
...
func (n *node) updateMaxGapLeaf() {
	...
	for p := n.parent; p != nil; p = p.parent {
		...
		p.maxGap = parentNewMax
	}
}

// updateMaxGapLocal updates maxGap of the calling node solely with no propagation to ancestor nodes.
func (n *node) updateMaxGapLocal() {
	if !n.hasChildren {
		// Leaf node iterates its gaps.
		n.maxGap = n.calculateMaxGapLeaf()
	} else {
		// Non-leaf node iterates its children.
		n.maxGap = n.calculateMaxGapInternal()
	}
}

// calculateMaxGapLeaf iterates the gaps within a leaf node and calculate the max.
//
// Preconditions: n must be a leaf node.
...
// calculateMaxGapInternal iterates children's maxGap within an internal node n and calculate the max.
//
...
// NextLargeEnoughGap returns the iterated gap's first next gap with larger length than minSize.
// If not found, return a terminal gap iterator (does NOT include this gap itself).
...
// NextLargeEnoughGapHelper is the helper function used by NextLargeEnoughGap to do the real recursions.
//
...
func (gap GapIterator) NextLargeEnoughGapHelper(minSize Key) GapIterator {
	...
	if gap.node != nil && gap.index == gap.node.nrSegments {
		// If gap is the trailing gap of a non-leaf node, crawl up to parent again and do recursion.
		gap.node, gap.index = gap.node.parent, gap.node.parentIndex
	}
	return gap.NextLargeEnoughGapHelper(minSize)
}

// PrevLargeEnoughGap returns the iterated gap's first prev gap with larger or equal length than minSize.
// If not found, return a terminal gap iterator (does NOT include this gap itself).
...
// PrevLargeEnoughGapHelper is the helper function used by PrevLargeEnoughGap to do the real recursions.
//
...
func (gap GapIterator) PrevLargeEnoughGapHelper(minSize Key) GapIterator {
	...
	if gap.node != nil && gap.index == 0 {
		// If gap is the first gap of a non-leaf node, crawl up to parent again and do recursion.
		gap.node, gap.index = gap.node.parent, gap.node.parentIndex
	}
	return gap.PrevLargeEnoughGapHelper(minSize)
}
