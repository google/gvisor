...
func removeDeadCode(insns []Instruction) ([]Instruction, bool) {
	reachable, err := findReachable(insns)
	if err != nil {
		panic(fmt.Sprintf("program was expected to be consistent before being optimized, but cannot determine instruction reachability: %v", err))
	}
