...
const (
	...
	maxUnconditionalJumpOffset = 65536
)
...
// skipRedundantLoads looks for jumps that go to "Load" instructions for
// which the `A` register already contains the same contents being loaded.
// For each such jump, it moves the jump target forward by one instruction
// in order to skip the redundant load.
func skipRedundantLoads(insns []Instruction) ([]Instruction, bool) {
	changed := false
	for pc, ins := range insns {
		if !ins.IsDeterministicLoad() {
			continue
		}
		// Look for all jumps that target this instruction.
		// We stop looking at instruction 1 because we can only optimize
		// if there is another load instruction *before* the jump instruction.
		for pc2 := pc - 1; pc2 >= 1; pc2-- {
			// Check if this is a jump instruction we're looking for.
			ins2 := insns[pc2]
			if !ins2.IsJump() {
				continue
			}
			jumpsToPC := false
			for _, offset := range ins2.JumpOffsets() {
				target := pc2 + int(offset.Offset) + 1
				if target != pc {
					continue
				}
				// Check if `target` can be incremented by one while still fitting
				// in jump limits.
				if offset.Offset == offset.Type.Max() {
					continue
				}
				jumpsToPC = true
				break
			...
			jumpPC := pc2
			jumpIns := insns[jumpPC]

			// Now walk backwards to see which instruction last modified "A".
			// We only consider "fallthrough" instructions rather than fully
			// walking the program backwards (possible in BPF, but complicated).
			lastPCModifyingA := -1
			for pc3 := pc2 - 1; pc3 >= 0; pc3-- {
				isn3 := insns[pc3]
				if !isn3.MayFallThrough() {
					break
				}
				if isn3.ModifiesRegisterA() {
					lastPCModifyingA = pc3
					break
				}
			}
			if lastPCModifyingA == -1 {
				continue
			}
			if loadIns := insns[lastPCModifyingA]; !loadIns.IsDeterministicLoad() || loadIns != ins {
				continue
			}

			// Check if there is any place in the program which may jump between
			// `ins2` (the jump to the load instruction) and `ins3` (the earlier
			// load instruction). If there is, then we can't optimize `ins2`.
			// We ignore where jump instructions that exist between `ins3` and
			// `ins2` are going; all that matters are jump instructions earlier
			// than `ins3`.
			jumpBetweenLoadsExists := false
			for pc4 := lastPCModifyingA - 1; pc4 >= 0 && !jumpBetweenLoadsExists; pc4-- {
				if !insns[pc4].IsJump() {
					continue
				}
				for _, offset := range insns[pc4].JumpOffsets() {
					if target := pc4 + int(offset.Offset) + 1; target > lastPCModifyingA && target <= jumpPC {
						jumpBetweenLoadsExists = true
						break
					}
				}
			}
			if jumpBetweenLoadsExists {
				continue
			}

			// We can finally optimize.
			if jumpIns.IsConditionalJump() {
				if trueTarget := jumpPC + int(jumpIns.JumpIfTrue) + 1; trueTarget == pc {
					jumpIns.JumpIfTrue++
				}
				if falseTarget := jumpPC + int(jumpIns.JumpIfFalse) + 1; falseTarget == pc {
					jumpIns.JumpIfFalse++
				}
			} else {
				jumpIns.K++
			}
			insns[jumpPC] = jumpIns
			changed = true
		}
	}
	return insns, changed
