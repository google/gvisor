...
func NewContainer(ctx context.Context, platform stdio.Platform, conf *ContainerConfig) (*Container, error) {
	...
	if conf.RestoreImagePath != "" {
		fi, err := os.Stat(conf.RestoreImagePath)
		if err != nil {
			return nil, fmt.Errorf("checkpoint %q: %w", conf.RestoreImagePath, err)
		}
		if !fi.IsDir() {
			return nil, fmt.Errorf("checkpoint %q: not a directory", conf.RestoreImagePath)
		}
	}
	...
	// Success
	cu.Release()
	var restoreConf *extension.RestoreConfig
	if conf.RestoreImagePath != "" {
		restoreConf = &extension.RestoreConfig{
