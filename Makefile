...
staged-binaries-check: ## Verifies STAGED_BINARIES matches the //debian:gvisor-release-tar fileset.
ifeq (,$(STAGED_BINARIES))
	...
	diff -u "$$T/release.txt" "$$T/staged.txt" \
	  || { echo "ERROR: STAGED_BINARIES member set diverges from //debian:gvisor-release-tar" >&2; rm -rf "$$T"; exit 1; }; \
	rm -rf "$$T"
endif
